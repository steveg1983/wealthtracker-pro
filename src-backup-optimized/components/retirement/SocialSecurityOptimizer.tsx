...
import { TrendingUpIcon, DollarSignIcon, CalendarIcon, InfoIcon } from '../icons';
...
export default function SocialSecurityOptimizer(): React.JSX.Element {
  ...
  useEffect(() => {
    calculateBenefits();
  }, [birthYear, averageEarnings, yearsWorked, claimingAge, lifeExpectancy]);
  
  const calculateBenefits = () => {
    // Calculate for selected claiming age
    const result = usRetirementService.calculateSocialSecurity(
      averageEarnings / 12, // Convert to monthly
      birthYear,
      claimingAge,
      yearsWorked
    );
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input
            type="number"
            value={claimingAge}
            onChange={(e) => setClaimingAge(parseFloat(e.target.value))}
            min="62"
            max="70"
            step="0.25"
            className="w-full px-3 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600"
                    ...
                    <p className="text-lg font-bold">
                      {claimingAge < calculation.fullRetirementAge ? (
                        <span className="text-red-600 dark:text-red-400">
                          -{(calculation.reduction * 100).toFixed(1)}%
                        </span>
                      ) : claimingAge > calculation.fullRetirementAge ? (
                        <span className="text-green-600 dark:text-green-400">
                          +{((claimingAge - calculation.fullRetirementAge) * 8).toFixed(1)}%
                        </span>
                      ) : (
                ...
                {calculation.breakEvenAge && (
                  <div className="mt-3 p-2 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
                    <p className="text-xs text-yellow-800 dark:text-yellow-200">
                      Break-even age vs. FRA: {calculation.breakEvenAge.toFixed(1)} years
                    </p>
                  </div>
                )}
              </div>
              ...
              {optimization && (
                <div className="mb-6">
                  ...
                  <div className="space-y-2">
                    ...
                    {[62, 65, 67, 70].map(age => {
                      const data = optimization.analysis.find((a: any) => Math.floor(a.age) === age);
                      if (!data) return null;
                      const percentage = (data.lifetime / optimization.lifetimeBenefit) * 100;
