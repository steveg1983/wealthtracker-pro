...
interface RetirementPlanFormValues {
  name: string;
  description?: string;
  data: {
    currentAge: number;
...
export default function RetirementPlanModal({
  ...
  onClose,
  onSave
}: { plan: FinancialPlan | null; onClose: () => void; onSave: (values: RetirementPlanFormValues) => void }): React.JSX.Element {
  const { formatCurrency: formatRegionalCurrency } = useRegionalCurrency();
  const financialData = useRealFinancialData();
  const { accounts } = useApp();

  const totalRetirementSavings = useMemo(() => accounts
    .filter(acc => acc.type === 'investment' || acc.subtype === 'retirement')
    .reduce((sum, acc) => sum + (acc.balance || 0), 0), [accounts]);

  const monthlyIncomeDefault = financialData ? financialData.monthlyIncome.toNumber() : 0;
  const planData = (plan?.data as Record<string, unknown>) ?? {};

  const [formData, setFormData] = useState({
    name: plan?.name || '',
    currentAge: parseNumericField(planData.currentAge, 35),
    retirementAge: parseNumericField(planData.retirementAge, 65),
    currentSavings: parseNumericField(planData.currentSavings, totalRetirementSavings || 0),
    monthlyContribution: parseNumericField(planData.monthlyContribution, 0),
    expectedReturn: parseNumericField(planData.expectedReturn, 0.07),
    inflationRate: parseNumericField(planData.inflationRate, 0.025),
    targetRetirementIncome: parseNumericField(planData.targetRetirementIncome, monthlyIncomeDefault)
  });
...
export type { RetirementPlanFormValues };
