...
| the renderer builds | 3,790.5 KiB raw / 1,163.8 KiB gzipped over 187 files (1 Sep 2026). It was 3,273.1 / 1,006.5 when the application was first mounted, 259.3 / 86.7 when it was React and a file chooser, and 81.8 kB when it was one screen of vanilla DOM. `npm run bundle:check:desktop` is that measurement as a command, with budgets ~1 % above it — deliberately tight, because the last move was DOWN: the spreadsheet writer left this edition on the owner's ruling of 1 Sep 2026 ("lose excel is fine as long as they can keep csv"), taking 498.6 KiB raw / 162.4 KiB gzip with it, and leaving that as headroom would have been banking the win as permission. `jspdf` is now the largest chunk and it stays — PDF is a format this edition offers |
