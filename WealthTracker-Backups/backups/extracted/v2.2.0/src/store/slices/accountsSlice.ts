...
const accountsSlice = createSlice({
  ...
  reducers: {
    setAccounts: (state, action: PayloadAction<Account[]>) => {
      state.accounts = serializeForRedux(action.payload) as Account[];
      state.error = null;
    },
    addAccount: (state, action: PayloadAction<Omit<Account, 'id' | 'lastUpdated'>>) => {
      ...
      state.accounts.push(serializeForRedux(newAccount) as Account);
    },
    ...
    recalculateAllBalances: (state, action: PayloadAction<Transaction[]>) => {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateAccountInSupabase.fulfilled, (state, action) => {
        ...
        const index = state.accounts.findIndex(acc => acc.id === action.payload.id);
        if (index !== -1) {
          state.accounts[index] = serializeForRedux(action.payload) as Account;
        }
      })
