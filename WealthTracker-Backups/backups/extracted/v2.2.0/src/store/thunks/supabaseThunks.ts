...
export const createBudgetInSupabase = createAsyncThunk(
  ...
  async (budgetData: Omit<Budget, 'id' | 'createdAt' | 'updatedAt' | 'spent'>, { rejectWithValue }) => {
    try {
      const userId = await getCurrentDatabaseUserId();
      if (!userId) {
        throw new Error('User not authenticated');
...
export const createGoalInSupabase = createAsyncThunk(
  ...
  async (goalData: Omit<Goal, 'id' | 'createdAt' | 'updatedAt' | 'currentAmount'>, { rejectWithValue }) => {
    try {
      const userId = await getCurrentDatabaseUserId();
      if (!userId) {
        throw new Error('User not authenticated');
