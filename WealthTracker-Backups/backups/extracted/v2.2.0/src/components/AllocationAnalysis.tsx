    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <select
            value={groupBy}
            onChange={(e) => setGroupBy(e.target.value as 'assetClass' | 'account' | 'symbol')}
          ...
          <ResponsiveContainer width="100%" height={400}>
            <PieChart>
              <Pie
                ...
                dataKey="value"
                label={(p) => {
                  const { name, percent } = p as unknown as { name: string; percent?: number };
                  return `${name}: ${percent !== undefined ? percent.toFixed(1) : '0.0'}%`;
                }}
              >
