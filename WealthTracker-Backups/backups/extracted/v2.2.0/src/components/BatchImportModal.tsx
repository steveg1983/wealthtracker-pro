...
export default function BatchImportModal({ isOpen, onClose }: BatchImportModalProps): React.JSX.Element {
  const { accounts, transactions, addTransaction, clearAllData } = useApp() as any;
  // Determine if test data is present (fallback if context does not provide)
                          ...
                          <p className="text-sm text-gray-600 dark:text-gray-400">
                            ...
                            {file.accountMatched && (
                              <span className="ml-2 text-primary">
                                → {accounts.find((a: any) => a.id === file.accountMatched)?.name}
                              </span>
                            )}
                          </p>
                        </div>
