...
export default function DataIntelligenceVerification() {
      ...
      if (!transactions || transactions.length === 0) {
        ...
        if (merchants.length === 0) {
          ...
          results.push({
            ...
            details: `Average confidence: ${getAverageConfidence(merchants)}%`
          });
        }
        ...
        if (subscriptions.length === 0) {
          ...
          results.push({
            ...
            details: `Monthly total: $${totalMonthly.toFixed(2)}`
          });
        }
  ...
  const getDateRange = (transactions: unknown[]) => {
  ...
  const getAverageConfidence = (merchants: unknown[]) => {
