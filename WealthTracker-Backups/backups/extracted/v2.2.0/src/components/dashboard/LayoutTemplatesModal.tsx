    ...
    if (template) {
      ...
      onSelect({
        ...
        layouts: {
          ...
          sm: template.layouts.lg.map((item) => ({ ...item, w: Math.min(item.w, 6) })), // Adjust for small
          xs: template.layouts.lg.map((item) => ({ ...item, w: 4, x: 0 })), // Stack for extra small
          xxs: template.layouts.lg.map((item) => ({ ...item, w: 2, x: 0 })) // Stack for mobile
        }
      } as LayoutTemplate & { layouts: Layouts });
    }
  };
