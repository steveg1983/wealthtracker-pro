...
type OfflineEntityData = Transaction | Account | Budget | Goal;
...
interface ConflictData {
  id: string;
  entity: string;
  clientData: OfflineEntityData;
  serverData: OfflineEntityData;
  timestamp: number;
  resolved: boolean;
}

interface AccountWithOriginal extends Account {
...
class OfflineStorageService {
      ...
      for (const operation of operations) {
        try {
          ...
          if (syncError.status === 409) {
            await this.handleConflict(operation, syncError.data as OfflineEntityData);
          } else {
  ...
  private resolveAccountConflict(client: Account, server: Account): ConflictResolution<Account> {
    ...
    const clientWithOriginal = client as AccountWithOriginal;
    const balanceDiff = client.balance - (clientWithOriginal.originalBalance || 0);
   ...
   */
  async resolveConflict(conflictId: string, resolution: ConflictResolution<OfflineEntityData>) {
   ...
   */
  private notifyConflict(conflict: ConflictData) {
...
export const useOfflineOperations = () => {
  ...
  const resolveConflict = React.useCallback(
    (conflictId: string, resolution: ConflictResolution<OfflineEntityData>) => 
