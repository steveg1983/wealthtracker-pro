...
class EnhancedEncryptionService {
  ...
  private async getStoredKeyData(): Promise<StoredKeyData | null> {
    try {
      // Try IndexedDB first
      const data = await this.getFromIndexedDB('encryption_keys', 'master');
      return data as StoredKeyData | null;
    } catch {
      // Fallback to sessionStorage
      const stored = sessionStorage.getItem('wt_master_key');
      return stored ? JSON.parse(stored) as StoredKeyData : null;
    }
  }
   ...
   */
  private async storeInIndexedDB(storeName: string, key: string, value: unknown): Promise<void> {
