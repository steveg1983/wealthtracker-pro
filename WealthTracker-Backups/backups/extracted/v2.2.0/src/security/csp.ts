...
export const viteCSPPlugin = (): Plugin => {
  return {
    name: 'csp-header',
    configureServer(server: ViteDevServer) {
      server.middlewares.use((_req: Request, res: Response & { locals?: Record<string, unknown> }, next: NextFunction) => {
...
export const cspMiddleware = (_req: Request, res: Response & { locals?: Record<string, unknown> }, next: NextFunction): void => {
  const nonce = generateNonce();
  const cspHeader = getCSPHeader(nonce);
  const securityHeaders = getSecurityHeaders();
