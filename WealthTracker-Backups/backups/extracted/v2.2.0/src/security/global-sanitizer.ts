 ...
 */
export const sanitizeObject = <T extends Record<string, unknown>>(obj: T, depth: number = 0): T => {
 ...
 */
export const sanitizeRequestMiddleware = (config: RequestConfig): RequestConfig => {
  if (config.data) {
 ...
 */
export const sanitizeResponseMiddleware = (response: ResponseData): ResponseData => {
  if (response.data) {
