...
// Context type for rate limiting
...
class RateLimiter {
  ...
  createLimiter(name: string, config: RateLimitConfig) {
    ...
    return {
       ...
       */
      check: (context?: RateLimitContext): { allowed: boolean; remaining: number; resetTime: number } => {
...
export const initializeRateLimiter = () => {
  ...
  window.fetch = async function(this: typeof globalThis, ...args: Parameters<typeof fetch>) {
...
export const getRateLimitStatus = () => {
  const status: Record<string, RateLimitStatusItem> = {};
