...
type LoaderResult = unknown | Transaction[] | Category[] | { summary: unknown } | { prices: Array<{ symbol: string; price: number }> };
...
class PredictiveLoadingService {
  ...
  private loadingQueue: Map<string, Promise<LoaderResult>>;
  ...
  private async fetchData(target: string): Promise<LoaderResult> {
  ...
  private async fetchYesterdayTransactions(): Promise<Transaction[]> {
    const yesterday = new Date();
    yesterday.setDate(yesterday.getDate() - 1);
    return this.fetchData(`transactions?date=${yesterday.toISOString()}`);
  }

  private async fetchPendingTransactions(): Promise<Transaction[]> {
  ...
  private async fetchMonthlySummary(): Promise<{ summary: unknown }> {
    return this.fetchData('reports/monthly-summary');
  }

  private async fetchStockPrices(): Promise<{ prices: Array<{ symbol: string; price: number }> }> {
  ...
  private isCacheStale<T>(data: CachedData<T>, ttl?: number): boolean {
