...
function transformGoalFromDb(dbGoal: GoalRow): Goal {
  return {
    ...
    status: (dbGoal.status as 'active' | 'paused' | 'completed' | 'cancelled') || 'active',
...
function transformGoalToDb(goal: Partial<Goal>): GoalUpdate {
  ...
  if (goal.priority !== undefined) dbGoal.priority = goal.priority as 'low' | 'medium' | 'high';
  if (goal.status !== undefined) dbGoal.status = goal.status as 'active' | 'paused' | 'completed' | 'cancelled';
  ...
  if (goal.icon !== undefined) dbGoal.icon = goal.icon || null;
  if (goal.color !== undefined) dbGoal.color = goal.color || null;
  ...
  try {
    ...
    for (const goal of localGoals) {
      try {
        ...
        await createGoal(clerkId, goalData as Omit<Goal, 'id' | 'progress'>);
      } catch (error) {
