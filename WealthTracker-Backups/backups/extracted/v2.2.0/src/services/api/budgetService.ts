...
function transformBudgetFromDb(dbBudget: BudgetRow): Budget {
  return {
    ...
    categoryId: dbBudget.category_id || undefined,
...
function transformBudgetToDb(budget: Partial<Budget>): BudgetUpdate {
  ...
  if (budget.period !== undefined) dbBudget.period = budget.period as 'daily' | 'weekly' | 'monthly' | 'quarterly' | 'yearly';
  if (budget.categoryId !== undefined) dbBudget.category_id = budget.categoryId || null;
  ...
  if (budget.notes !== undefined) dbBudget.notes = budget.notes || null;
  ...
  try {
    ...
    for (const budget of localBudgets) {
      try {
        ...
        await createBudget(clerkId, budgetData);
      } catch (error) {
