...
type CategoryRow = Database['public']['Tables']['categories']['Row'] & {
  // Fields that may not be in the generated types yet
...
function transformCategoryFromDb(dbCategory: CategoryRow): Category {
  return {
    ...
    type: dbCategory.type as 'income' | 'expense' | undefined,
    ...
    accountId: dbCategory.account_id ?? undefined,
...
export async function updateCategory(categoryId: string, updates: Partial<Category>): Promise<Category> {
  try {
    ...
    if (updates.type !== undefined) updateData.type = updates.type as 'income' | 'expense';
