...
import type { Transaction, Account, Category, Investment, Budget, Goal } from '../types';
...
// New interfaces for export data structures
...
class ExportService {
  ...
  async generatePDFReport(exportData: ExportReportData, options: ExportOptions): Promise<void> {
    // Lazy load jsPDF
    if (!jsPDF) {
    ...
    // Add sections
    exportData.sections.forEach((section: ExportSection) => {
      if (yPosition > 250) {
        doc.addPage();
        yPosition = 20;
      }
      ...
      if (section.id === 'budgets' && exportData.data.budgetPerformance) {
        doc.setFontSize(10);
        exportData.data.budgetPerformance.forEach((budget: BudgetPerformance) => {
          if (yPosition > 270) {
            doc.addPage();
            yPosition = 20;
          }
      ...
      if (section.id === 'goals' && exportData.data.goalProgress) {
        doc.setFontSize(10);
        exportData.data.goalProgress.forEach((goal: GoalProgress) => {
          if (yPosition > 270) {
            doc.addPage();
            yPosition = 20;
          }
  ...
  async generateExcelReport(exportData: ExportReportData, options: ExportOptions): Promise<void> {
  ...
  private async createComprehensiveExcelWorkbook(exportData: ExportReportData, options: ExportOptions): Promise<WorkBook> {
    ...
    if (options.includeTransactions && exportData.data.filteredTransactions) {
      ...
      exportData.data.filteredTransactions.forEach((transaction: Transaction) => {
  ...
  private downloadExcel(workbook: WorkBook, filename: string): void {
  ...
  async generateCSVReport(exportData: ExportReportData, options: ExportOptions): Promise<void> {
    // Create CSV content
    ...
    return data.reduce((groups, item) => {
      ...
      switch (groupBy) {
        case 'category':
          ...
          break;
        case 'account':
          key = ('accountId' in item ? item.accountId : 'account' in item ? (item as Record<string, unknown>).account as string : undefined) || 'Unknown';
          break;
        case 'month': {
