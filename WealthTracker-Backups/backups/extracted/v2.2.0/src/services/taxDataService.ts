...
class TaxDataService {
   ...
   */
  private getMarginalRate(income: number, brackets: unknown[]): number {
    for (const bracket of brackets) {
    ...
    try {
      ...
      for (const testCase of testCases) {
        ...
        if (region === 'US') {
          ...
          logger.debug('UK Test Tax', { actual: Number(actualTax.toFixed(2)), expected: expectedTax });
          logger.debug('UK Test NI', { actual: Number(actualNI.toFixed(2)), expected: expectedNI });
