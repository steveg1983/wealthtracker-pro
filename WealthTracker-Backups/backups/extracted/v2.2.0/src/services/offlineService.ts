...
// Type for entity data that can be stored offline
...
class OfflineService {
  ...
  private setupEventListeners(): void {
    ...
    if ('serviceWorker' in navigator && 'SyncManager' in window) {
      navigator.serviceWorker.ready.then((registration) => {
        ...
        if (regWithSync.sync) {
          return regWithSync.sync.register('sync-offline-data');
        }
      }).catch((err) => {
  ...
  private async handleSyncConflict(item: QueueItem): Promise<void> {
    if (!this.db) await this.init();
