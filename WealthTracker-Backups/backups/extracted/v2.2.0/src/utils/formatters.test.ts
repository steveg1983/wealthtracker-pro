...
describe('Formatters', () => {
  ...
  describe('error handling and edge cases', () => {
    ...
    it('handles string input that can be coerced to number', () => {
      expect(() => formatCurrency('123.45' as unknown as number)).not.toThrow();
      expect(() => formatNumber('123.45' as unknown as number)).not.toThrow();
      expect(() => formatCompactNumber('123.45' as unknown as number)).not.toThrow();
    });
