...
const polyfillSafari = () => {
  ...
  if (!Promise.allSettled) {
    ...
    (Promise as any).allSettled = function<T>(promises: Array<Promise<T>>): Promise<Array<Settled<T>>> {
      const mapped = promises.map(p =>
        Promise.resolve(p).then<Settled<T>>(
          (value) => ({ status: 'fulfilled', value }),
          (reason) => ({ status: 'rejected', reason })
        )
      );
      return Promise.all(mapped) as Promise<Array<Settled<T>>>;
    };
  }
