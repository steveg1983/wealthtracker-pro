...
export function lazyImport<T extends ComponentType<any>>(
  ...
  const importWithRetry = async (): Promise<{ default: T }> => {
    try {
      ...
      if (retryCount < retries) {
        retryCount++;
        logger.warn(`Failed to load component, retrying (${retryCount}/${retries})...`, { message: err.message });
        
        // Wait before retrying
        await new Promise(resolve => setTimeout(resolve, retryDelay * retryCount));
        
        // Clear module cache to force re-download
        const maybeChunkError = (err as any);
        if ((maybeChunkError && typeof maybeChunkError === 'object' && 'webpackChunkName' in maybeChunkError) || err.message?.includes('Loading chunk')) {
      ...
      logger.error('Failed to load component after retries:', { message: err.message, stack: err.stack });
  ...
  if (preload && typeof window !== 'undefined') {
    importFunc().catch((error: unknown) => {
      const err: Error = error instanceof Error ? error : new Error(String(error));
      logger.warn('Preload failed for component:', { message: err.message });
    });
  }
...
export function preloadComponent(
  ...
  return importFunc()
    ...
    .catch((error: unknown) => {
      const err: Error = error instanceof Error ? error : new Error(String(error));
      logger.warn('Failed to preload component:', { message: err.message });
    });
}
