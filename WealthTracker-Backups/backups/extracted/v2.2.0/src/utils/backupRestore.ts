...
export function validateBackupData(data: unknown): data is BackupData {
  ...
  for (const account of obj.accounts as unknown[]) {
    if (!account || typeof account !== 'object') return false;
    const acc = account as Record<string, unknown>;
    if (!acc.id || !acc.name || typeof acc.balance !== 'number') return false;
  }
  
  for (const transaction of obj.transactions as unknown[]) {
    if (!transaction || typeof transaction !== 'object') return false;
    const txn = transaction as Record<string, unknown>;
    if (!txn.id || !txn.date || typeof txn.amount !== 'number') return false;
  }
