...
export const useSanitizedForm = <T extends Record<string, FormValue>>(
  initialValues: T,
  ...
  const handleChange = useCallback((fieldName: keyof T) => {
    return (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement> | FormValue) => {
  ...
  const setValue = useCallback((fieldName: keyof T, value: FormValue) => {
    const fieldConfig = config[fieldName as string];
