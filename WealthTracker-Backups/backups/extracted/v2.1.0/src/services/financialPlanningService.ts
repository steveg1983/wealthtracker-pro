...
class FinancialPlanningService {
  ...
  private clearMockData() {
    // Check if we have the mock "Primary Retirement Plan" and remove it
    const savedPlans = localStorage.getItem('financial-retirement-plans');
    if (savedPlans) {
      try {
        const plans = JSON.parse(savedPlans);
        const filteredPlans = plans.filter((plan: any) => 
          plan.name !== 'Primary Retirement Plan' || plan.id !== '1'
        );
        if (filteredPlans.length !== plans.length) {
          localStorage.setItem('financial-retirement-plans', JSON.stringify(filteredPlans));
        }
      } catch (error) {
  ...
  // Removed createSampleData method - no longer creating mock data
