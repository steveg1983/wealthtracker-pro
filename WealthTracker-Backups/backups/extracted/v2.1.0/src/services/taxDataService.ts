import { Region } from '../hooks/useRegionalSettings';
...
class TaxDataService {
   ...
   */
  calculateTax(income: number, region: Region, options?: {
    filingStatus?: 'single' | 'married' | 'separate' | 'head';
    state?: string;
    scottish?: boolean;
    taxYear?: UKTaxYear;
   ...
   */
  private calculateUSTax(income: number, filingStatus: string, state?: string) {
   ...
   */
  private calculateUKTax(income: number, scottish: boolean, taxYear: UKTaxYear = '2024-25') {
   ...
   */
  private getMarginalRate(income: number, brackets: any[]): number {
    for (const bracket of brackets) {
  ...
  private async verifyDataChecksum(region: Region): Promise<boolean> {
    try {
      ...
      // For now, just verify we can calculate a checksum
      // In production, would compare against known good checksums
  ...
  private async verifyCalculationsWork(region: Region): Promise<boolean> {
    try {
      ...
      for (const testCase of testCases) {
        const result = this.calculateTax(testCase.income, region, {
        ...
        if (region === 'US') {
          ...
          if (Math.abs(actualTax - expectedTax) > tolerance) {
            logger.error(
              ...
              `Expected: $${expectedTax}, Got: $${actualTax.toFixed(2)}`
            );
            allPassed = false;
          } else {
          ...
          // Debug logging
          console.log(`UK Test: ${testCase.description}`);
          console.log(`  Income: £${testCase.income}`);
          console.log(`  Actual Tax: £${actualTax.toFixed(2)}, Expected: £${expectedTax}`);
          console.log(`  Actual NI: £${actualNI.toFixed(2)}, Expected: £${expectedNI}`);
          
          if (Math.abs(actualTax - expectedTax) > tolerance) {
            logger.error(
              `UK tax calculation failed: ${testCase.description}. ` +
              `Expected tax: £${expectedTax}, Got: £${actualTax.toFixed(2)}`
            );
            allPassed = false;
          }
          ...
          if (Math.abs(actualNI - expectedNI) > tolerance) {
            logger.error(
              ...
              `Expected NI: £${expectedNI}, Got: £${actualNI.toFixed(2)}`
            );
            allPassed = false;
          }
