-- Fix UUID Migration Script V3 (Clean Version)
...
-- Core tables (only if they're still UUID)
DO $$ 
BEGIN
    -- accounts
    IF EXISTS (
    ...
    -- subscriptions (already TEXT but check anyway)
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
...
-- ========================================
-- STEP 4: Re-add foreign key constraints pointing to clerk_user_id
-- ========================================
