...
export default function QIFImportModal({ isOpen, onClose }: QIFImportModalProps): React.JSX.Element {
  ...
  const parseFile = async (file: File) => {
    ...
    try {
      ...
      const nextParseResult: { transactions: Array<{ date: string; amount: number; payee?: string; memo?: string; category?: string; checkNumber?: string; cleared?: boolean }>; accountType?: string } = {
        transactions: parsed.transactions,
      };

      if (parsed.accountType) {
        nextParseResult.accountType = parsed.accountType;
      }
