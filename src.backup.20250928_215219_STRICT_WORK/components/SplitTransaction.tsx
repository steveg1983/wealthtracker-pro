...
export default function SplitTransaction({ 
  ...
  const addSplit = () => {
    const balancedAmount = remaining > 0 ? remaining : 0;
    const newSplit: SplitItem = {
      id: Date.now().toString(),
      amount: balancedAmount.toFixed(2),
      category: 'Uncategorized',
      description: '',
      percentage: totalAmount > 0 ? (balancedAmount / totalAmount) * 100 : 0
    };
    ...
    setSplits(prev => {
      const newSplits = prev.map(split => {
        ...
        if (field === 'amount') {
          ...
          const normalizedAmount = safeNumeric.toFixed(2);

          const percentage = totalAmount > 0
            ? (safeNumeric / totalAmount) * 100
            : 0;
        ...
        if (field === 'percentage') {
          const parsedPercentage = typeof value === 'number'
            ? value
            : parseFloat((value as string) || '0');
          const safePercentage = Number.isFinite(parsedPercentage) ? parsedPercentage : 0;
          const derivedAmount = ((safePercentage / 100) * totalAmount).toFixed(2);
  ...
  const distributeByPercentage = (percentages: number[]) => {
    ...
    setSplits(prev => prev.map((split, index) => ({
      ...
      amount: (((percentages[index] || 0) / 100) * totalAmount).toFixed(2)
    })));
  };
