...
type BankRegion = 'UK' | 'US' | 'EU' | 'Canada' | 'Australia' | 'Asia' | 'Global';
type BankType = 'traditional' | 'digital' | 'investment' | 'crypto' | 'business' | 'payment';
...
const REGION_LABELS: Record<BankRegion, string> = {
  'UK': 'United Kingdom',
  'US': 'United States',
  'EU': 'Europe',
  'Canada': 'Canada',
...
const TYPE_ICONS: Record<BankType, React.ComponentType<{ size?: number }>> = {
...
export default function BankFormatSelector({ onBankSelected, selectedBank, className }: BankFormatSelectorProps): React.JSX.Element {
  ...
  const [selectedRegion, setSelectedRegion] = useState<'all' | BankRegion>('all');
  const [selectedType, setSelectedType] = useState<'all' | BankType>('all');
  ...
  const groupedBanks = useMemo(() => {
    ...
    filteredBanks.forEach(bank => {
      ...
      const groupName = selectedRegion !== 'all'
        ? TYPE_LABELS[bank.type]
        : REGION_LABELS[bank.region];
        ...
        <div className="flex flex-wrap gap-4">
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              onChange={(e) => setSelectedRegion(e.target.value as 'all' | BankRegion)}
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              value={selectedType}
              onChange={(e) => setSelectedType(e.target.value as 'all' | BankType)}
              className="px-3 py-1 border border-gray-300 dark:border-gray-600 rounded focus:ring-2 focus:ring-gray-500 dark:bg-gray-700 dark:text-white text-sm"
            >
              <option value="all">All Types</option>
              {Object.entries(TYPE_LABELS).map(([key, name]) => (
                <option key={key} value={key}>{name}</option>
              ))}
            </select>
          </div>
                      ...
                      <div className="flex items-start justify-between mb-2">
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                            {bank.region} • {TYPE_LABELS[bank.type]}
                          </p>
                        </div>
                        {isSelected && (
