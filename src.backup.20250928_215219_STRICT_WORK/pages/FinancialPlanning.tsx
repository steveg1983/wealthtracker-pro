...
export default function FinancialPlanning() {
  ...
  const evaluateGoalProgress = (goal: Goal) => {
    ...
    const contribution = goal.contributionAmount ?? 0;
    const progress = target > 0 ? Math.min(100, (current / target) * 100) : 0;
    const now = new Date();
    const dueDate = goal.targetDate ? new Date(goal.targetDate) : null;

    if (!dueDate) {
      return { onTrack: contribution > 0 || remaining === 0, progress };
    }
    ...
    const monthsUntilDue = Math.max(0, Math.round((dueDate.getTime() - now.getTime()) / millisecondsPerMonth));
    const monthsNeeded = Math.ceil(remaining / contribution);
    const onTrack = monthsNeeded <= monthsUntilDue;
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-600 dark:text-gray-400">
                              ...
                              {summary.averageApr != null && (
                                <span className="ml-2">• Avg APR {(summary.averageApr * 100).toFixed(1)}%</span>
                              )}
                              {strategy && (
