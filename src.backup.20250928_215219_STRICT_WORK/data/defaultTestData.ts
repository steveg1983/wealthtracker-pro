...
const convertInvestmentsToHoldings = (investments: Investment[]): Holding[] => {
  return investments.map(inv => {
    const purchaseCost = inv.quantity * inv.purchasePrice;
    const holding: Holding = {
      ...
      gain: inv.currentValue - purchaseCost,
      gainPercent: purchaseCost === 0 ? 0 : ((inv.currentValue - purchaseCost) / purchaseCost) * 100,
      currency: 'GBP',
    };

    if (typeof inv.currentPrice === 'number') {
      holding.currentPrice = inv.currentPrice;
    }

    if (inv.lastUpdated) {
      holding.lastUpdated = inv.lastUpdated;
    }
