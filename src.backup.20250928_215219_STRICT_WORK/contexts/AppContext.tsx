    ...
    deleteTag: (id): void => {
      ...
      if (tag) {
        ...
        setDecimalTransactions((prev): DecimalTransaction[] => prev.map((transaction): DecimalTransaction => {
          ...
          const filteredTags = transaction.tags.filter((t): boolean => t !== tag.name);
          return {
            ...transaction,
            tags: filteredTags
          };
    ...
    processRecurringTransactions: (): void => {
      ...
      setDecimalRecurringTransactions((prev): DecimalRecurringTransaction[] => prev.map((rt): DecimalRecurringTransaction => {
        ...
        while (currentDate <= today) {
          const newTransactionBase: Transaction = {
            id: crypto.randomUUID(),
          ...
          const transactionWithTags = rt.tags ? { ...newTransactionBase, tags: rt.tags } : newTransactionBase;
          transactions.push(toDecimalTransaction(transactionWithTags));
