...
const mapSupabaseAccountToDomain = (row: SupabaseAccountRow): Account => {
  ...
  const account: Record<string, unknown> = {
    ...
    openingBalance: typeof metadata.openingBalance === 'number' ? metadata.openingBalance : row.initial_balance,
    updatedAt
  };

  if (row.institution) account.institution = row.institution;
  if (metadata.openingBalanceDate) account.openingBalanceDate = new Date(String(metadata.openingBalanceDate));
  if (typeof metadata.notes === 'string') account.notes = metadata.notes;
  if (typeof metadata.plaidConnectionId === 'string') account.plaidConnectionId = metadata.plaidConnectionId;
  if (typeof metadata.plaidAccountId === 'string') account.plaidAccountId = metadata.plaidAccountId;
  if (typeof metadata.mask === 'string') account.mask = metadata.mask;
  if (typeof metadata.available === 'number') account.available = metadata.available;
...
const normaliseLocalAccount = (raw: Account | (Account & Record<string, unknown>)): Account => {
  ...
  return pruneUndefined(result) as unknown as Account;
};
