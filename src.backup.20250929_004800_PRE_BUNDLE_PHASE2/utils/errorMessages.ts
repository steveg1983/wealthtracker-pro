...
export function getUserFriendlyError(error: unknown): string {
  ...
  if (error instanceof Error) {
    ...
    if (error.name === 'NetworkError' || message.includes('fetch')) {
      const networkMessage = errorMessageMap['NetworkError'] ?? 'Connection problem. Please check your internet and try again.';
  ...
  if (typeof error === 'object' && error !== null && 'field' in error) {
    ...
    const friendly = errorMessageMap[validationError.message];
    return friendly ?? validationError.message;
  }
