...
class USRetirementService {
  ...
  calculateSocialSecurity(
    ...
    return {
      ...
      credits,
      eligible: true,
      ...(breakEvenAge !== undefined && { breakEvenAge })
    };
  }
  ...
  calculateMedicare(
    ...
    return {
      ...
      totalMonthlyPremium,
      irmaaApplies,
      ...(irmaaApplies && adjustedPremium !== undefined && { adjustedPremium })
    };
  }
  ...
  calculateFullRetirementProjection(params: {
    ...
    let estimatedTax = 0;
    if ('federal' in taxResult) {
      estimatedTax = taxResult.total;
    } else if ('incomeTax' in taxResult) {
      estimatedTax = taxResult.total;
    }

    const afterTaxIncome = totalAnnualIncome - estimatedTax;
    ...
    return {
      ...
      ...(rothIRA && { rothIRA }),
      ...(rmd && { requiredMinimumDistribution: rmd })
    };
  }
  ...
  private estimateMarginalTaxRate(income: number): number {
    ...
    return lastBracket ? lastBracket.rate : 0;
  }
