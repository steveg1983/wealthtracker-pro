...
export class AuthService {
  private static coerceDate(value: string | number | Date | null | undefined): Date {
  ...
  static mapClerkUser(clerkUser: UserResource): AuthUser {
    const passkeyInfo = clerkUser as unknown as {
    ...
    const authUser: AuthUser = {
      ...
      createdAt: AuthService.coerceDate(clerkUser.createdAt),
      lastSignIn: AuthService.coerceDate(clerkUser.lastSignInAt ?? clerkUser.createdAt),
      emailVerified: clerkUser.primaryEmailAddress?.verification?.status === 'verified',
      hasPasskey:
        Boolean(passkeyInfo.passkeyEnabled) ||
        (Array.isArray(passkeyInfo.passkeys) && passkeyInfo.passkeys.length > 0),
      hasMFA: clerkUser.totpEnabled || clerkUser.backupCodeEnabled || clerkUser.twoFactorEnabled
    };

    if (clerkUser.fullName) {
      authUser.name = clerkUser.fullName;
    }
    if (clerkUser.imageUrl) {
      authUser.imageUrl = clerkUser.imageUrl;
    }
