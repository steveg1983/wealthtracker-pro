...
class RealtimeService {
  ...
  private hasLoggedStubWarning = false;

  private async getClient(): Promise<SupabaseClient<any> | null> {
    const client = await ensureSupabaseClient();
    if ((client as { __isStub?: boolean }).__isStub) {
      if (!this.hasLoggedStubWarning) {
        logger.warn('[RealtimeService] Supabase client unavailable (stub mode); skipping realtime wiring.');
        this.hasLoggedStubWarning = true;
      }
      return null;
    }

    this.hasLoggedStubWarning = false;
    return client;
  }
  ...
  private scheduleReconnect(): void {
    ...
    this.reconnectTimeout = setTimeout(() => {
      ...
      void this.reconnectAllSubscriptions();
    }, delay);
  }
