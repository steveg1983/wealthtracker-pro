...
const formatCSSValue = (value: unknown): string | null => {
...
export function applyTheme(theme: Theme): void {
  ...
  const setVariable = (name: string, rawValue: unknown) => {
    const formatted = formatCSSValue(rawValue);
    if (formatted === null) {
      return;
    }
    root.style.setProperty(name, formatted);
  };
  
  // Apply color tokens
  Object.entries(theme.colors).forEach(([category, values]) => {
    Object.entries(values).forEach(([key, value]) => {
      setVariable(`--color-${category}-${key}`, value);
    });
  });
  
  // Apply typography tokens
  Object.entries(typography.fontSize).forEach(([key, value]) => {
    setVariable(`--font-size-${key}`, value);
  });
  
  Object.entries(typography.fontWeight).forEach(([key, value]) => {
    setVariable(`--font-weight-${key}`, value);
  });
  
  Object.entries(typography.lineHeight).forEach(([key, value]) => {
    setVariable(`--line-height-${key}`, value);
  });
  
  // Apply spacing tokens
  Object.entries(spacing).forEach(([key, value]) => {
    const propertyKey = key.replace('.', '-');
    setVariable(`--spacing-${propertyKey}`, value);
  });
  
  // Apply border radius tokens
  Object.entries(borderRadius).forEach(([key, value]) => {
    setVariable(`--radius-${key}`, value);
  });
  
  // Apply shadow tokens
  Object.entries(shadows).forEach(([key, value]) => {
    setVariable(`--shadow-${key}`, value);
  });
  
  // Apply animation tokens
  Object.entries(animations.duration).forEach(([key, value]) => {
    setVariable(`--duration-${key}`, value);
  });
  
  Object.entries(animations.easing).forEach(([key, value]) => {
    setVariable(`--easing-${key}`, value);
  });
...
export function generateThemeCSS(theme: Theme): string {
  ...
  Object.entries(theme.colors).forEach(([category, values]) => {
    ...
    Object.entries(values).forEach(([key, value]) => {
      ...
      if (formatted !== null) {
        lines.push(`  --color-${category}-${key}: ${formatted};`);
      }
    });
  });
  ...
  Object.entries(typography.fontSize).forEach(([key, value]) => {
    const formatted = formatCSSValue(value);
    if (formatted !== null) {
      lines.push(`  --font-size-${key}: ${formatted};`);
    }
  });
  ...
  Object.entries(typography.fontWeight).forEach(([key, value]) => {
    const formatted = formatCSSValue(value);
    if (formatted !== null) {
      lines.push(`  --font-weight-${key}: ${formatted};`);
    }
  });
  ...
  Object.entries(typography.lineHeight).forEach(([key, value]) => {
    const formatted = formatCSSValue(value);
    if (formatted !== null) {
      lines.push(`  --line-height-${key}: ${formatted};`);
    }
  });
  ...
  Object.entries(spacing).forEach(([key, value]) => {
    ...
    const formatted = formatCSSValue(value);
    if (formatted !== null) {
      lines.push(`  --spacing-${propertyKey}: ${formatted};`);
    }
  });
  ...
  Object.entries(shadows).forEach(([key, value]) => {
    const formatted = formatCSSValue(value);
    if (formatted !== null) {
      lines.push(`  --shadow-${key}: ${formatted};`);
    }
  });
  ...
  Object.entries(animations.duration).forEach(([key, value]) => {
    const formatted = formatCSSValue(value);
    if (formatted !== null) {
      lines.push(`  --duration-${key}: ${formatted};`);
    }
  });
  ...
  Object.entries(animations.easing).forEach(([key, value]) => {
    const formatted = formatCSSValue(value);
    if (formatted !== null) {
      lines.push(`  --easing-${key}: ${formatted};`);
    }
  });
