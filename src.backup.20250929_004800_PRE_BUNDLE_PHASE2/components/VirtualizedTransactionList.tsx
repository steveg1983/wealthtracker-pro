import React, { memo, useCallback, useRef, useMemo, type MutableRefObject } from 'react';
...
type TransactionRowProps = ListChildComponentProps<TransactionRowData>;
...
const TransactionRow = memo(({ index, style, data, isScrolling: _isScrolling }: TransactionRowProps): React.JSX.Element | null => {
  const {
    transactions,
    formatCurrency,
    onTransactionClick,
  ...
  const isSelected = transaction?.id ? selectedTransactions?.has(transaction.id) || false : false;
  ...
  return (
    <div
      ...
      aria-label={`Transaction: ${description}, ${formattedAmount} on ${new Date(transaction.date).toLocaleDateString()}`}
    >
      {showBulkActions && (
        <input
          type="checkbox"
  ...
  const prevId = prevTransaction?.id;
  const nextId = nextTransaction?.id;

  const prevSelected = prevId ? prevProps.data.selectedTransactions?.has(prevId) ?? false : false;
  const nextSelected = nextId ? nextProps.data.selectedTransactions?.has(nextId) ?? false : false;
...
export const VirtualizedTransactionList = memo(function VirtualizedTransactionList({
  ...
  const listRef = useRef<FixedSizeList<TransactionRowData> | null>(null);

  // Memoize item data to prevent re-renders
  const itemData = useMemo<TransactionRowData>(() => {
    const base: TransactionRowData = {
      transactions,
      formatCurrency
    };

    if (onTransactionClick) {
      base.onTransactionClick = onTransactionClick;
    }
    if (onTransactionEdit) {
      base.onTransactionEdit = onTransactionEdit;
    }
    if (onTransactionDelete) {
      base.onTransactionDelete = onTransactionDelete;
    }
    if (selectedTransactions) {
      base.selectedTransactions = selectedTransactions;
    }
    if (onSelectionChange) {
      base.onSelectionChange = onSelectionChange;
    }
    if (showBulkActions) {
      base.showBulkActions = true;
    }
          ...
          >
            {({ onItemsRendered, ref }) => (
              <FixedSizeListComponent
                ref={(list) => {
                  const listInstance = list as FixedSizeList<TransactionRowData> | null;

                  if (typeof ref === 'function') {
                    ref(listInstance);
                  } else if (ref) {
                    (ref as MutableRefObject<FixedSizeList<TransactionRowData> | null>).current = listInstance;
                  }
