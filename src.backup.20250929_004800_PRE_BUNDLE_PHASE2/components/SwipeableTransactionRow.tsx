...
export const SwipeableTransactionRow = memo(function SwipeableTransactionRow({
  ...
  const handleSwipeLeft = useCallback(async () => {
    await triggerHaptic(HAPTIC_PATTERN.SELECTION);
    setIsRevealed('right');
    setOffset(-100);
  }, [triggerHaptic]);
  ...
  const handleTap = useCallback(async () => {
    if (isRevealed) {
      ...
      await triggerHaptic(HAPTIC_PATTERN.LIGHT);
      onView(transaction);
    }
  }, [isRevealed, onView, transaction, triggerHaptic]);

  const handleLongPress = useCallback(async () => {
    await triggerHaptic(HAPTIC_PATTERN.MEDIUM);
    onEdit(transaction);
    ...
    <div className="relative overflow-hidden">
      ...
      <div className="absolute inset-0 flex items-center">
        <div className="flex items-center gap-2 px-4">
          {onReconcile && !transaction.cleared && (
            <button
              onClick={async () => {
                await triggerHaptic(HAPTIC_PATTERN.SUCCESS);
      ...
      <div className="absolute inset-0 flex items-center justify-end">
        <div className="flex items-center gap-2 px-4">
          ...
          <button
            onClick={async () => {
              await triggerHaptic(HAPTIC_PATTERN.WARNING);
              if (confirm('Delete this transaction?')) {
                await triggerHaptic(HAPTIC_PATTERN.ERROR);
