...
export function EmptyState({ 
  ...
  const configs: Record<string, EmptyStateConfig> = {
    ...
    budgets: {
      icon: <PieChartIcon size={48} className="text-primary" />,
      ...
      secondaryActions: [
        { label: "Budget Templates", icon: <StarIcon size={16} /> },
    ...
    goals: {
      ...
      secondaryActions: [
        { label: "Goal Ideas", icon: <StarIcon size={16} /> }
      ],
      illustration: 'target'
    },
    investments: {
    ...
    categories: {
      ...
      secondaryActions: [
        { label: "Suggested Categories", icon: <StarIcon size={16} /> }
      ],
      illustration: 'chart'
    },
    reports: {
  ...
  const configCandidate = configs[type];
  const config = configCandidate ?? configs.default;
  if (!config) {
    throw new Error(`Missing empty state configuration for type: ${type}`);
  }
    ...
    <div className={`flex flex-col items-center justify-center py-12 px-6 text-center ${className}`}>
      ...
      <div className="relative w-48 h-48 mb-8">
        <div className="absolute inset-0 text-primary">
          {illustrations[config.illustration ?? 'rocket'] ?? illustrations.rocket}
        </div>
...
function getProTip(type: EmptyStateProps['type']): string {
  const tips: Record<EmptyStateProps['type'], string> = {
