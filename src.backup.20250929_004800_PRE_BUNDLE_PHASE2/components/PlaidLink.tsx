...
import type { PlaidPublicToken } from '../services/plaidService';
...
export default function PlaidLink({ onSuccess, onError }: PlaidLinkProps) {
  ...
  const handleConnectBank = useCallback(async () => {
    ...
    try {
      ...
      const connection = await plaidService.addConnection(simulatedPublicToken, clerkId);
      
      // Sync accounts immediately
      const accounts = clerkId
        ? await plaidService.syncAccountsFromBackend(clerkId, connection.id)
        : await plaidService.syncAccounts(connection.id);
  ...
  }, [addAccount, onSuccess, onError, user?.id]);
  ...
  const handleSync = useCallback(async (connectionId: string) => {
    ...
    try {
      ...
      const accounts = user?.id
        ? await plaidService.syncAccountsFromBackend(user.id, connectionId)
        : await plaidService.syncAccounts(connectionId);
      accounts.forEach(account => {
        addAccount(account);
      });
      ...
      const transactions = user?.id
        ? await plaidService.syncTransactionsFromBackend(user.id, connectionId, startDate)
        : await plaidService.syncTransactions(connectionId, startDate);
  ...
  }, [addAccount, addTransaction, user?.id]);
