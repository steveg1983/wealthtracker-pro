...
import { CheckCircleIcon, XIcon, TagIcon, FolderIcon, TrashIcon, DownloadIcon, XCircleIcon, GripVerticalIcon } from './icons';
        ...
        <div className="flex items-center gap-2">
          {primaryOps.map(operation => (
            <Button
              ...
              {...(operation.id === 'categorize' ? { leftIcon: FolderIcon } :
                operation.id === 'tag' ? { leftIcon: TagIcon } :
                operation.id === 'clear' ? { leftIcon: CheckCircleIcon } :
                operation.id === 'delete' ? { leftIcon: TrashIcon } : {})}
            >
              {operation.label}
            </Button>
          ))}
