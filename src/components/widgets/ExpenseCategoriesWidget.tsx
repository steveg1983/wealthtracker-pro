...
export default function ExpenseCategoriesWidget({ isCompact = false }: ExpenseCategoriesWidgetProps): React.JSX.Element {
  ...
  const CustomTooltip: React.FC<TooltipProps<number, string>> = ({ active, payload }) => {
    if (active && payload && payload.length) {
      ...
      const percentage = typeof dataPoint.payload?.percentage === 'number'
        ? dataPoint.payload.percentage
        : 0;
      return (
        <div className="bg-white dark:bg-gray-800 p-2 rounded shadow-lg border border-gray-200 dark:border-gray-700">
          <p className="font-medium">{dataPoint.name}</p>
          <p className="text-sm">{formatCurrency(Number(dataPoint.value) || 0)}</p>
      ...
      <div className="flex gap-4">
        ...
        <div className="w-32 h-32 flex-shrink-0">
          <DynamicPieChart
            ...
            innerRadius={25}
            outerRadius={45}
            colors={categoryData.categories.map(cat => cat.color)}
            height={128}
        ...
        <div className="flex-1 space-y-1 overflow-y-auto max-h-32">
          {categoryData.categories.slice(0, isCompact ? 4 : 6).map((category) => (
