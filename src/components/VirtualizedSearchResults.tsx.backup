...
export function VirtualizedSearchResults({
  ...
  const processedResults = useMemo(() => {
    ...
    Object.entries(grouped).forEach(([type, items]) => {
      flattened.push({ isHeader: true, type, count: items.length } as SearchResult | { isHeader: true; type: string; count: number });
  ...
  const renderResult = useCallback((item: SearchResult | { isHeader: true; type: string; count: number }, index: number, isScrolling?: boolean) => {
