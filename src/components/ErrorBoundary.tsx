...
class ErrorBoundaryClass extends Component<Props & { resetKey?: string }, State> {
  ...
  componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    this.logger.error('Error caught by boundary', { error, context: 'ErrorBoundary' });
            ...
            <div className="flex justify-center mb-6">
              {staleChunk ? (
                <div className="w-20 h-20 bg-gray-100 dark:bg-gray-700 rounded-full flex items-center justify-center">
                  {offline
                    ? <WifiOffIcon size={40} className="text-gray-600 dark:text-gray-400" />
                    : <DownloadIcon size={40} className="text-gray-600 dark:text-gray-400" />}
                </div>
              ) : (
