...
export class AnomalyDetectionService {
    ...
    categoryGroups.forEach((categoryTransactions, category) => {
      ...
      categoryTransactions.forEach(transaction => {
        ...
        if (zScore > threshold) {
          ...
          const zScoreDisplay = formatDecimal(new Decimal(zScore), 1);
  ...
  private detectFrequencySpikes(
    ...
    merchantGroups.forEach((merchantTransactions, merchant) => {
      ...
      for (let i = 1; i < sortedTrans.length - 1; i++) {
        const days = differenceInDays(new Date(sortedTrans[i].date), new Date(sortedTrans[i-1].date));
        intervals.push(days);
      }
      ...
      const lastInterval = differenceInDays(
        new Date(sortedTrans[sortedTrans.length - 1].date),
        new Date(sortedTrans[sortedTrans.length - 2].date)
      );
  ...
  private detectDuplicateCharges(transactions: Transaction[]): Anomaly[] {
    ...
    transactions
      .filter(t => t.type === 'expense')
      .forEach(t => {
        const amountKey = formatDecimal(toDecimal(Math.abs(t.amount)), 2);
      ...
      for (let i = 0; i < group.length - 1; i++) {
        for (let j = i + 1; j < group.length; j++) {
          const daysDiff = Math.abs(
            differenceInDays(new Date(group[i].date), new Date(group[j].date))
          );
          
          if (daysDiff <= 3) {
            anomalies.push({
              id: `duplicate-${group[i].id}-${group[j].id}`,
