...
export class TimeSeriesAnalysis {
  ...
  static exponentialSmoothing(
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (i < data.length) {
        ...
        level = toDecimal(alpha).times(data[i])
      ...
      const error = i > 0 ? data[i].minus(forecasts[i - 1].value).abs() : toDecimal(0);
  ...
  static seasonalDecomposition(
    ...
    for (let i = 0; i < values.length; i++) {
      if (i < trend.length) {
        detrended.push(values[i].minus(trend[i]));
      } else {
    ...
    for (let i = 0; i < values.length; i++) {
      if (i < trend.length) {
        residual.push(values[i].minus(trend[i]).minus(seasonal[i]));
      } else {
  ...
  static linearRegression(data: DecimalInstance[]): {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const residual = data[i].minus(predicted);
      ssRes = ssRes.plus(residual.pow(2));
      
      const totalDiff = data[i].minus(meanY);
  ...
  static forecast(
    ...
    if (hasSeasonality) {
      ...
      const trend = slope.times(values.length);
      ...
      for (let i = 1; i <= periodsAhead; i++) {
        ...
        const standardError = this.calculateStandardError(values, forecast);
  ...
  static detectAnomalies(
    ...
    const median = sorted.length % 2 === 0
      ? sorted[sorted.length / 2 - 1].plus(sorted[sorted.length / 2]).dividedBy(2)
    ...
    const mad = sortedDeviations.length % 2 === 0
      ? sortedDeviations[sortedDeviations.length / 2 - 1]
          .plus(sortedDeviations[sortedDeviations.length / 2])
          .dividedBy(2)
