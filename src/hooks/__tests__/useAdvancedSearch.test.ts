 ...
 * Tests for the useAdvancedSearch hook
 */
...
describe('useAdvancedSearch', () => {
  ...
  describe('initialization', () => {
    ...
    it('loads data on mount when autoLoad is true', async () => {
      const { result } = renderHook(() => useAdvancedSearch({ autoLoad: true }), {
        wrapper: AllProviders
      });
  ...
  describe('actions', () => {
    ...
    it('handles errors gracefully', async () => {
      ...
      const { result } = renderHook(() => useAdvancedSearch({ fetcher: mockFetch }), {
        wrapper: AllProviders
      });
    ...
    it('cancels pending requests on unmount', async () => {
      ...
      const { result, unmount } = renderHook(() => useAdvancedSearch(), {
        wrapper: AllProviders
      });
  ...
  describe('memoization', () => {
    it('memoizes expensive calculations', () => {
      ...
      const { result, rerender } = renderHook(
        ({ value }) => useAdvancedSearch({ value, calculate: expensiveCalculation }),
        {
          wrapper: AllProviders,
