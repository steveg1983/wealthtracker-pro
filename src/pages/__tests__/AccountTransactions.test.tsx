...
describe('Account register — open, closed, and gone', () => {
  ...
  it('keeps the ?txn deep link across the re-open', async () => {
    ...
    // The deep-linked row arrives selected, and already an editor: its own
    // Description cell is a box holding the payee, and the strip of actions is
    // on the line beneath it.
    await waitFor(() => {
      expect(document.querySelector('[data-quick-edit="actions"]')).not.toBeNull();
    });
    expect(screen.getByLabelText('Transaction description')).toHaveValue('Synthetic closed row');
  });
