...
const accountsSlice = createSlice({
  ...
  reducers: {
    setAccounts: (state, action: PayloadAction<Account[]>) => {
      state.accounts = serializeForRedux<Account[]>(action.payload);
      state.error = null;
    },
    addAccount: (state, action: PayloadAction<Omit<Account, 'id' | 'lastUpdated'>>) => {
      ...
      state.accounts.push(serializeForRedux<Account>(newAccount));
    },
    updateAccount: (state, action: PayloadAction<{ id: string; updates: Partial<Account> }>) => {
      ...
      const timestamp = now();
      const lastUpdated = action.payload.updates.lastUpdated ?? existing.lastUpdated ?? timestamp;
      ...
      state.accounts[index] = serializeForRedux<Account>(updatedAccount);
    },
    ...
    recalculateAllBalances: (_state, _action: PayloadAction<Transaction[]>) => {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateAccountInSupabase.fulfilled, (state, action) => {
        ...
        const index = state.accounts.findIndex(acc => acc.id === action.payload.id);
        if (index !== -1) {
          state.accounts[index] = serializeForRedux<Account>(action.payload);
        }
      })
