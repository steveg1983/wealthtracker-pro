...
const generateDemoTransactions = (): Transaction[] => {
  ...
  for (let i = 0; i < 50; i++) {
    ...
    const expenseAmount = toDecimal(Math.random())
      .times(toDecimal(200).minus(10))
      .plus(10)
      .toDecimalPlaces(2, Decimal.ROUND_HALF_UP)
      .toNumber();
    const incomeAmount = toDecimal(Math.random())
      .times(toDecimal(4000).minus(1000))
      .plus(1000)
      .toDecimalPlaces(2, Decimal.ROUND_HALF_UP)
      .toNumber();
    ...
    transactions.push({
      ...
      amount: isExpense ? -expenseAmount : incomeAmount,
...
const generateDemoGoals = (): Goal[] => {
  const buildGoal = (
    ...
    const targetDecimal = toDecimal(targetAmount);
    const progressDecimal = targetDecimal.greaterThan(0)
      ? toDecimal(currentAmount).dividedBy(targetDecimal).times(100)
      : toDecimal(0);
    const progress = Decimal.min(progressDecimal, toDecimal(100)).toDecimalPlaces(0, Decimal.ROUND_HALF_UP).toNumber();
