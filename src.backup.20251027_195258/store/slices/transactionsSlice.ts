...
const transactionsSlice = createSlice({
  ...
  reducers: {
    setTransactions: (state, action: PayloadAction<Transaction[]>) => {
      state.transactions = serializeForRedux<Transaction[]>(action.payload);
      state.error = null;
    },
    addTransaction: (state, action: PayloadAction<Omit<Transaction, 'id'>>) => {
      ...
      state.transactions.push(serializeForRedux<Transaction>(newTransaction));
    },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateTransactionInSupabase.fulfilled, (state, action) => {
        ...
        if (index !== -1) {
          state.transactions[index] = serializeForRedux<Transaction>(action.payload);
        }
      })
