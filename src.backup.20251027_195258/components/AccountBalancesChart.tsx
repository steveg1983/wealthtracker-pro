...
const AccountBalancesChart = React.memo(function AccountBalancesChart() {
  ...
  const formatCurrencyShort = (rawValue: number) => {
    const decimalValue = toDecimal(rawValue);
    const sign = decimalValue.isNegative() ? '-' : '';
    const formattedMagnitude = formatChartNumber(decimalValue.abs().toNumber());
    return `${sign}${getCurrencySymbol(displayCurrency)}${formattedMagnitude}`;
  };
