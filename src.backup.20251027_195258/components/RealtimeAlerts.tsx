...
import { formatSignedPercentageValue } from '@wealthtracker/utils';
...
export default function RealtimeAlerts({
  ...
  const { recentAnomalies, portfolioChange24h, connectionState } = syncContext;
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const alertIds = recentAnomalies.map(a => a.id);
  ...
  const visibleAnomalies = showAnomalies 
    ? recentAnomalies.filter(a => !dismissedAlerts.has(a.id))
    : [];
  ...
  if (!showPortfolioChange && !showAnomalies) {
    return null;
  }
  ...
  const getAnomalyIcon = (severity: SpendingAnomaly['severity']): React.ReactNode => {
    if (severity === 'high') {
        ...
        `}>
          ...
          <div className="flex-1">
            ...
            <div className={`text-lg font-bold ${
              ...
              {formatSignedPercentageValue(portfolioChange24h ?? 0, 2)}
            </div>
          </div>
        </div>
      )}
