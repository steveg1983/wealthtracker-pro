  ...
  const createStyledWorkbook = () => {
    ...
    if (options.budgets) {
      const budgetData = budgets.map(budget => {
        const spentDecimal = transactions
          .filter(t => {
            const tDate = t.date instanceof Date ? t.date : new Date(t.date);
        ...
        const remaining = budgetAmount.minus(spentDecimal);
        const percentUsed = budgetAmount.gt(0)
          ? spentDecimal.dividedBy(budgetAmount).times(100)
          : toDecimal(0);
        const percentUsedNumber = percentUsed.toNumber();
        
        return {
          Category: budget.categoryId,
          'Budget Amount': budgetAmount.toDecimalPlaces(2).toNumber(),
          Spent: spentDecimal.toDecimalPlaces(2).toNumber(),
          Remaining: remaining.toDecimalPlaces(2).toNumber(),
          '% Used': formatPercentageValue(percentUsed, 1),
          Status: remaining.isNegative() ? 'Over Budget' : percentUsedNumber > 80 ? 'Warning' : 'On Track'
        };
      });
