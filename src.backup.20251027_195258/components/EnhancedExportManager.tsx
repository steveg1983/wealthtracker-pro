...
const toAmountDecimal = (value: number | string | null | undefined) => {
...
export default function EnhancedExportManager(): React.JSX.Element {
  ...
  const generatePDF = async () => {
    ...
    const totals = filteredTransactions.reduce(
      (acc, transaction) => {
        const amount = toAmountDecimal(transaction.amount);
      ...
      { income: toDecimal(0), expenses: toDecimal(0) }
    );
    const income = totals.income;
    const expenses = totals.expenses;
    const expensesAbs = expenses.abs();
    const net = income.plus(expenses);
    
    pdf.setFontSize(10);
    pdf.text(`Total Income: ${formatCurrency(income, defaultCurrency)}`, 20, 55);
    pdf.text(`Total Expenses: ${formatCurrency(expensesAbs, defaultCurrency)}`, 20, 62);
    pdf.text(`Net: ${formatCurrency(net, defaultCurrency)}`, 20, 69);
    ...
    if (options.reportType === 'transactions' || options.reportType === 'custom') {
      const tableData = filteredTransactions.map(t => {
        const amount = toAmountDecimal(t.amount);
        ...
        return [
          ...
          formatCurrency(absoluteAmount, defaultCurrency),
          amount.greaterThanOrEqualTo(0) ? 'Income' : 'Expense'
        ];
      });
    ...
    if (options.reportType === 'budget' && budgets.length > 0) {
      ...
      const budgetData = budgets.map(b => {
        const budgetAmount = toAmountDecimal(b.amount);
        ...
        return [
          b.categoryId,
          formatCurrency(budgetAmount, defaultCurrency),
          formatCurrency(spent, defaultCurrency),
          formatCurrency(remaining, defaultCurrency),
          formatPercentageValue(percentage, 1)
        ];
      });
  ...
  const generateExcel = async () => {
    ...
    const totals = filteredTransactions.reduce(
      (acc, transaction) => {
        ...
        if (amount.greaterThan(0)) {
          ...
          acc.expenses = acc.expenses.plus(amount.abs());
        }
        acc.net = acc.net.plus(amount);
        return acc;
      },
      {
        income: toDecimal(0),
    ...
    const summaryData = [
      ...
      [''],
      ['Summary Statistics'],
      ['Total Income:', totals.income.toDecimalPlaces(2).toNumber()],
      ['Total Expenses:', totals.expenses.toDecimalPlaces(2).toNumber()],
      ['Net Amount:', totals.net.toDecimalPlaces(2).toNumber()],
      ...
      const budgetData = [
        ...
        ...budgets.map(b => {
          ...
          const percentage = budgetAmount.isZero()
            ? toDecimal(0)
            : spent.dividedBy(budgetAmount).times(100);
          const percentageValue = percentage.toDecimalPlaces(1).toNumber();

          return [
            b.categoryId,
            budgetAmount.toDecimalPlaces(2).toNumber(),
            spent.toDecimalPlaces(2).toNumber(),
            remaining.toDecimalPlaces(2).toNumber(),
            percentageValue,
            percentage.greaterThan(100)
              ? 'Over Budget'
              : percentage.greaterThan(80)
                ? 'Warning'
                : 'On Track'
          ];
        })
    ...
    filteredTransactions.forEach(t => {
      const current = categoryGroups.get(t.category) ?? toDecimal(0);
      categoryGroups.set(t.category, current.plus(toAmountDecimal(t.amount)));
    });
    ...
    const categoryData = [
      ...
      ...Array.from(categoryGroups.entries()).map(([category, total]) => {
        ...
        const average = count === 0 ? toDecimal(0) : total.dividedBy(count);
        return [
          category,
          total.toDecimalPlaces(2).toNumber(),
          count,
          average.toDecimalPlaces(2).toNumber()
        ];
      })
    ];
