...
export default function SplitTransactionModal({ isOpen, onClose, transaction }: SplitTransactionModalProps): React.JSX.Element | null {
  ...
  useEffect(() => {
    if (transaction) {
      const totalSplitDecimal = splits.reduce(
        (sum, split) => sum.plus(toDecimal(split.amount)),
        toDecimal(0)
      );
      const remainingDecimal = toDecimal(transaction.amount).minus(totalSplitDecimal);
      setRemainingAmount(toStorageNumber(remainingDecimal));
    }
  }, [splits, transaction]);
  ...
  const handleSplitChange = (index: number, field: keyof SplitItem, value: string | number): void => {
    setSplits(prev => prev.map((split, i) => {
      ...
      if (field === 'amount') {
        ...
        const safeValue = toStorageNumber(decimalValue);
        return {
          ...split,
          amount: safeValue
        };
  ...
  const handleSave = (): void => {
    ...
    if (splits.length === 1) {
      ...
      const updateData = {
        ...transaction,
        ...(firstSplit?.category ? { category: firstSplit.category } : {}),
        ...(firstSplit?.description ? { description: firstSplit.description } : {})
      };
      updateTransaction(transaction.id, updateData);
    } else {
