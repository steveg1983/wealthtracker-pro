...
import { useHapticFeedback, HAPTIC_PATTERN } from '../hooks/useHapticFeedback';
import type { HapticPattern } from '../hooks/useHapticFeedback';
...
export function HapticButton({
  hapticPattern = HAPTIC_PATTERN.LIGHT,
  ...
  const handleClick = async (e: React.MouseEvent<HTMLButtonElement>) => {
    if (!disabled && !hapticDisabled) {
      ...
      if (hapticPattern === HAPTIC_PATTERN.LIGHT) {
        switch (variant) {
          case 'danger':
            pattern = HAPTIC_PATTERN.WARNING;
            break;
          case 'success':
            pattern = HAPTIC_PATTERN.SUCCESS;
            break;
          case 'primary':
            pattern = HAPTIC_PATTERN.MEDIUM;
            break;
          default:
...
export function HapticToggle({
  ...
  const handleToggle = async () => {
    if (!disabled && !hapticDisabled) {
      ...
      await trigger(checked ? HAPTIC_PATTERN.LIGHT : HAPTIC_PATTERN.MEDIUM);
    }
  ...
  const handleChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!disabled && !hapticDisabled) {
      ...
      if (currentStep !== lastStep) {
        await trigger(HAPTIC_PATTERN.SELECTION);
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <HapticButton
              size="sm"
              variant="ghost"
              onClick={() => testHaptic(HAPTIC_PATTERN.LIGHT)}
              hapticDisabled
            >
              Light
            </HapticButton>
            <HapticButton
              size="sm"
              variant="ghost"
              onClick={() => testHaptic(HAPTIC_PATTERN.MEDIUM)}
              hapticDisabled
            >
              Medium
            </HapticButton>
            <HapticButton
              size="sm"
              variant="ghost"
              onClick={() => testHaptic(HAPTIC_PATTERN.HEAVY)}
              hapticDisabled
            >
              Heavy
            </HapticButton>
            <HapticButton
              size="sm"
              variant="ghost"
              onClick={() => testHaptic(HAPTIC_PATTERN.SUCCESS)}
              hapticDisabled
            >
              Success
            </HapticButton>
