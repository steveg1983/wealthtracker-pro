...
import { formatPercentageFromRatio } from '@wealthtracker/utils';
...
const formatPercentageInput = (value: number, decimals: number = 2): string => {
...
export default function RetirementPlanner({ onDataChange }: RetirementPlannerProps) {
  ...
  const loadPlans = async () => {
    ...
    try {
      const retirementPlans = await financialPlanningService.getFinancialPlans(clerkId, {
        plan_type: 'retirement'
      });
      setPlans(retirementPlans || []);
      if (retirementPlans && retirementPlans.length > 0 && !selectedPlan) {
        setSelectedPlan(retirementPlans[0] ?? null);
      }
    } catch (error) {
  ...
  const handleDeletePlan = async (plan: FinancialPlan) => {
    if (!clerkId) return;

    if (window.confirm('Are you sure you want to delete this retirement plan?')) {
      try {
        await financialPlanningService.deleteFinancialPlan(clerkId, plan.id);
        await loadPlans();
        onDataChange();
  ...
  const formatPercentage = (value: number) => formatPercentageFromRatio(value, 1);
...
function RetirementPlanModal({ plan, onClose, onSave }: RetirementPlanModalProps) {
  const { region } = useRegionalSettings();
  const { currency, formatCurrency: formatRegionalCurrency } = useRegionalCurrency();
  const financialData = useRealFinancialData();
  const { accounts } = useApp();

  const retirementAccountTypes = new Set(['investment', 'assets', 'asset']);
  const totalRetirementSavings = accounts
    .filter(acc => retirementAccountTypes.has(acc.type as string))
    .reduce((sum, acc) => sum + (acc.balance || 0), 0);

  const defaultMonthlyIncome = financialData?.monthlyIncome?.toNumber?.() ?? 0;
  ...
  const [formData, setFormData] = useState({
    ...
    targetRetirementIncome: plan?.data?.targetRetirementIncome ?? defaultMonthlyIncome
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const planPayload: FinancialPlanCreate = {
      plan_type: 'retirement',
      name: formData.name || plan?.name || 'Retirement Plan',
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                type="number"
                value={formatPercentageInput(formData.expectedReturn, 2)}
                onChange={(e) => {
                  const inputValue = e.target.value;
                  if (inputValue === '') {
                    setFormData({ ...formData, expectedReturn: 0 });
                    return;
                  }
                  try {
                    const parsedValue = new Decimal(inputValue).dividedBy(100).toNumber();
                    setFormData({ ...formData, expectedReturn: parsedValue });
                  } catch {
            ...
            <div>
              ...
              <input
                type="number"
                value={formatPercentageInput(formData.inflationRate, 2)}
                onChange={(e) => {
                  const inputValue = e.target.value;
                  if (inputValue === '') {
                    setFormData({ ...formData, inflationRate: 0 });
                    return;
                  }
                  try {
                    const parsedValue = new Decimal(inputValue).dividedBy(100).toNumber();
                    setFormData({ ...formData, inflationRate: parsedValue });
                  } catch {
                    // ignore invalid numeric input
                  }
                }}
              ...
              {formData.targetRetirementIncome > 0 && yearsToRetirement > 0 && (
                <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                  Equivalent to <span className="font-semibold text-gray-700 dark:text-gray-300">
                    {formatRegionalCurrency(futureValueOfTargetIncome)}
                  </span> in {formData.retirementAge - formData.currentAge} years
                  ({formatPercentageFromRatio(formData.inflationRate, 1)} inflation)
                </p>
              )}
            </div>
          </div>
