...
export default function CategorySuggestion({ 
  ...
  const confidencePercentDecimal = new Decimal(topSuggestion.confidence ?? 0).times(100);
  const confidencePercent = confidencePercentDecimal.toDecimalPlaces(0, Decimal.ROUND_HALF_UP).toNumber();
  ...
  const confidenceDisplay = formatPercentageFromRatio(topSuggestion.confidence ?? 0, 0);
