...
type AccountCreatePayload = Omit<Account, 'id'> & { initialBalance: number };
...
export default function AddAccountModal({ isOpen, onClose }: AddAccountModalProps): React.JSX.Element {
  ...
  const setValidationErrorField = (
    field: keyof AccountValidationErrors,
    message?: string
  ) => {
  ...
  const updateField = <K extends keyof AccountFormData>(field: K, value: AccountFormData[K]) => {
    ...
    if (field === 'name') {
      ...
      setValidationErrorField('name', nameError);
    }
    ...
    if (field === 'sortCode' || field === 'accountNumber') {
      ...
      setValidationErrorField('bankDetails', bankError);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!formData.balance || formData.balance.toString().trim() === '') {
        throw new Error('Please enter a valid balance');
      }

      const balanceDecimal = parseCurrencyDecimal(formData.balance);
      if (balanceDecimal.isNaN()) {
        throw new Error('Please enter a valid balance');
      }

      const balance = toStorageNumber(balanceDecimal);
      
      // Create the account with bank details if provided
      const accountData: AccountCreatePayload = {
