...
type SupabaseRealtimeChannel = ReturnType<SupabaseDatabase['channel']>;
...
export default function RealtimeSyncTest() {
  ...
  const addStatus = useCallback((message: string) => {
    ...
    setTestStatus(prev =>
      [...prev, `${new Date().toLocaleTimeString()}: ${message}`].slice(-EVENT_HISTORY_LIMIT)
    );
  }, [log]);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    let client: SupabaseDatabase | null = null;
    ...
    const runTest = async () => {
      try {
        ...
        createTimeoutRef.current = setTimeout(async () => {
          ...
          const testAccount: AccountInsert = {
            user_id: databaseId,
  ...
  }, [addStatus, clerkId, databaseId, isLoading, log]);
