...
type ConditionField = 'description' | 'amount' | 'accountId';
type ConditionOperator =
  ...
  | 'lessThan'
  | 'between';
type ActionType = 'setCategory' | 'addTag' | 'modifyDescription' | 'setAccount' | 'skip';
type DescriptionModification = 'replace' | 'prepend' | 'append' | 'regex';
...
function RuleFormModal({ rule, categories, accounts, onSave, onClose }: RuleFormModalProps) {
  ...
  const updateCondition = (index: number, updates: Partial<ImportRuleCondition>) => {
    ...
    if (currentCondition) {
      conditions[index] = { ...currentCondition, ...updates };
  ...
  const updateAction = (index: number, updates: Partial<ImportRuleAction>) => {
    ...
    if (currentAction) {
      actions[index] = { ...currentAction, ...updates };
                  ...
                  <div key={index} className="flex gap-2 items-start bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg">
                    ...
                    <input
                      ...
                      onChange={(e) => updateCondition(index, {
                        value: condition.field === 'amount'
                          ? toStorageNumber(parseCurrencyDecimal(e.target.value))
                          : e.target.value
                      })}
                      placeholder="Value"
                    ...
                    {condition.operator === 'between' && (
                      <input
                        ...
                        onChange={(e) => updateCondition(index, {
                          value2: toStorageNumber(parseCurrencyDecimal(e.target.value))
                        })}
                    ...
                    {action.type === 'modifyDescription' && (
                      <>
                        <select
                          ...
                          onChange={(e) => updateAction(index, { modification: e.target.value as DescriptionModification })}
                          className="px-2 py-1 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded text-sm"
                        >
