...
export default function ImportDataModal({ isOpen, onClose }: ImportDataModalProps): React.JSX.Element {
  const { addAccount, addTransaction, accounts, transactions, clearAllData } = useApp();
  ...
  const parseOFX = (content: string): ParsedData => {
    ...
    const balanceDecimal = balanceMatch ? parseCurrencyDecimal(balanceMatch[1] || '0') : parseCurrencyDecimal(0);
    ...
    for (const trans of transactionMatches) {
      ...
      if (dateMatch && amountMatch) {
        ...
        const amountDecimal = parseCurrencyDecimal(amountMatch[1] || '0');
  ...
  const handleClearAndImport = async () => {
    ...
    setTimeout(() => {
      // Now import the data - existing data cleared
      if (preview) {
        setImporting(true);
        importDataToApp();
      }
  ...
  const handleImport = async () => {
    ...
    if (hasExistingData && !showTestDataWarning) {
                  ...
                  {preview.transactions.length > 0 && (
                    <>
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                        First: {preview.transactions[0]?.description.substring(0, 30) || 'N/A'}...
                      </p>
                    </>
                  )}
                </div>
