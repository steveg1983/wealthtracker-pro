...
import {
  ...
  HAS_REQUIRED_SUBSCRIPTION_MESSAGE,
  UPGRADE_TO_PREMIUM_TEXT,
  GO_BACK_TEXT,
  PREMIUM_INCLUDES_TITLE,
  PREMIUM_FEATURES,
  ACCESS_DENIED_TITLE,
  ACCESS_DENIED_MESSAGE,
  RETURN_TO_DASHBOARD_TEXT,
  DEVELOPMENT_MODE_BANNER_TEXT,
  CHECKING_SUBSCRIPTION_TEXT
} from '../../constants/authMessages';
import type { SubscriptionTier, UserSubscription } from '@app-types/subscription';
...
export function ProtectedRoute({ 
  ...
  const shouldBypassChecks = isTestMode || isDemoMode || isDevelopmentMode;

  // Show loading state while Clerk is initializing (skip in test mode, demo mode, or development)
  if (!isLoaded && !shouldBypassChecks) {
    return (
  ...
  if (!isSignedIn && !shouldBypassChecks) {
            ...
            <div className="mt-6 p-4 bg-blue-50 dark:bg-gray-900/20 rounded-lg">
              <div className="flex items-start gap-2">
                ...
                <div className="text-sm text-blue-800 dark:text-blue-200">
                  <p className="font-medium mb-1">{PREMIUM_INCLUDES_TITLE}</p>
      ...
      return (
        <div className="min-h-screen bg-blue-50 dark:bg-gray-900 flex items-center justify-center p-4">
          <div className="max-w-md w-full bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8">
            ...
            <p className="text-gray-600 dark:text-gray-400 text-center mb-6">
              {ACCESS_DENIED_MESSAGE(requiredRole)}
            </p>
