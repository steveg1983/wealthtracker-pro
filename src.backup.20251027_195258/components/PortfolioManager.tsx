...
const PortfolioManager = memo(function PortfolioManager({ accountId: _accountId, holdings, onUpdate }: PortfolioManagerProps) {
  ...
  const sharesDecimalInput = parseCurrencyDecimal(shares);
  const averageCostDecimalInput = parseCurrencyDecimal(averageCost);
  const showCostBasisPreview = shares.trim() !== '' && averageCost.trim() !== '';
  ...
  const validateAndSaveHolding = async () => {
    ...
    const sharesDecimal = parseCurrencyDecimal(shares);
    const costDecimal = parseCurrencyDecimal(averageCost);
    const sharesNum = sharesDecimal.toNumber();
    ...
    try {
      ...
      logger.error('Unable to validate symbol.', error);
    ...
    <div className="space-y-6">
              ...
              <div className="flex-1">
                <h4 className="font-semibold text-gray-900 dark:text-white">
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    {new Decimal(holding.shares).toDecimalPlaces(2, Decimal.ROUND_HALF_UP).toNumber()} shares @ {formatCurrency(holding.averageCost)}
                  </p>
              </div>
      ...
      >
        <div className="space-y-4">
          ...
          {/* Cost Basis Preview */}
          {showCostBasisPreview && !sharesDecimalInput.isNaN() && !averageCostDecimalInput.isNaN() && (
            <div className="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Cost Basis: {formatCurrency(sharesDecimalInput.times(averageCostDecimalInput))}
              </p>
            </div>
          )}
