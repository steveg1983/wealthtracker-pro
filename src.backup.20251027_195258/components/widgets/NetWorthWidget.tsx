...
import type { DecimalInstance, DecimalAccount, DecimalTransaction } from '../../types/decimal-types';
...
export default function NetWorthWidget({ size }: NetWorthWidgetProps): React.JSX.Element {
  ...
  const { netWorth, change, changePercent, chartData } = useMemo(() => {
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      transactions.filter((t: DecimalTransaction) => {
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <div className={`flex items-center gap-1 text-sm ${
            ...
            <span>{isPositive ? '+' : ''}{formatCurrency(change)}</span>
            <span>({formatSignedPercentageValue(changePercent, 2)})</span>
          </div>
        </div>
      </div>
