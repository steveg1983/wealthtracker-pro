...
const ALLOWED_PERIODS: readonly ReminderPeriod[] = [7, 14, 30];

const resolvePeriod = (value: number | undefined): ReminderPeriod => {
  if (value && ALLOWED_PERIODS.includes(value as ReminderPeriod)) {
    return value as ReminderPeriod;
  }
  return 14;
};
...
export default function BillReminderWidget({
  ...
  const initialPeriod = resolvePeriod(settings.daysAhead);
  const [selectedPeriod, setSelectedPeriod] = useState<ReminderPeriod>(
    initialPeriod
  );
  ...
  const upcomingBills = useMemo(() => {
    ...
    transactionGroups.forEach((groupTransactions) => {
      ...
      for (let i = 0; i < sorted.length - 1 && i < 5; i++) {
        ...
        if (!currentTx || !nextTx) continue;

        const days = Math.floor(
          (new Date(currentTx.date).getTime() - new Date(nextTx.date).getTime())
          / (1000 * 60 * 60 * 24)
        );
        intervals.push(days);
      }
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full text-center p-4">
        <div className="w-16 h-16 bg-gray-100 dark:bg-gray-800 rounded-full flex items-center justify-center mb-4">
          <ProfessionalIcon name="notification" size={32} className="text-gray-400" />
        </div>
  ...
  const handlePeriodChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    const periodValue = Number(event.target.value) as ReminderPeriod;
    ...
    <div className="space-y-4">
      ...
      {overdueBills.length > 0 && (
        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-3">
          <div className="flex items-center">
            <ProfessionalIcon name="error" size={20} className="text-red-600 dark:text-red-400 mr-2" />
          ...
          >
            <div className="flex items-start justify-between">
              <div className="flex-1">
                <div className="flex items-center">
                  {bill.isOverdue ? (
                    <ProfessionalIcon name="error" size={16} className="text-red-500 mr-2" />
                  ) : bill.isPaid ? (
                    <ProfessionalIcon name="success" size={16} className="text-green-500 mr-2" />
                  ) : (
                    <ProfessionalIcon name="clock" size={16} className="text-gray-400 mr-2" />
                  )}
      ...
      {upcomingBills.length > 5 && (
        ...
        >
          ...
          <ProfessionalIcon name="chevronRight" size={16} className="ml-1" />
        </button>
      )}
