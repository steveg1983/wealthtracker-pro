...
export default function ExpenseBreakdownWidget({ size, settings }: ExpenseBreakdownWidgetProps): React.JSX.Element {
  ...
  const expenseData: ExpenseDatum[] = useMemo(() => {
    const now = new Date();
    ...
    const categoryTotals = expenses.reduce((acc, t) => {
      ...
      const current = acc.get(categoryName) ?? toDecimal(0);
      acc.set(categoryName, current.plus(amount));
      return acc;
    }, new Map<string, DecimalInstance>());

    return Array.from(categoryTotals.entries())
      .map(([name, amount]) => ({ name, amount, value: amount.toNumber() }))
        ...
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie
              ...
              cy="50%"
              labelLine={false}
              label={size !== 'small' ? ({ name, percent }) => `${name} ${formatPercentageFromRatio(percent ?? 0, 0)}` : false}
            ...
            {showLegend && size !== 'small' && (
              <Legend 
                ...
                formatter={(value, entry) => {
                  const payload =
                    typeof entry === 'object' && entry !== null && 'payload' in entry
                      ? (entry as { payload?: ExpenseDatum }).payload
                      : undefined;
                  const amount = payload?.amount ?? toDecimal(0);
