...
export default function SyncStatusWidget({ 
  ...
  }, [settings.autoRefresh, checkSyncStatus]);
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case 'synced':
        return <ProfessionalIcon name="success" size={20} />;
      case 'syncing':
        return <ProfessionalIcon name="refresh" size={20} className="animate-spin" />;
      case 'error':
        return <ProfessionalIcon name="close2" size={20} />;
      case 'pending':
        return <ProfessionalIcon name="clock" size={20} />;
      case 'offline':
        return <ProfessionalIcon name="wifi" size={20} />;
      default:
        return <ProfessionalIcon name="warning" size={20} />;
    }
  };
    ...
    <div className="space-y-4">
          ...
          >
            {isSyncing ? (
              <span className="flex items-center">
                <ProfessionalIcon name="refresh" size={16} className="mr-1 animate-spin" />
                Syncing
              </span>
            ) : (
              'Sync All'
      ...
      {!isOnline && (
        <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-3">
          <div className="flex items-center">
            <ProfessionalIcon name="wifi" size={20} className="text-yellow-600 dark:text-yellow-400 mr-2" />
            ...
            >
              <div className="flex items-center justify-between">
                ...
                {status.status !== 'syncing' && isOnline && (
                  ...
                  >
                    <ProfessionalIcon name="refresh" size={16} />
                  </button>
                )}
                ...
                {status.status === 'syncing' && status.totalItems && (
                  <div className="text-xs text-gray-500 dark:text-gray-400">
                    {(status.itemsSynced ?? 0)}/{status.totalItems}
                  </div>
                )}
              </div>
                ...
                <div className="mt-2">
                  <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1">
                    <div 
                      ...
                      style={{ 
                        width: `${status.totalItems ? ((status.itemsSynced ?? 0) / status.totalItems) * 100 : 0}%` 
                      }}
                    />
                  </div>
                </div>
      ...
      <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
        <div className="flex items-center">
          <ProfessionalIcon name="upload" size={16} className="mr-1" />
          <span>
