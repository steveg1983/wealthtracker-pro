  ...
  const { formData, updateField, handleSubmit, setFormData } = useModalForm<FormData>(
    ...
    },
    {
      onSubmit: (data) => {
        ...
        const normalizedSortCode = toOptionalString(data.sortCode);
        const normalizedAccountNumber = toOptionalString(data.accountNumber);

        // Professional validation for bank details
        if (normalizedSortCode && normalizedAccountNumber &&
            normalizedSortCode !== '00-00-00' && normalizedAccountNumber !== '00000000') {
          const duplicate = accounts.find(a => 
            a.sortCode === normalizedSortCode && 
        ...
        const updates: Partial<Account> = { type: data.type };

        const institution = toOptionalString(data.institution);
        ...
        if (normalizedSortCode !== undefined) {
          updates.sortCode = normalizedSortCode;
        } else if (account.sortCode) {
          updates.sortCode = '';
        }

        if (normalizedAccountNumber !== undefined) {
          updates.accountNumber = normalizedAccountNumber;
        } else if (account.accountNumber) {
          updates.accountNumber = '';
        }

        if (data.openingBalance) {
          updates.openingBalance = toStorageNumber(parseCurrencyDecimal(data.openingBalance));
  ...
  useEffect(() => {
    if (account) {
      const openingBalanceValue =
        account.openingBalance !== undefined ? account.openingBalance.toString() : '';
      const openingBalanceDateValue = account.openingBalanceDate
      ...
      setFormData({
        ...
        openingBalanceDate: openingBalanceDateValue.toISOString().slice(0, 10),
