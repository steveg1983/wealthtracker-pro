...
export default function EnhancedExportModal({ 
  ...
  type GroupByOption = NonNullable<ExportOptions['groupBy']>;
  type BooleanExportOption = 'includeCharts' | 'includeTransactions' | 'includeAccounts' | 'includeInvestments' | 'includeBudgets';
  ...
  const exportToggleOptions: Array<{ key: BooleanExportOption; label: string; disabled?: boolean }> = [
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-2xl p-6 max-h-[90vh] overflow-y-auto">
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  const nextFormat = e.target.value as ExportOptions['format'];
                  setExportOptions(prev => ({ ...prev, format: nextFormat }));
                }}
                className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
              >
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  const nextGroupBy = e.target.value as GroupByOption;
                  setExportOptions(prev => ({ ...prev, groupBy: nextGroupBy }));
                }}
                className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
              >
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {exportToggleOptions.map(({ key, label, disabled }) => (
