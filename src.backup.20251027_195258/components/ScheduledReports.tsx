...
const calculateNextRun = (report: ScheduledReport, fromDate: Date = new Date()): Date => {
  const [hoursRaw, minutesRaw] = report.time.split(':');
...
`;
      ...
      const notification = report.format !== 'pdf'
        ? {
            ...baseNotification,
            action: {
              label: 'View Report',
  ...
  }, [addNotification, accounts, categories, formatCurrency, transactions]);
  ...
  }, [generateReport, reports, updateNextRunTime]);

  const handleAddReport = (reportData: Partial<ScheduledReport>) => {
    const baseConfig = reportData.reportConfig ?? {
    ...
    const newReport: ScheduledReport = {
      ...
      reportConfig: {
        ...
        includeAccounts: baseConfig.includeAccounts ?? [],
        ...(baseConfig.includeCategories && baseConfig.includeCategories.length > 0
          ? { includeCategories: baseConfig.includeCategories }
          : {}),
        ...(baseConfig.customDays !== undefined ? { customDays: baseConfig.customDays } : {})
      }
    };

    if (reportData.dayOfWeek !== undefined) {
      newReport.dayOfWeek = reportData.dayOfWeek;
    }

    if (reportData.dayOfMonth !== undefined) {
      newReport.dayOfMonth = reportData.dayOfMonth;
    }

    const initialRecipients = reportData.recipients ?? [];
    if (initialRecipients.length > 0) {
      newReport.recipients = initialRecipients;
    }
  ...
  const handleUpdateReport = (reportId: string, updates: Partial<ScheduledReport>) => {
    setReports(prev => prev.map(report => {
      if (report.id === reportId) {
        ...
        const mergedRecipients = updates.recipients ?? report.recipients;
        if (mergedRecipients && mergedRecipients.length > 0) {
          updated.recipients = mergedRecipients;
        } else {
          delete updated.recipients;
        }

        if (updates.dayOfWeek === undefined && 'dayOfWeek' in updates) {
          delete updated.dayOfWeek;
        }

        if (updates.dayOfMonth === undefined && 'dayOfMonth' in updates) {
          ...
          <div className="p-6 space-y-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(event) => {
                  const nextFrequency = event.target.value as ScheduledReport['frequency'];
                  setFormData(prev => ({ ...prev, frequency: nextFrequency }));
                }}
                className="w-full px-3 py-2 bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm border border-gray-300/50 dark:border-gray-600/50 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
              >
            ...
            <div>
              ...
              <select
                ...
                onChange={(event) => {
                  const nextFormat = event.target.value as ScheduledReport['format'];
                  setFormData(prev => ({ ...prev, format: nextFormat }));
                }}
                className="w-full px-3 py-2 bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm border border-gray-300/50 dark:border-gray-600/50 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
              >
            ...
            <div>
              ...
              <select
                ...
                onChange={(event) => {
                  const nextRange = event.target.value as ScheduledReport['reportConfig']['dateRange'];
                  setFormData(prev => ({
                    ...prev,
                    reportConfig: {
                      ...(prev.reportConfig ?? { dateRange: 'month', includeAccounts: [] }),
                      dateRange: nextRange
                    }
                  }));
                }}
