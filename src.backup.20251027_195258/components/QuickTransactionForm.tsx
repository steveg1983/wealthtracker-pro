...
export function QuickTransactionForm({ 
  ...
  const transferCategories = useMemo(
    () => categories.filter(category => category.isTransferCategory && !!category.accountId),
    [categories]
  );
  ...
  const filteredCategories = useMemo(() => {
    ...
    return categories.filter(category => category.type === type && !category.parentId);
  }, [categories, transferCategories, type]);
  ...
  useEffect(() => {
    ...
    if (type === 'transfer') {
      const matchingTransferCategory = filteredCategories.find(
        category => category.accountId === toAccountId
      );

      if (matchingTransferCategory) {
        if (matchingTransferCategory.id !== categoryId) {
          setCategoryId(matchingTransferCategory.id);
        }
        return;
      }
    }

    const currentCategoryInList = filteredCategories.some(category => category.id === categoryId);
    if (!currentCategoryInList) {
      setCategoryId(filteredCategories[0]!.id);
    }
  }, [categoryId, filteredCategories, toAccountId, type]);
  ...
  useEffect(() => {
    ...
    const alternativeAccount = accounts.find(account => account.id !== accountId);
    if (alternativeAccount) {
      setToAccountId(alternativeAccount.id);
    } else {
      setToAccountId('');
    }
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (type === 'transfer') {
      ...
      if (toAccountId === accountId) {
        showError('Select a different destination account for transfers');
        return;
      }
    }
    ...
    try {
      const amountDecimal = parseCurrencyDecimal(amount);
      const normalizedAmount = toStorageNumber(amountDecimal.abs());
      const signedAmount = type === 'income' ? normalizedAmount : -normalizedAmount;
      ...
      const transferMetadata = type === 'transfer' && toAccountId
        ? {
              ...
              {type === 'transfer' ? (
                ...
                <>
                  ...
                  <select
                    ...
                    className="w-full px-2 py-2 text-sm bg-blue-50 dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent"
                    required={type === 'expense' || type === 'income'}
                  >
