...
import type { ProfessionalIconName } from '../icons/ProfessionalIcons';
...
export default function BillingDashboard({
  ...
  const getStatusBadge = (status: string) => {
    let config: { color: string; icon: ProfessionalIconName };

    switch (status) {
      case 'active':
        config = { color: 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-200', icon: 'success' };
        break;
      case 'trialing':
        config = { color: 'bg-blue-100 text-blue-800 dark:bg-gray-900/20 dark:text-blue-200', icon: 'success' };
        break;
      case 'past_due':
        config = { color: 'bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-200', icon: 'warning' };
        break;
      case 'cancelled':
        config = { color: 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-200', icon: 'warning' };
        break;
      default:
        config = { color: 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-200', icon: 'warning' };
    }
    ...
    return (
      <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${config.color}`}>
        <ProfessionalIcon name={config.icon} size={12} />
        {status.replace('_', ' ')}
      </span>
    );
  };
    ...
    return (
      <div className={`bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-6 ${className}`}>
        <div className="flex items-center gap-3">
          <ProfessionalIcon name="warning" size={20} className="text-red-600 dark:text-red-400" />
          <div>
  ...
  return (
    <div className={`space-y-8 ${className}`}>
      ...
      <div className="flex items-center justify-between">
        ...
        >
          <ProfessionalIcon name="settings" size={16} />
          Manage Billing
          <ProfessionalIcon name="link" size={14} />
        </button>
      </div>
          ...
          <div className="space-y-6">
            ...
            {subscription.status === 'trialing' && subscription.trialEnd && (
              <div className="bg-amber-50 dark:bg-amber-900/20 rounded-xl p-4 shadow-md border-l-4 border-amber-400 dark:border-amber-600">
                <div className="flex items-start gap-3">
                  <ProfessionalIcon name="success" size={20} className="text-amber-600 dark:text-amber-400 flex-shrink-0 mt-0.5" />
                  <div>
            ...
            {subscription.cancelAtPeriodEnd && (
              <div className="bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-lg p-4">
                <div className="flex items-start gap-3">
                  <ProfessionalIcon name="warning" size={20} className="text-orange-600 dark:text-orange-400 flex-shrink-0 mt-0.5" />
                  <div>
          ...
          <div className="space-y-3">
            {billingHistory.paymentMethods.map((method) => (
              <div key={method.id} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                <div className="flex items-center gap-3">
                  <ProfessionalIcon name="creditCard" size={20} className="text-gray-600 dark:text-gray-400" />
                  <div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-md p-6">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-6">
              We're working on in-app plan management. In the meantime, please contact support or visit the billing portal to adjust your plan.
            </p>
