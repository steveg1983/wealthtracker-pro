...
import type { SubscriptionPlan, SubscriptionTier } from '@app-types/subscription';
            ...
            {plan.isPopular && (
              <div className="absolute -top-4 left-1/2 transform -translate-x-1/2">
                <div className="bg-gradient-to-r from-gray-600 to-purple-600 text-white px-4 py-1 rounded-full text-sm font-medium flex items-center gap-1">
                  <ProfessionalIcon name="star" size={14} />
                  Most Popular
                </div>
              </div>
            )}
            ...
            {plan.tier === currentTier && (
              <div className="absolute -top-4 right-4">
                <div className="bg-green-500 text-white px-3 py-1 rounded-full text-sm font-medium flex items-center gap-1">
                  <ProfessionalIcon name="check" size={14} />
                  Current
                </div>
              </div>
            )}
              ...
              <div className="text-center mb-8">
                ...
                >
                  {loadingPlan === plan.id ? (
                    ...
                    <>
                      {plan.tier === 'free' && <ProfessionalIcon name="star" size={16} />}
                      {plan.tier !== 'free' && plan.tier !== currentTier && <ProfessionalIcon name="creditCard" size={16} />}
                      {getPlanButtonText(plan)}
                      {plan.tier !== currentTier && <ProfessionalIcon name="arrowRight" size={16} />}
                    </>
                  )}
                </button>
              </div>
                ...
                <ul className="space-y-3">
                  {plan.features.map((feature, index) => (
                    <li key={index} className="flex items-start gap-3">
                      <ProfessionalIcon name="check" size={16} className="text-green-500 mt-0.5 flex-shrink-0" />
        ...
        <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-8">
          ...
          <div className="flex flex-wrap justify-center gap-6 text-sm text-gray-600 dark:text-gray-400">
            <div className="flex items-center gap-2">
              <ProfessionalIcon name="check" size={16} className="text-green-500" />
              Cancel anytime
            </div>
            <div className="flex items-center gap-2">
              <ProfessionalIcon name="check" size={16} className="text-green-500" />
