...
function ScheduleReportModal({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const baseReport: ScheduledCustomReport = {
      ...
      ...(scheduledReport?.lastRun ? { lastRun: scheduledReport.lastRun } : {})
    };

    const scheduleForNextRun: ScheduledCustomReport = {
      ...baseReport,
      ...(formData.dayOfWeek !== undefined ? { dayOfWeek: formData.dayOfWeek } : {}),
      ...(formData.dayOfMonth !== undefined ? { dayOfMonth: formData.dayOfMonth } : {})
    };

    const report: ScheduledCustomReport = {
      ...scheduleForNextRun,
      nextRun: scheduledReportService.calculateNextRun(scheduleForNextRun)
    };
          ...
          <div className="p-6 space-y-4 overflow-y-auto max-h-[60vh]">
            ...
            <div>
              ...
              <select
                ...
                onChange={(event) => {
                  const nextFrequency = event.target.value as ScheduledCustomReport['frequency'];
            ...
            <div>
              ...
              <select
                ...
                onChange={(event) => {
                  const nextFormat = event.target.value as ScheduledCustomReport['deliveryFormat'];
                  setFormData(prev => ({ ...prev, deliveryFormat: nextFormat }));
                }}
                className="w-full px-3 py-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg"
              >
