...
export default function DuplicateDetection({ 
  ...
  const calculateStringSimilarity = (str1: string, str2: string): number => {
    ...
    const similarityDecimal = toDecimal(1)
      .minus(toDecimal(distance).dividedBy(maxLen))
      .times(100);
    if (similarityDecimal.lessThan(0)) {
      return 0;
    }
    return similarityDecimal.toDecimalPlaces(0).toNumber();
  };
  ...
  const calculateSimilarity = useCallback((t1: Transaction, t2: Transaction): SimilarityScore => {
    ...
    const daysDiffDecimal = toDecimal(date1.getTime())
      .minus(date2.getTime())
      .abs()
      .dividedBy(msPerDay);
    const dateThresholdDecimal = toDecimal(dateThreshold);
    let dateScoreDecimal = toDecimal(0);
    if (dateThresholdDecimal.greaterThan(0) && daysDiffDecimal.lessThanOrEqualTo(dateThresholdDecimal)) {
      const ratio = daysDiffDecimal.dividedBy(dateThresholdDecimal);
      dateScoreDecimal = toDecimal(100).minus(ratio.times(50));
    }
    const dateScore = dateScoreDecimal.lessThan(0) ? 0 : dateScoreDecimal.toNumber();
    
    // Amount similarity
    const amount1Decimal = toDecimal(t1.amount);
    const amount2Decimal = toDecimal(t2.amount);
    const amountDiffDecimal = amount1Decimal.minus(amount2Decimal).abs();
    const amountThresholdDecimal = toDecimal(amountThreshold);
    const absAmount1 = amount1Decimal.abs();
    const absAmount2 = amount2Decimal.abs();
    const amountMaxDecimal = absAmount1.greaterThan(absAmount2) ? absAmount1 : absAmount2;
    let amountScoreDecimal = toDecimal(0);
    if (amountDiffDecimal.lessThanOrEqualTo(amountThresholdDecimal)) {
      amountScoreDecimal = toDecimal(100);
    } else if (amountMaxDecimal.greaterThan(0)) {
      amountScoreDecimal = toDecimal(100).minus(
        amountDiffDecimal.dividedBy(amountMaxDecimal).times(100)
      );
    } else {
      amountScoreDecimal = toDecimal(100);
    }
    if (amountScoreDecimal.lessThan(0)) {
      amountScoreDecimal = toDecimal(0);
    }
    const amountScore = amountScoreDecimal.toNumber();
    
    // Description similarity
    const descriptionScore = calculateStringSimilarity(t1.description, t2.description);
    
    // Total weighted score
    const totalScoreDecimal = toDecimal(dateScore).times(0.3)
      .plus(toDecimal(amountScore).times(0.4))
      .plus(toDecimal(descriptionScore).times(0.3));
    const totalScore = totalScoreDecimal.toNumber();
  ...
  const findExistingDuplicates = useMemo(() => {
    ...
    transactions.forEach((transaction, index) => {
      ...
      if (potential.length > 0) {
        ...
        const confidenceDecimal = potential.reduce((max, candidate) => {
          const score = toDecimal(calculateSimilarity(transaction, candidate).totalScore);
          return score.greaterThan(max) ? score : max;
        }, toDecimal(0));
        const confidence = confidenceDecimal.toNumber();
  ...
  const checkNewTransactions = useCallback(() => {
    ...
    newTransactions.forEach((candidate, index) => {
      const progressDecimal = toDecimal(index + 1)
        .dividedBy(newTransactions.length)
        .times(100);
      setScanProgress(progressDecimal.toNumber());
      ...
      if (matches.length > 0) {
        const confidenceDecimal = matches.reduce((max, match) => {
          const score = toDecimal(calculateSimilarity(normalized, match).totalScore);
          return score.greaterThan(max) ? score : max;
        }, toDecimal(0));
        const confidence = confidenceDecimal.toNumber();
        ...
        if (autoSelectHighConfidence && confidenceDecimal.greaterThanOrEqualTo(toDecimal(90))) {
    ...
    setScanning(false);
  }, [newTransactions, transactions, similarityThreshold, autoSelectHighConfidence, calculateSimilarity]);
  ...
  }, [isOpen, newTransactions, checkNewTransactions, findExistingDuplicates]);
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setAmountThreshold(parseCurrencyDecimal(e.target.value || '0').toNumber())}
                min="0"
                step="0.01"
                className="w-full px-3 py-1 border border-gray-300 dark:border-gray-600 rounded
                         bg-white dark:bg-gray-700"
        ...
        {scanning && (
          <div className="mb-6">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-sm text-gray-600 dark:text-gray-400">
                {formatPercentageValue(scanProgress, 0)}
              </span>
            </div>
              ...
              <div key={index} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div className={`text-sm font-medium ${getConfidenceColor(group.confidence)}`}>
                    {formatPercentageValue(group.confidence, 0)} match
                  </div>
                </div>
                  ...
                  {group.potential.map(transaction => (
                    <div key={transaction.id} className="flex items-center justify-between bg-gray-50 
                      ...
                      <div className="text-xs text-gray-500">
                        {formatPercentageValue(calculateSimilarity(group.original, transaction).totalScore, 0)} similar
                      </div>
                    </div>
                  ))}
                </div>
