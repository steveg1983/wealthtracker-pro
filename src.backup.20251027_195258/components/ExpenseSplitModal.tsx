...
export default function ExpenseSplitModal({ 
  ...
  const handleCreateSplit = () => {
    ...
    const splits = selectedMembers.map<{ memberId: string; amount: DecimalInstance; percentage?: number }>(memberId => {
      const amount = toDecimal(customSplits[memberId]?.amount || 0);

      if (splitMethod === 'percentage') {
        const percentageValue = customSplits[memberId]?.percentage;
        return percentageValue !== undefined
          ? { memberId, amount, percentage: percentageValue }
          : { memberId, amount };
      }
            ...
            {splitMethod === 'percentage' && (
              <div className="flex items-center justify-between mt-2">
                ...
                <span className={`font-bold ${
                  ...
                  {formatPercentageValue(getTotalPercentage(), 1)}
                </span>
              </div>
            )}
          </div>
