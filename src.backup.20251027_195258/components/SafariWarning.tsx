...
import {
  shouldShowSafariWarning,
  dismissSafariWarning,
  initClerkSafariCompat,
  type SafariCompatInfo
} from '../utils/clerkSafarifix';
...
export default function SafariWarning(): React.JSX.Element | null {
  ...
  const hasPrivateMode = !(compatInfo.localStorage ?? true) || !(compatInfo.sessionStorage ?? true);
  const hasCookieIssues = !(compatInfo.thirdPartyCookies ?? true);
