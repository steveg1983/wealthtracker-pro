...
const SORT_OPTIONS: SortOption[] = ['name', 'confidence', 'frequency', 'lastUpdated'];
...
export default function MerchantEnrichment({ onDataChange }: MerchantEnrichmentProps) {
  ...
  const highConfidenceCount = useMemo(
    () => merchants.filter(merchant => merchant.confidence >= 0.9).length,
    [merchants]
  );
  ...
  const highConfidencePercent = useMemo(() => {
    ...
    return toDecimal(highConfidenceCount).dividedBy(merchants.length).times(100);
  }, [highConfidenceCount, merchants.length]);

  const averageConfidencePercent = useMemo(() => {
    if (merchants.length === 0) {
      return toDecimal(0);
    }
    const totalConfidence = merchants.reduce(
      (sum, merchant) => sum.plus(toDecimal(merchant.confidence)),
      toDecimal(0)
    );
    return totalConfidence.dividedBy(merchants.length).times(100);
  }, [merchants]);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <span className={`font-medium ${getConfidenceColor(enrichmentResult.confidence)}`}>
                  {formatPercentageValue(toDecimal(enrichmentResult.confidence).times(100), 0)}
                </span>
              </div>
              <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg p-4 shadow-sm">
          ...
          <div className="mt-2 text-sm text-gray-500 dark:text-gray-400">
            {formatPercentageValue(highConfidencePercent, 0)} accuracy
          </div>
        </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-orange-600 dark:text-orange-400">
                {formatPercentageValue(averageConfidencePercent, 0)}
              </p>
            </div>
                  ...
                  <tr key={merchant.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${getConfidenceBadge(merchant.confidence)}`}>
                        <StarIcon size={12} />
                        {formatPercentageValue(toDecimal(merchant.confidence).times(100), 0)}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
