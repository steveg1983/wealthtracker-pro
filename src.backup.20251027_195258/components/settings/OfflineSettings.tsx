...
const formatBytes = (bytes: number): string => {
  ...
  const valueDecimal = toDecimal(bytes).dividedBy(Decimal.pow(1024, exponent));
  const decimals = exponent === 0 ? 0 : 1;
  return `${valueDecimal.toDecimalPlaces(decimals, Decimal.ROUND_HALF_UP).toString()} ${units[exponent]}`;
};
