...
const formatDecimalValue = (value: DecimalInstance | number, decimals: number = 2): number =>
  new Decimal(value ?? 0).toDecimalPlaces(decimals, Decimal.ROUND_HALF_UP).toNumber();

const formatSignedPercentage = (value: DecimalInstance | number, decimals: number = 2): string => {
  const decimalValue = new Decimal(value ?? 0);
  const sign = decimalValue.greaterThanOrEqualTo(0) ? '+' : '';
  return `${sign}${formatDecimalValue(decimalValue, decimals)}%`;
};
...
export default function RealTimePortfolioEnhanced({ 
  ...
  const { formatCurrency, getCurrencySymbol } = useCurrencyDecimal();
  const [isRefreshing, setIsRefreshing] = useState(false);
  const currencyLabel = useMemo(
    () => `${getCurrencySymbol(baseCurrency)} ${baseCurrency.toUpperCase()}`.trim(),
    [baseCurrency, getCurrencySymbol]
  );
const formatInBaseCurrency = useCallback(
    (amount: DecimalInstance | number) => formatCurrency(amount, baseCurrency),
    ...
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl rounded-2xl shadow-lg border border-white/20 dark:border-gray-700/50 p-4">
          <div className="flex items-center justify-between">
            <div className="flex-1">
              <p className="text-sm text-gray-500 dark:text-gray-400">Portfolio Value ({currencyLabel})</p>
              {isLoading ? (
                <Skeleton variant="text" className="w-24 h-8" />
              ) : (
                <p className="text-xl font-bold text-gray-900 dark:text-white">
                  {formatInBaseCurrency(portfolioMetrics.totalValue)}
                </p>
              )}
            </div>
        ...
        <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl rounded-2xl shadow-lg border border-white/20 dark:border-gray-700/50 p-4">
          <div className="flex items-center justify-between">
            <div className="flex-1">
              <p className="text-sm text-gray-500 dark:text-gray-400">Day Change ({currencyLabel})</p>
              {isLoading ? (
                <Skeleton variant="text" className="w-24 h-8" />
              ) : (
                <>
                  <p className={`text-xl font-bold ${
                    ...
                    {portfolioMetrics.totalDayChange.greaterThanOrEqualTo(0) ? '+' : ''}
                    {formatInBaseCurrency(portfolioMetrics.totalDayChange)}
                  </p>
                  <p className={`text-sm ${
                    ...
                    {formatSignedPercentage(portfolioMetrics.totalDayChangePercent, 2)}
                  </p>
                </>
              )}
            </div>
        ...
        <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl rounded-2xl shadow-lg border border-white/20 dark:border-gray-700/50 p-4">
          <div className="flex items-center justify-between">
            <div className="flex-1">
              <p className="text-sm text-gray-500 dark:text-gray-400">Total Gain/Loss ({currencyLabel})</p>
              {isLoading ? (
                <Skeleton variant="text" className="w-24 h-8" />
              ) : (
                <>
                  <p className={`text-xl font-bold ${
                    ...
                    {portfolioMetrics.totalGain.greaterThanOrEqualTo(0) ? '+' : ''}
                    {formatInBaseCurrency(portfolioMetrics.totalGain)}
                  </p>
                  <p className={`text-sm ${
                    ...
                    {formatSignedPercentage(portfolioMetrics.totalGainPercent, 2)}
                  </p>
                </>
              )}
            </div>
        ...
        <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl rounded-2xl shadow-lg border border-white/20 dark:border-gray-700/50 p-4">
          <div className="flex items-center justify-between">
            <div className="flex-1">
              <p className="text-sm text-gray-500 dark:text-gray-400">Total Invested ({currencyLabel})</p>
              {isLoading ? (
                <Skeleton variant="text" className="w-24 h-8" />
              ) : (
                <p className="text-xl font-bold text-gray-900 dark:text-white">
                  {formatInBaseCurrency(portfolioMetrics.totalCost)}
                </p>
              )}
            </div>
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3">
                    <div>
                     ...
                     <p className="text-sm text-gray-500 dark:text-gray-400">
                        {formatDecimalValue(holding.shares, 2)} shares @ {formatInBaseCurrency(holding.averageCost)}
                      </p>
                    </div>
                    {holding.quote && (
                ...
                <div className="grid grid-cols-4 gap-6 text-right">
                  ...
                  <div>
                    ...
                    {isLoading ? (
                      ...
                      <p className="font-semibold text-gray-900 dark:text-white">
                        {formatInBaseCurrency(holding.currentPrice)}
                      </p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    {isLoading ? (
                      ...
                      <p className={`font-semibold ${
                        ...
                        {formatSignedPercentage(holding.dayChangePercent)}
                      </p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    {isLoading ? (
                      ...
                      <p className="font-semibold text-gray-900 dark:text-white">
                        {formatInBaseCurrency(holding.marketValue)}
                      </p>
                    )}
                  </div>
                    ...
                    {isLoading ? (
                      ...
                      <>
                        <p className={`font-semibold ${
                          ...
                          {holding.gain.greaterThanOrEqualTo(0) ? '+' : ''}
                          {formatInBaseCurrency(holding.gain)}
                        </p>
                        <p className={`text-xs ${
                          ...
                          {formatSignedPercentage(holding.gainPercent)}
                        </p>
                      </>
                    )}
                  </div>
