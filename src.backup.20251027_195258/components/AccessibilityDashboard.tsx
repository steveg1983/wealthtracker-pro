...
type ColorCombination = (typeof commonCombinations)[number];

type ContrastEntry = ColorCombination & {
  result: ReturnType<typeof ColorContrastChecker.checkContrast>;
};
...
const AccessibilityDashboard: React.FC = () => {
  ...
  const [colorContrastResults, setColorContrastResults] = React.useState<ContrastEntry[]>([]);

  React.useEffect(() => {
    // Run color contrast audit
    const results = commonCombinations.map<ContrastEntry>((combo) => ({
      ...combo,
  ...
  const renderOverview = () => {
    const overallScorePercent = stats.total === 0
      ? 100
      : (() => {
          const scoreDecimal = new Decimal(1)
            .minus(new Decimal(stats.errors).dividedBy(50))
            .times(100)
            .toDecimalPlaces(0, Decimal.ROUND_HALF_UP);
          if (scoreDecimal.lessThan(0)) return 0;
          if (scoreDecimal.greaterThan(100)) return 100;
          return scoreDecimal.toNumber();
        })();
