...
interface AddInvestmentPayload {
  ticker: string;
  name: string;
  shares: number;
  purchasePrice: number;
  purchaseDate: string;
  assetType: FormData['investmentType'];
  accountId: string;
  currency?: string;
}
...
interface AddInvestmentModalProps {
  ...
  onClose: () => void;
  accountId?: string;
  onAdd?: (investment: AddInvestmentPayload) => void | Promise<void>;
}
...
export default function AddInvestmentModal({ isOpen, onClose, accountId, onAdd }: AddInvestmentModalProps) {
  const { accounts, addTransaction } = useApp();
  const { formatCurrency } = useCurrencyDecimal();
  
  const createInitialFormData = (initialAccountId?: string): FormData => ({
  ...
  const { formData, updateField, handleSubmit, setFormData } = useModalForm<FormData>(
    () => createInitialFormData(accountId),
    {
      onSubmit: (data) => {
        ...
        const unitsDecimal = parseCurrencyDecimal(data.units);
        const priceDecimal = parseCurrencyDecimal(data.pricePerUnit);
        const feesDecimal = parseCurrencyDecimal(data.fees || '0');
        const stampDutyDecimal = parseCurrencyDecimal(data.stampDuty || '0');
        const totalDecimal = unitsDecimal.times(priceDecimal).plus(feesDecimal).plus(stampDutyDecimal);
        const total = toStorageNumber(totalDecimal);
        const account = accounts.find(a => a.id === data.selectedAccountId);
        
        // Create the investment transaction
        const description = `${data.investmentType === 'share' ? 'Buy' : 'Investment'}: ${data.name}${data.stockCode ? ` (${data.stockCode})` : ''} - ${data.units} units @ ${formatCurrency(priceDecimal, account?.currency || 'GBP')}/unit`;
        ...
        addTransaction({
          ...
          investmentData: {
            ...
            quantity: toStorageNumber(unitsDecimal),
            pricePerShare: toStorageNumber(priceDecimal),
            transactionFee: toStorageNumber(feesDecimal),
            stampDuty: toStorageNumber(stampDutyDecimal),
            totalCost: total
          }
        });
        ...
        if (onAdd) {
          const basePayload = {
            ...
            name: data.name,
            shares: toStorageNumber(unitsDecimal),
            purchasePrice: toStorageNumber(priceDecimal),
            purchaseDate: data.date,
            assetType: data.investmentType,
            accountId: data.selectedAccountId
          };

          const payload = account?.currency
            ? { ...basePayload, currency: account.currency }
            : basePayload;
  ...
  const calculateTotal = () => {
    const unitsDecimal = parseCurrencyDecimal(formData.units || '0');
    const priceDecimal = parseCurrencyDecimal(formData.pricePerUnit || '0');
    const feesDecimal = parseCurrencyDecimal(formData.fees || '0');
    const stampDutyDecimal = parseCurrencyDecimal(formData.stampDuty || '0');
    return toStorageNumber(unitsDecimal.times(priceDecimal).plus(feesDecimal).plus(stampDutyDecimal));
  };
  ...
  const feesDecimal = parseCurrencyDecimal(formData.fees || '0');
  const stampDutyDecimal = parseCurrencyDecimal(formData.stampDuty || '0');
  const shareCostDecimal = unitsDecimal.times(priceDecimal);
            ...
            {(formData.units && formData.pricePerUnit) && (
              <div className="mt-2 text-sm text-gray-500 dark:text-gray-400 space-y-1">
                <div className="flex justify-between">
                  ...
                  <span>{formatCurrency(shareCostDecimal, selectedAccount?.currency || 'GBP')}</span>
                </div>
                {feesDecimal.greaterThan(0) && (
                  <div className="flex justify-between">
                    <span>Transaction Fee:</span>
                    <span>{formatCurrency(feesDecimal, selectedAccount?.currency || 'GBP')}</span>
                  </div>
                )}
                {stampDutyDecimal.greaterThan(0) && (
                  <div className="flex justify-between">
                    <span>Stamp Duty:</span>
                    <span>{formatCurrency(stampDutyDecimal, selectedAccount?.currency || 'GBP')}</span>
                  </div>
                )}
              </div>
            )}
