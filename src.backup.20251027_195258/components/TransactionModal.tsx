...
type FieldName = 'date' | 'description' | 'amount' | 'category' | 'accountId';
type FieldValue = string | string[] | boolean | Date;
...
export default function TransactionModal({ isOpen, onClose, transaction }: TransactionModalProps): React.JSX.Element | null {
  ...
  const formatDateValue = (value: Date | string): string => {
  ...
  useEffect(() => {
    ...
    setErrors({});
    setTouched({});
  }, [transaction, accounts, defaultAccountId]);

  // Validation functions
  const validateField = (name: FieldName, value: FieldValue): string | undefined => {
    switch (name) {
      case 'date':
      ...
      case 'description':
        ...
        break;
      case 'amount': {
        if (typeof value !== 'string' || value.trim() === '') return 'Amount is required';
        const amountDecimal = parseCurrencyDecimal(value);
        if (amountDecimal.lessThanOrEqualTo(0)) return 'Amount must be a positive number';
        break;
      }
      case 'category':
        if (typeof value !== 'string' || value.trim() === '') return 'Category is required';
        break;
      case 'accountId':
        if (typeof value !== 'string' || value.trim() === '') return 'Account is required';
        break;
    }
    return undefined;
  };
  ...
  const validateForm = (): FormErrors => {
    ...
    const accountError = validateField('accountId', formData.accountId);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (Object.keys(validation).length > 0) {
      ...
      const errorMessages = Object.values(validation).filter(Boolean).join('. ');
    ...
    const transactionData = {
      ...
      amount: toStorageNumber(parseCurrencyDecimal(formData.amount)),
    };
        ...
        <form onSubmit={handleSubmit} className="space-y-4" noValidate>
          ...
          <div>
            ...
            <select
              ...
              aria-required="true"
              aria-invalid={touched.accountId && !!errors.accountId}
              aria-describedby={touched.accountId && errors.accountId ? 'account-error' : undefined}
              className={`w-full px-3 py-2 bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm border ${
