...
interface RolloverSettings {
  ...
  maxAmount?: number | undefined; // Optional cap
...
export default function BudgetRollover() {
    ...
    return budgets.map(budget => {
      ...
      if (rolloverSettings.enabled && !rolloverSettings.excludeCategories.includes(budget.categoryId)) {
        if (remaining.greaterThan(0) || (remaining.lessThan(0) && rolloverSettings.carryNegative)) {
          ...
          if (rolloverSettings.maxAmount !== undefined) {
            const maxDecimal = toDecimal(rolloverSettings.maxAmount);
            if (rolloverAmount.greaterThan(maxDecimal)) {
              rolloverAmount = maxDecimal;
            }
          }
        }
      }
  ...
  const applyRollover = () => {
    const rollovers = rolloverData
      .filter((data): data is NonNullable<typeof data> => Boolean(data) && data.willRollover)
      .map(data => {
