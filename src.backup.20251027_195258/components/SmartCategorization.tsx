  ...
  if (showInline) {
    return (
      <div className="flex items-center gap-2 py-1">
        <StarIcon size={14} className="text-purple-500 flex-shrink-0" />
        ...
        <button
          ...
          `}
        title={`${topSuggestion.reason} (${formatPercentageValue(topSuggestion.confidence, 0)} confidence)`}
        >
          <span
          ...
          <span className="font-semibold">
            {formatPercentageValue(topSuggestion.confidence, 0)}
          </span>
                ...
                >
                  ...
                  >
                    ...
                    <span className={`text-xs px-1.5 py-0.5 rounded-full ${getConfidenceColor(suggestion.confidence)}`}>
                      {formatPercentageValue(suggestion.confidence, 0)}
                    </span>
                  </button>
                  <button
    ...
    <div className="bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-lg p-4">
      ...
      <div className="space-y-2">
        {suggestions.map((suggestion, index) => {
          ...
          const confidencePercent = formatPercentageValue(suggestion.confidence, 0);
      ...
      batch.forEach(transaction => {
        if (!transaction.category) {
          ...
          if (transSuggestions.length > 0) {
            ...
            const [topSuggestion] = transSuggestions;
            if (topSuggestion && topSuggestion.confidence >= 0.8) {
              setSelected(prev => new Map(prev).set(transaction.id, topSuggestion.categoryId));
            }
          }
        }
      });
  ...
  const selectAllHighConfidence = () => {
    ...
    suggestions.forEach((transSuggestions, transactionId) => {
      ...
      if (topSuggestion && topSuggestion.confidence >= 0.7) {
        newSelected.set(transactionId, topSuggestion.categoryId);
      }
    });
        ...
        <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <StarIcon size={24} className="text-purple-600 dark:text-purple-400" />
              <div>
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className="text-xs font-semibold text-gray-500 dark:text-gray-400">
                              {formatPercentageValue(toDecimal(suggestion.confidence).times(100), 0)}
                            </span>
