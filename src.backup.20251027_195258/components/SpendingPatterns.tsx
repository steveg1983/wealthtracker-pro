...
type PatternType = SpendingPattern['patternType'] | 'all';

export default function SpendingPatterns({ onDataChange }: SpendingPatternsProps): React.JSX.Element {
  const [patterns, setPatterns] = useState<SpendingPattern[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [selectedType, setSelectedType] = useState<PatternType>('all');
  ...
  const patternTypes: PatternType[] = ['all', 'recurring', 'seasonal', 'trend', 'anomaly'];
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-sm">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {(() => {
            ...
            const activeDecimal = toDecimal(activeCount);

            return patternTypes.slice(1).map(type => {
              const typeCount = patterns.filter(p => p.patternType === type && p.isActive).length;
              const countDecimal = toDecimal(typeCount);
              const percentageDecimal = activeDecimal.gt(0)
                ? countDecimal.dividedBy(activeDecimal).times(100)
                : toDecimal(0);

              return (
                <div key={type} className="text-center">
                  <div className="flex items-center justify-center mb-2">
                    {type !== 'all' && getPatternTypeIcon(type)}
                  </div>
                  <div className="text-2xl font-bold text-gray-900 dark:text-white">{countDecimal.toNumber()}</div>
                  <div className="text-sm text-gray-600 dark:text-gray-400 capitalize">{type}</div>
                  <div className="text-xs text-gray-500 dark:text-gray-400">{formatPercentageValue(percentageDecimal, 1)}</div>
                </div>
              );
            });
          })()}
      ...
      <div className="flex flex-wrap items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            value={sortBy}
            onChange={(e) => setSortBy(e.target.value as 'confidence' | 'amount' | 'detectedAt' | 'frequency')}
            className="px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm"
          >
                  ...
                  <tr key={pattern.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${getConfidenceBadge(pattern.confidence)}`}>
                        ...
                        {formatPercentageValue(toDecimal(pattern.confidence).times(100), 0)}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
