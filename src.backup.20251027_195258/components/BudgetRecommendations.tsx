...
import { budgetRecommendationService, type BudgetAnalysis, type BudgetRecommendation, type RecommendationConfig } from '../services/budgetRecommendationService';
import {
  TrendingUpIcon,
  TrendingDownIcon,
  ArrowRightIcon,
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => updateConfig('aggressiveness', e.target.value as RecommendationConfig['aggressiveness'])}
                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700"
              >
      ...
      {analysis.recommendations.length > 0 && (
        <div className="space-y-3">
          ...
          {analysis.recommendations.map(recommendation => {
            const confidenceDecimal = new Decimal(recommendation.confidence ?? 0).times(100);
            const confidenceValue = confidenceDecimal.toDecimalPlaces(0, Decimal.ROUND_HALF_UP).toNumber();
            const confidenceDisplay = formatPercentageFromRatio(recommendation.confidence ?? 0, 0);
            const confidenceColor = confidenceValue >= 80
              ? 'text-green-600 dark:text-green-400'
              : confidenceValue >= 60
                ? 'text-yellow-600 dark:text-yellow-400'
                : 'text-gray-600 dark:text-gray-400';
            const trendDisplay = formatPercentageFromRatio((recommendation.trendPercentage ?? 0) / 100, 0);
                  ...
                  <div className="mt-3 space-y-2">
                    <div className="flex items-center gap-4 text-sm text-gray-600 dark:text-gray-400">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span>
                          {recommendation.spendingTrend} {trendDisplay}
                        </span>
                      </div>
                      <div className="flex items-center gap-1">
