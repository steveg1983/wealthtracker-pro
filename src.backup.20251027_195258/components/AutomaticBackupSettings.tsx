...
const BACKUP_HISTORY_STORAGE_KEY = 'money_management_backup_history';
const BACKUP_HISTORY_LIMIT = 10;

const isValidFormat = (value: unknown): value is BackupConfig['format'] =>
  value === 'json' || value === 'csv' || value === 'all';

const readBackupHistory = (): BackupHistoryEntry[] => {
  if (typeof window === 'undefined') {
    return [];
  }

  try {
    const stored = window.localStorage.getItem(BACKUP_HISTORY_STORAGE_KEY);
    if (!stored) {
      return [];
    }
    ...
    return parsed.reduce<BackupHistoryEntry[]>((acc, entry) => {
      const timestampValue = typeof entry?.timestamp === 'number'
        ? entry.timestamp
        : new Date(entry?.timestamp ?? Date.now()).valueOf();

      const normalised: BackupHistoryEntry = {
        success: Boolean(entry?.success),
        timestamp: timestampValue,
      };

      if (isValidFormat(entry?.format)) {
        normalised.format = entry.format;
      }
      if (typeof entry?.filesCreated === 'number') {
        normalised.filesCreated = entry.filesCreated;
      }
      if (typeof entry?.size === 'number') {
        normalised.size = entry.size;
      }
      if (typeof entry?.error === 'string') {
        normalised.error = entry.error;
      }
...
const persistBackupHistory = (history: BackupHistoryEntry[]): void => {
  if (typeof window === 'undefined') {
    return;
  }

  try {
    window.localStorage.setItem(BACKUP_HISTORY_STORAGE_KEY, JSON.stringify(history));
  } catch (error) {
    logger.error('Failed to persist backup history:', error);
  }
};

const normaliseStoredBackups = (backups: Array<{
  id: number;
  filename: string;
  format: string;
  timestamp: number;
  size: number;
  cloudSynced: boolean;
}>): StoredBackup[] => backups.map((backup) => ({
  id: backup.id,
...
export default function AutomaticBackupSettings() {
  ...
  const loadBackupData = useCallback(async (): Promise<void> => {
    try {
      ...
      if (history.length > BACKUP_HISTORY_LIMIT) {
        persistBackupHistory(history.slice(0, BACKUP_HISTORY_LIMIT));
      }

      const stored = await automaticBackupService.getStoredBackups();
      setStoredBackups(normaliseStoredBackups(stored));
    } catch (error) {
  ...
  const formatFileSize = (bytes: number): string => {
    const sizeDecimal = new Decimal(bytes ?? 0);
    const kilobyte = new Decimal(1024);
    const megabyte = kilobyte.times(1024);

    if (sizeDecimal.lessThan(kilobyte)) {
      return `${sizeDecimal.toDecimalPlaces(0, Decimal.ROUND_HALF_UP).toNumber()} B`;
    }

    if (sizeDecimal.lessThan(megabyte)) {
      const kilobytes = sizeDecimal.dividedBy(kilobyte).toDecimalPlaces(1, Decimal.ROUND_HALF_UP);
      return `${kilobytes.toString()} KB`;
    }

    const megabytes = sizeDecimal.dividedBy(megabyte).toDecimalPlaces(1, Decimal.ROUND_HALF_UP);
