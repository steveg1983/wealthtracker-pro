    ...
    <>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-2">
        {!isModal ? (
          <div className="flex items-center gap-2">
            ...
            {onOpenModal && (
              ...
              >
                <ProfessionalIcon name="maximize" size={18} className="text-gray-500 dark:text-gray-400" />
              </button>
            )}
          </div>
        ) : null}
        <div className={`flex items-center gap-3 ${isModal ? 'w-full justify-between' : ''}`}>
          ...
          >
            <ProfessionalIcon name="settings" size={18} className="text-gray-500 dark:text-gray-400" />
          </button>
        </div>
      </div>
              ...
              >
                ...
                {categoryData.category.isHeader && !categoryData.showSubtotal ? (
                  ...
                  categoryData.monthlyData.map((monthData: { month: string; income: number; expenditure: number; net: number }, monthIndex: number) => {
                    ...
                    if (!month) {
                      return (
                        <td key={`${monthData.month}-${monthIndex}`} className="py-2.5 px-1 text-center">
                    ...
                    return (
                      <td 
                        ...
                        onClick={() => isClickable && monthData && handleCellClick(categoryData, monthData, month)}
                      >
                        {monthData.income > 0 && (
