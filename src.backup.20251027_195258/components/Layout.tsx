...
export default function Layout(): React.JSX.Element {
  ...
  // Default values for features not in preferences context yet
  ...
  const getNextPrevPage = (direction: 'next' | 'prev', currentPath: string): string | null => {
    ...
    if (direction === 'next') {
      return currentIndex < pages.length - 1 ? pages[currentIndex + 1] || null : null;
    } else {
      return currentIndex > 0 ? pages[currentIndex - 1] || null : null;
    }
  };
  ...
  useEffect(() => {
    ...
    const handleOpenConflictResolver = (event: Event) => {
      ...
      if (parsedConflict) {
        ...
        logger.warn('Layout: Ignoring conflict resolver event with invalid payload');
      } else {
        showConflictModal();
      }
    };
  ...
  useEffect(() => {
    const handler = async (event: Event) => {
      ...
      const sanitizedSignals = detail.signals
        ? Object.fromEntries(
      ...
      try {
        ...
        if (detail.title) {
          explainRequest.title = detail.title;
        }
        if (sanitizedSignals) {
          explainRequest.signals = sanitizedSignals;
        }

        const res = await aiService.explainPage(explainRequest);
  ...
  return (
    <div className="flex min-h-screen bg-tertiary dark:bg-gray-900">
      ...
      <div className="sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 z-50">
        <a 
          href="#main-content" 
          className="inline-block px-4 py-2 bg-gray-600 text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2"
        >
          Skip to main content
        </a>
        <a 
          href="#main-navigation" 
          className="inline-block px-4 py-2 ml-2 bg-gray-600 text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2"
        >
          Skip to navigation
        </a>
      </div>
