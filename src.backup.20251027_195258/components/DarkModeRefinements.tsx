...
import {
  ...
  ClockIcon,
  PaletteIcon,
  SettingsIcon as AdjustmentsIcon,
  SettingsIcon,
  XIcon as CloseIcon
} from './icons';
...
export function DarkModeRefinements({ 
  ...
  const determineTheme = useCallback((): 'light' | 'dark' => {
    ...
    if (settings.mode === 'auto' && settings.autoSchedule?.enabled) {
      ...
      const lightTime = (lightHour || 0) * 60 + (lightMin || 0);
      const darkTime = (darkHour || 0) * 60 + (darkMin || 0);
  ...
  const themeModes: Array<{ value: DarkModeSettings['mode']; icon: React.ReactNode; label: string }> = [
  ...
  const contrastModes: Array<{ value: DarkModeSettings['contrastMode']; label: string; desc: string }> = [
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-md overflow-hidden">
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">
                  <AdjustmentsIcon size={16} className="inline mr-1" />
                  Contrast Level
                </label>
