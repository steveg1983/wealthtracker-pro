...
type BudgetFrequency = 'monthly' | 'weekly' | 'biweekly' | 'quarterly' | 'yearly';
...
export default function RecurringBudgetTemplates() {
  const { categories, budgets, addBudget, deleteBudget } = useApp();
  ...
  const calculateNextDate = (frequency: BudgetFrequency): Date => {
    const now = new Date();
  ...
  const duplicateTemplate = (template: BudgetTemplate) => {
    const { lastApplied, ...templateBase } = template;
    const newTemplate = {
      ...templateBase,
      id: Date.now().toString(),
      ...
      isActive: false,
      createdAt: new Date()
      // lastApplied omitted for new template
    };
    setTemplates([...templates, newTemplate]);
  };

  const getFrequencyText = (frequency: BudgetFrequency) => {
    switch (frequency) {
          ...
          >
            ...
            <div className="mb-4">
              ...
              <div className="space-y-1 max-h-24 overflow-y-auto">
                {template.budgetItems.slice(0, 3).map(item => (
                  <div key={`${template.id}-${item.name}`} className="flex items-center justify-between text-xs">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(event) => {
                    const nextFrequency = event.target.value as BudgetFrequency;
                    setNewTemplate(prev => ({ ...prev, frequency: nextFrequency }));
                  }}
                  className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                >
