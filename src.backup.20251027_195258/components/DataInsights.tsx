...
export default function DataInsights({ onDataChange }: DataInsightsProps) {
  ...
  type InsightSortOption = 'createdAt' | 'severity' | 'category';
  type InsightTypeFilter = 'all' | SpendingInsight['type'];

  const [insights, setInsights] = useState<SpendingInsight[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [filter, setFilter] = useState<SeverityFilter>('all');
  const [typeFilter, setTypeFilter] = useState<InsightTypeFilter>('all');
  const [sortBy, setSortBy] = useState<InsightSortOption>('createdAt');
  ...
  const insightTypes: InsightTypeFilter[] = ['all', 'subscription_alert', 'spending_spike', 'new_merchant', 'category_trend', 'duplicate_transaction'];
      ...
      <div className="flex flex-wrap items-center gap-4">
        <div className="flex items-center gap-2">
          ...
          <select
            value={filter}
            onChange={(e) => {
              const nextFilter = e.target.value as SeverityFilter;
        ...
        <div className="flex items-center gap-2">
          <select
            value={typeFilter}
            onChange={(e) => {
              const nextType = e.target.value as InsightTypeFilter;
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            value={sortBy}
            onChange={(e) => {
              const nextSort = e.target.value as InsightSortOption;
              setSortBy(nextSort);
            }}
            className="px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm"
          >
