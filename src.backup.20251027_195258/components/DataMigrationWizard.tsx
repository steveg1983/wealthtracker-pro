...
const resolveMimeType = (extension: string): string => {
  ...
  if (normalized === '.qif' || normalized === '.qfx') {
...
export default function DataMigrationWizard({
  ...
  const dropzoneAccept = useMemo<Accept | undefined>(() => {
    if (!selectedSource) {
      return undefined;
    }
    const source = MIGRATION_SOURCES.find(s => s.id === selectedSource);
    if (!source) {
      return undefined;
    }
    return source.fileTypes.reduce<Accept>((acc, extension) => {
      const mime = resolveMimeType(extension);
      const current = acc[mime] ?? [];
      acc[mime] = current.includes(extension) ? current : [...current, extension];
      return acc;
    }, {} as Accept);
  }, [selectedSource]);
  ...
  if (dropzoneAccept) {
    dropzoneOptions.accept = dropzoneAccept;
  }
  ...
  const formatFileSize = (bytes: number): string => {
    ...
    if (sizeDecimal.lessThan(megabyte)) {
      const kilobytes = sizeDecimal.dividedBy(kilobyte).toDecimalPlaces(2, Decimal.ROUND_HALF_UP);
      return `${kilobytes.toString()} KB`;
    }

    const megabytes = sizeDecimal.dividedBy(megabyte).toDecimalPlaces(2, Decimal.ROUND_HALF_UP);
