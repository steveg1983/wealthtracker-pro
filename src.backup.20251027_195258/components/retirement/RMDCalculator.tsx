...
const getDistributionFactor = (age: number, spouseBeneficiary: boolean, spouseAge: number | null): number => {
  // If spouse is sole beneficiary and more than 10 years younger, use Joint Life Table
  if (spouseBeneficiary && spouseAge != null && (age - spouseAge) > 10) {
      // Simplified calculation - in practice would use full Joint Life Table
      const ageDiff = age - spouseAge;
      return (UNIFORM_LIFETIME_TABLE[age] || 27.4) + (ageDiff - 10) * 0.5; // Approximate
    }
    
    // Use Uniform Lifetime Table for most cases
  if (age >= 120) return UNIFORM_LIFETIME_TABLE[120] || 27.4;
  ...
  useEffect(() => {
    ...
    const balanceDecimal = toDecimal(calculation.accountBalance);

    // Calculate RMD amount
    const rmdAmountDecimal = balanceDecimal.dividedBy(factorDecimal);
    const rmdAmount = rmdAmountDecimal.toDecimalPlaces(2, Decimal.ROUND_HALF_UP).toNumber();

    // Calculate penalty (25% of RMD not taken, reduced to 10% if corrected within 2 years)
    const penalty = rmdAmountDecimal
      .times(0.25)
      .toDecimalPlaces(2, Decimal.ROUND_HALF_UP)
      .toNumber();

    // Estimate taxes (assuming 22% federal bracket for demonstration)
    const taxEstimate = rmdAmountDecimal
      .times(0.22)
      .toDecimalPlaces(2, Decimal.ROUND_HALF_UP)
      .toNumber();
    ...
    let projectedBalanceDecimal = balanceDecimal;
    const growthRateDecimal = toDecimal(1.05); // Assume 5% growth
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (projectedAge >= rmdAge) {
        const projectedFactor = getDistributionFactor(
          projectedAge,
          calculation.spouseBeneficiary,
          calculation.spouseAge != null ? calculation.spouseAge + i : null
        );
        const projectedFactorDecimal = toDecimal(projectedFactor);
        const projectedRMDDecimal = projectedBalanceDecimal.dividedBy(projectedFactorDecimal);

        projections.push({
          age: projectedAge,
          balance: projectedBalanceDecimal.toDecimalPlaces(2, Decimal.ROUND_HALF_UP).toNumber(),
          rmd: projectedRMDDecimal.toDecimalPlaces(2, Decimal.ROUND_HALF_UP).toNumber()
        });

        // Update balance for next year (growth minus RMD)
        projectedBalanceDecimal = projectedBalanceDecimal.minus(projectedRMDDecimal).times(growthRateDecimal);
      }
    }
    ...
    return (
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center gap-2 text-amber-600 mb-4">
          <ProfessionalIcon name="warning" className="h-5 w-5" size={20} />
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          {calculation.age < results.rmdAge ? (
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              <div className="flex items-start gap-2">
                <ProfessionalIcon name="info" className="h-5 w-5 text-green-600 mt-0.5" size={20} />
                <div>
            ...
            <>
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-4">
                <div className="flex items-center gap-2 mb-3">
                  <ProfessionalIcon name="calculator" className="h-5 w-5 text-gray-600" size={20} />
                ...
                <div className="space-y-2 text-sm text-gray-600">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {toDecimal(results.distributionFactor ?? 0)
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {formatCurrency(calculation.accountBalance)} ÷ {toDecimal(results.distributionFactor ?? 0)
                        .toDecimalPlaces(1, Decimal.ROUND_HALF_UP)
                        .toString()}
                    </span>
                  </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-amber-50 rounded-lg p-3">
                  <div className="flex items-center gap-2 mb-1">
                    <ProfessionalIcon name="calendar" className="h-4 w-4 text-amber-600" size={16} />
                ...
                <div className="bg-red-50 rounded-lg p-3">
                  <div className="flex items-center gap-2 mb-1">
                    <ProfessionalIcon name="warning" className="h-4 w-4 text-red-600" size={16} />
              ...
              <div className="bg-gray-50 rounded-lg p-3">
                <div className="flex items-center gap-2 mb-1">
                  <ProfessionalIcon name="dollar" className="h-4 w-4 text-gray-600" size={16} />
      ...
      {results.projections.length > 0 && (
        <div className="mt-6">
          <h4 className="font-semibold text-gray-900 mb-3 flex items-center gap-2">
            <ProfessionalIcon name="trendingDown" className="h-5 w-5 text-gray-600" size={20} />
                    ...
                    <td className="text-right py-2 px-3">
                      {toDecimal(
                        getDistributionFactor(
                          projection.age,
                          calculation.spouseBeneficiary,
                          calculation.spouseAge != null ? calculation.spouseAge + index : null
                        ) ?? 0
                      )
                        .toDecimalPlaces(1, Decimal.ROUND_HALF_UP)
                        .toString()}
      ...
      <div className="mt-6 p-4 bg-amber-50 border border-amber-200 rounded-lg">
        <div className="flex items-start gap-2">
          <ProfessionalIcon name="warning" className="h-5 w-5 text-amber-600 mt-0.5" size={20} />
