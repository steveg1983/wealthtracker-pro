...
export default function NIYearsTracker(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    const qualifiesForPension = totalExpectedYears >= STATE_PENSION_2024.minYearsForAny;

    let weeklyPensionDecimal = toDecimal(0);

    if (qualifiesForPension) {
      const baseYearsDecimal = toDecimal(Math.min(totalExpectedYears, STATE_PENSION_2024.fullYears));
      const contractedReduction = toDecimal(contractedOutYears).times(0.2);
      const adjustedYearsDecimal = Decimal.max(
        baseYearsDecimal.minus(contractedReduction),
        toDecimal(0),
      );

      weeklyPensionDecimal = toDecimal(STATE_PENSION_2024.fullWeekly)
        .dividedBy(toDecimal(STATE_PENSION_2024.fullYears))
        .times(adjustedYearsDecimal);
    }

    const annualPensionDecimal = weeklyPensionDecimal.times(52);
    const percentageOfFullDecimal = qualifiesForPension && !weeklyPensionDecimal.isZero()
      ? weeklyPensionDecimal.dividedBy(toDecimal(STATE_PENSION_2024.fullWeekly)).times(100)
      : toDecimal(0);
    ...
    const canBuyYears = trackerData.years
      .filter(y => {
        const yearNum = parseInt(y.year.split('-')[0] || '0');
        return y.status === 'gap' && yearNum >= sixYearsAgo && yearNum <= currentYear;
      })
      .map(y => y.year);
    
    const costToBuyGapsDecimal = toDecimal(canBuyYears.length).times(
      STATE_PENSION_2024.voluntaryContributionCost,
    );
    ...
    const newWeeklyPensionDecimal = toDecimal(STATE_PENSION_2024.fullWeekly)
      .dividedBy(toDecimal(STATE_PENSION_2024.fullYears))
      .times(toDecimal(newTotalYears));
    const additionalWeeklyDecimal = Decimal.max(
      newWeeklyPensionDecimal.minus(weeklyPensionDecimal),
      toDecimal(0),
    );
    const additionalAnnualDecimal = additionalWeeklyDecimal.times(52);
    ...
    setResults({
      ...
      totalExpectedYears,
      weeklyPension: weeklyPensionDecimal.toDecimalPlaces(2, Decimal.ROUND_HALF_UP).toNumber(),
      annualPension: annualPensionDecimal.toDecimalPlaces(2, Decimal.ROUND_HALF_UP).toNumber(),
      percentageOfFull: percentageOfFullDecimal.toDecimalPlaces(1, Decimal.ROUND_HALF_UP).toNumber(),
      canBuyYears,
      costToBuyGaps: costToBuyGapsDecimal.toDecimalPlaces(2, Decimal.ROUND_HALF_UP).toNumber(),
      additionalPensionFromBuying: additionalAnnualDecimal
    ...
    return (
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center gap-2 text-amber-600 mb-4">
          <ProfessionalIcon name="error" className="h-5 w-5" size={20} />
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <div className="flex items-start gap-2">
              <ProfessionalIcon name="info" className="h-5 w-5 text-gray-600 mt-0.5" size={20} />
        ...
        <div className="space-y-4">
          <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg p-4">
            <div className="flex items-center gap-2 mb-3">
              <ProfessionalIcon name="success" className="h-5 w-5 text-green-600" size={20} />
          ...
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-4">
            <div className="flex items-center gap-2 mb-3">
              <ProfessionalIcon name="pound" className="h-5 w-5 text-gray-600" size={20} />
              ...
              <>
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{formatPercentageValue(results.percentageOfFull ?? 0, 1)}</span>
                  </div>
                </div>
          ...
          {results.canBuyYears.length > 0 && (
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
              <div className="flex items-start gap-2">
                <ProfessionalIcon name="trendingUp" className="h-5 w-5 text-amber-600 mt-0.5" size={20} />
                  ...
                  <div className="text-sm text-amber-800 space-y-2">
                    ...
                    <div className="mt-3 pt-3 border-t border-amber-300">
                      ...
                      <p className="text-xs mt-2 text-amber-700">
                        Payback period: {toDecimal(results.costToBuyGaps ?? 0)
                          .dividedBy(toDecimal(results.additionalPensionFromBuying || 1))
      ...
      <div className="mt-6">
        <h4 className="font-semibold text-gray-900 mb-3 flex items-center gap-2">
          <ProfessionalIcon name="calendar" className="h-5 w-5 text-gray-600" size={20} />
                ...
                <tr key={index} className="border-b hover:bg-gray-50">
                  ...
                  <td className="py-2 px-3">
                    <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${
                      ...
                      {year.status === 'paid' && <ProfessionalIcon name="success" className="h-3 w-3" size={12} />}
                      {year.status === 'gap' && <ProfessionalIcon name="close2" className="h-3 w-3" size={12} />}
