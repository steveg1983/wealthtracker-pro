...
export function PredictiveLoadingIndicator({
  ...
  const oneDecimal = toDecimal(1);
  const progressRatioDecimal = toDecimal(preloadProgress);
  const clampedProgressDecimal = progressRatioDecimal.lessThan(zeroDecimal)
    ? zeroDecimal
    : progressRatioDecimal.greaterThan(oneDecimal)
      ? oneDecimal
      : progressRatioDecimal;
  const progressDisplay = formatPercentageFromRatio(clampedProgressDecimal, 0);
  const progressWidthPercent = clampedProgressDecimal.times(100).toNumber();

  const totalPrefetchDecimal = toDecimal(metrics.totalPrefetches ?? 0);
  const successRateRatio = totalPrefetchDecimal.greaterThan(0)
    ? toDecimal(metrics.successfulPrefetches ?? 0).dividedBy(totalPrefetchDecimal)
    : oneDecimal;
  const successRateDisplay = formatPercentageFromRatio(successRateRatio, 0);
...
function formatBytes(bytes: number): string {
  ...
  const value = toDecimal(bytes).dividedBy(Math.pow(k, i)).toDecimalPlaces(1);
  return `${value.toNumber()} ${sizes[i]}`;
}
