...
import { toDecimal, formatPercentageFromRatio, formatPercentageValue } from '@wealthtracker/utils';
...
export default function AdvancedAnalytics() {
  ...
  const getImpactIcon = (impact: 'positive' | 'negative' | 'neutral') => {
    switch (impact) {
      case 'positive':
        return <ProfessionalIcon name="success" size={20} className="text-green-600 dark:text-green-400" />;
      case 'negative':
        return <ProfessionalIcon name="error" size={20} className="text-red-600 dark:text-red-400" />;
      case 'neutral':
        return <ProfessionalIcon name="info" size={20} className="text-gray-600 dark:text-gray-500" />;
    }
  };

  const tabs: Array<{ id: TabType; label: string; iconName: ProfessionalIconName; count: number }> = [
    { id: 'insights', label: 'Insights', iconName: 'lightbulb', count: insights.length },
    { id: 'anomalies', label: 'Anomalies', iconName: 'warning', count: anomalies.length },
    { id: 'predictions', label: 'Predictions', iconName: 'trendingUp', count: predictions.length },
    { id: 'opportunities', label: 'Opportunities', iconName: 'piggyBank', count: opportunities.length }
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-2 bg-gray-100 dark:bg-gray-800 p-1 rounded-lg overflow-x-auto">
        {tabs.map(tab => (
          ...
          >
            <ProfessionalIcon name={tab.iconName} size={16} />
            <span>{tab.label}</span>
            {tab.count > 0 && (
          ...
          <div className="space-y-4">
            {insights.length > 0 ? (
              ...
              <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                <ProfessionalIcon name="lightbulb" size={48} className="mx-auto mb-3 opacity-50" />
        ...
        {activeTab === 'anomalies' && (
          <div className="space-y-4">
            {anomalies.length > 0 ? (
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-4 mt-2 text-sm">
                        ...
                        <span className="text-red-600 dark:text-red-400">
                          {formatPercentageValue(anomaly.percentageAboveNormal, 0)} above normal
                        </span>
                      </div>
                    </div>
              ...
              <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                <ProfessionalIcon name="warning" size={48} className="mx-auto mb-3 opacity-50" />
            ...
            {predictions.length > 0 ? (
              <>
                ...
                {predictions.map(prediction => {
                  const confidencePercent = toDecimal(prediction.confidence).times(100);
                  const boundedConfidencePercent = confidencePercent.lessThan(0)
                    ? toDecimal(0)
                    : confidencePercent.greaterThan(100)
                    ? toDecimal(100)
                    : confidencePercent;
                  const confidenceProgress = boundedConfidencePercent.toDecimalPlaces(0).toNumber();
                  const confidenceDisplay = formatPercentageFromRatio(prediction.confidence, 0);
                      ...
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <div className="flex items-center gap-2">
                          {prediction.trend === 'increasing' ? (
                            <ProfessionalIcon name="arrowUp" size={16} className="text-red-600 dark:text-red-400" />
                          ) : prediction.trend === 'decreasing' ? (
                            <ProfessionalIcon name="arrowDown" size={16} className="text-green-600 dark:text-green-400" />
                          ) : (
              ...
              <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                <ProfessionalIcon name="trendingUp" size={48} className="mx-auto mb-3 opacity-50" />
        ...
        {activeTab === 'opportunities' && (
          <div className="space-y-4">
            {opportunities.length > 0 ? (
              ...
              <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                <ProfessionalIcon name="piggyBank" size={48} className="mx-auto mb-3 opacity-50" />
