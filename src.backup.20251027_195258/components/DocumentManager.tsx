...
export default function DocumentManager({
  ...
  useEffect(() => {
    loadDocuments();
    loadStorageStats();
  }, [transactionId, accountId, loadDocuments, loadStorageStats]);
  ...
  const formatFileSize = (bytes: number): string => {
    ...
    if (size.lessThan(1024)) {
      return `${size.toDecimalPlaces(0, Decimal.ROUND_HALF_UP).toNumber()} B`;
    }
                  ...
                  <div>
                    ...
                    <dl className="space-y-1 text-sm">
                      ...
                      <div>
                        ...
                        <dd className="font-medium text-gray-900 dark:text-white">
                          {new Decimal(selectedDocument.extractedData.confidence ?? 0)
                            .times(100)
      ...
      {editingDocument && (
        ...
        >
          ...
          <ModalFooter>
            ...
            <button
              onClick={() => {
                const documentToUpdate = editingDocument;
                if (!documentToUpdate) {
                  return;
                }
                ...
                if (documentToUpdate.notes !== undefined) {
                  updates.notes = documentToUpdate.notes;
                }
                handleUpdateDocument(documentToUpdate, updates);
              }}
              className="px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700"
            >
              Save Changes
