...
export default function ZeroBasedBudgeting() {
  ...
  const loadPeriods = () => {
    ...
    if (stored) {
      const parsed: SavedZeroBudgetPeriod[] = JSON.parse(stored);
      const mappedPeriods = parsed.map((p): ZeroBudgetPeriod => {
        const items = p.items.map((item): ZeroBudgetItem => {
          const mappedItem: ZeroBudgetItem = {
            id: item.id,
          ...
          if (item.subcategory) {
            mappedItem.subcategory = item.subcategory;
          }
          if (item.notes) {
            mappedItem.notes = item.notes;
          }
          if (item.startDate) {
            mappedItem.startDate = new Date(item.startDate);
          }
          if (item.endDate) {
            mappedItem.endDate = new Date(item.endDate);
          }
        ...
        return {
          ...
          items,
          status: p.status as ZeroBudgetPeriod['status'],
      ...
      // Set active period
      const active = mappedPeriods.find((p) => p.status === 'active');
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    {activePeriod.totalIncome === 0
                      ? '0.0%'
                      : `${formatDecimalFixed((totals.allocated / activePeriod.totalIncome) * 100, 1)}%`}
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    {activePeriod.totalIncome === 0
                      ? '0.0%'
                      : `${formatDecimalFixed((totals.approved / activePeriod.totalIncome) * 100, 1)}%`}
                  </p>
                </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setFormData({ ...formData, totalIncome: parseCurrencyDecimal(e.target.value || '0').toNumber() })}
                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700"
                placeholder="0.00"
                required
              />
...
function BudgetItemModal({ item, categories, onSave, onClose }: BudgetItemModalProps) {
  const frequencyOptions: ZeroBudgetItem['frequency'][] = ['once', 'weekly', 'monthly', 'quarterly', 'annual'];
  const priorityOptions: ZeroBudgetItem['priority'][] = ['essential', 'important', 'nice-to-have'];

  const isFrequency = (value: string): value is ZeroBudgetItem['frequency'] =>
    frequencyOptions.some(option => option === value);

  const isPriority = (value: string): value is ZeroBudgetItem['priority'] =>
    priorityOptions.some(option => option === value);
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData, amount: parseCurrencyDecimal(e.target.value || '0').toNumber() })}
                  className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700"
                  placeholder="0.00"
                  required
                />
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (isFrequency(nextFrequency)) {
                      setFormData({ ...formData, frequency: nextFrequency });
                    }
                  }}
                  className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700"
                >
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (isPriority(nextPriority)) {
                      setFormData({ ...formData, priority: nextPriority });
                    }
                  }}
                  className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700"
                >
