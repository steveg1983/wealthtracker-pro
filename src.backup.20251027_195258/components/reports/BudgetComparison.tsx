...
import { formatPercentageValue, formatSignedPercentageValue, toDecimal, type DecimalInstance } from '@wealthtracker/utils';
  ...
  const categoryComparisons = useMemo(() => {
    ...
    const transactionsByCategory = transactions
      ...
      .reduce((acc, t) => {
        ...
        acc[categoryId].total = acc[categoryId].total.plus(toDecimal(t.amount));
        acc[categoryId].transactions.push(t);
        return acc;
      }, {} as Record<string, { total: DecimalInstance; transactions: Transaction[] }>);
    ...
    activeBudgets.forEach(budget => {
      ...
      let budgetedAmount = toDecimal(budget.amount || 0);
      ...
      const actualSpending = transactionsByCategory[categoryId]?.total || toDecimal(0);
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            <h2 className="text-xl font-semibold text-gray-900 dark:text-white flex items-center gap-2">
              <ProfessionalIcon name="chartBar" size={24} />
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <div className="flex items-center gap-2">
              <ProfessionalIcon name="calendar" size={20} className="text-gray-500" />
              <select
            ...
            {/* Sort By */}
            <div className="flex items-center gap-2">
              <ProfessionalIcon name="settingsAlt" size={20} className="text-gray-500" />
              <select
                value={sortBy}
                onChange={(e) => setSortBy(e.target.value as 'name' | 'variance' | 'percent')}
                className="px-3 py-2 bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm border border-gray-300/50 dark:border-gray-600/50 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent dark:text-white text-sm"
              >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl rounded-2xl shadow-lg border border-white/20 dark:border-gray-700/50 p-4">
          <div className="flex items-center justify-between">
            ...
            <ProfessionalIcon
              name={totals.actual > totals.budgeted ? 'trendingUp' : 'trendingDown'}
              size={32}
              className={totals.actual > totals.budgeted ? 'text-red-500' : 'text-green-500'}
            />
          </div>
        </div>
        ...
        <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl rounded-2xl shadow-lg border border-white/20 dark:border-gray-700/50 p-4">
          <div className="flex items-center justify-between">
            ...
            <span className={`text-sm font-medium px-2 py-1 rounded-full ${
              ...
              {formatPercentageValue(Math.abs(totals.variancePercent), 1)}
            </span>
          </div>
        </div>
                  ...
                  <div className="relative">
                    ...
                    <div className="flex justify-between mt-1">
                      ...
                      <span className={`text-xs font-medium ${getStatusColor(category.status)}`}>
                        {formatPercentageValue(spentPercent, 1)}
                      </span>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        <p className={`font-medium ${getStatusColor(category.status)}`}>
                          {formatSignedPercentageValue(category.variancePercent, 1)}
                        </p>
                      </div>
                    </div>
