...
export function SimplifiedReconciliation({ account, onClose }: SimplifiedReconciliationProps): React.JSX.Element {
  ...
  const currentBalance = useMemo(() => {
    const isLiabilityAccount = account.type === 'loan' || account.type === 'credit' || account.type === 'mortgage';
  ...
  const autoMatch = useCallback(() => {
    ...
    const target = parseCurrencyDecimal(targetBalance).toNumber();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="px-6 py-4 bg-blue-50 dark:bg-gray-900/50 border-b border-gray-200 dark:border-gray-700">
          ...
          {targetBalance && Math.abs(parseCurrencyDecimal(targetBalance).toNumber() - projectedBalance) > 0.01 && (
            <div className="mt-3 p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg">
              <div className="flex items-center gap-2">
                <AlertCircleIcon size={16} className="text-yellow-600 dark:text-yellow-400" />
                <p className="text-sm text-yellow-800 dark:text-yellow-200">
                  Difference of {formatCurrency(Math.abs(parseCurrencyDecimal(targetBalance).toNumber() - projectedBalance))} from target
                </p>
              </div>
            </div>
          )}
