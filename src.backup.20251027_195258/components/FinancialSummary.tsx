...
import { formatPercentageValue, formatSignedPercentageValue } from '@wealthtracker/utils';
...
export default function FinancialSummary({ period }: FinancialSummaryProps) {
  ...
  const formatChange = (value: number, decimals: number = 1) =>
  ...
  return (
    <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl rounded-2xl shadow-lg border border-white/20 dark:border-gray-700/50 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className={`rounded-xl p-4 ${
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
            {formatPercentageValue(summary.savingsRate, 1)} savings rate
          </p>
        </div>
      </div>
                  ...
                  .map((budget, index) => (
                    <div key={index} className="flex items-center justify-between">
                      ...
                      <span className={`text-sm font-medium ${
                        ...
                        <span className="text-xs ml-1">({formatPercentageValue(budget.percentage, 0)})</span>
                      </span>
                    </div>
                  ))}
              </div>
      ...
      <div className="flex gap-3 mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
        <button
          onClick={() => {
            const summaryText = financialSummaryService.formatSummaryText(summary, displayCurrency);
