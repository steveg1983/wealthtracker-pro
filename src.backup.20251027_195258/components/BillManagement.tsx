...
export default function BillManagement() {
  ...
  const [, setEditingBill] = useState<Bill | null>(null);
  const [, setSelectedBill] = useState<string | null>(null);
  ...
  const handleAddBill = () => {
    ...
    const dueDate = new Date(newBill.dueDate || getTodayIsoDate());
    const nextDueDate = calculateNextDueDate(dueDate, newBill.frequency);
    
    const amountDecimal = parseCurrencyDecimal(newBill.amount);
  ...
  const handlePayBill = (billId: string) => {
    ...
    const lateFee = isLate ? toDecimal(25) : undefined;
