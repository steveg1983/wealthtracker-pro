...
type AccountListItem = Account | { isHeader: true; title: string };
...
export function VirtualizedAccountList({
  ...
  const processedAccounts = useMemo((): AccountListItem[] => {
    ...
    Object.entries(grouped).forEach(([type, items]) => {
      result.push({ isHeader: true, title: type });
      result.push(...items);
    });
  ...
  const isHeaderItem = (item: AccountListItem): item is { isHeader: true; title: string } => {
      ...
      >
        ...
        {account.tags && account.tags.length > 0 && (
          <div className="flex gap-2 mt-2">
            {account.tags.map((tag: string, idx: number) => (
              <span
                key={idx}
  ...
  const getItemHeight = useCallback((index: number) => {
    ...
    if (isHeaderItem(item)) return 40;
    if (item.tags && item.tags.length > 0) return 100;
    return 80;
  }, [processedAccounts]);
