...
type QIFPreviewResult = ReturnType<typeof qifImportService.parseQIF>;

type QIFImportResultState =
  | {
      success: true;
      imported: number;
      duplicates: number;
...
export default function QIFImportModal({ isOpen, onClose }: QIFImportModalProps): React.JSX.Element {
  ...
  const [parseResult, setParseResult] = useState<QIFPreviewResult | null>(null);
  const [importResult, setImportResult] = useState<QIFImportResultState | null>(null);
  ...
  const handleDrop = useCallback(
    (event: React.DragEvent<HTMLDivElement>) => {
      ...
      if (!droppedFile.name.toLowerCase().endsWith('.qif')) {
        alert('Please select a QIF file');
        return;
      }
