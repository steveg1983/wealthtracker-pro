...
export default function RealTimePortfolio({ accountId, accountName, currency }: RealTimePortfolioProps) {
  ...
  const holdings: StockHolding[] = useMemo(() => {
    return (account?.holdings || [])
      .filter(holding => holding.ticker && holding.shares)
      .map(holding => ({
        symbol: holding.ticker,
        shares: holding.shares,
        averageCost:
          holding.averageCost ?? (holding.shares !== 0 ? holding.value / holding.shares : 0),
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={`text-xl font-bold ${portfolioMetrics.totalGainPercent.greaterThanOrEqualTo(0) ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                  ...
                  {portfolioMetrics.totalGainPercent
                ...
                <div className="flex items-center gap-6">
                  <div className="text-right">
                    ...
                    <p className="font-semibold text-gray-900 dark:text-white">
                      {new Decimal(holding.shares).toDecimalPlaces(2, Decimal.ROUND_HALF_UP).toNumber()}
                    </p>
                  </div>
                  ...
                  <div className="text-right">
                    ...
                    <p className={`text-xs ${holding.gainPercent.greaterThanOrEqualTo(0) ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                      ...
                      {holding.gainPercent.toDecimalPlaces(2, Decimal.ROUND_HALF_UP).toNumber()}%
                    </p>
                  </div>
                  ...
                  <div className="text-right">
                    ...
                    <p className="font-semibold text-gray-900 dark:text-white">
                      {holding.allocation.toDecimalPlaces(1, Decimal.ROUND_HALF_UP).toNumber()}%
                    </p>
                  </div>
                </div>
              </div>
