...
import { SearchIcon, FilterIcon, XIcon, ClockIcon, StarIcon } from './icons';
...
const isSortByOption = (value: string): value is NonNullable<SearchOptions['sortBy']> => {
  switch (value) {
    case 'date':
    case 'amount':
    case 'description':
...
const isSortOrderOption = (value: string): value is NonNullable<SearchOptions['sortOrder']> => {
...
export function EnhancedSearchBar({
      ...
      switch (e.key) {
        ...
        case 'Enter':
          if (selectedSuggestion >= 0 && selectedSuggestion < suggestions.length) {
            e.preventDefault();
            const suggestionValue = suggestions[selectedSuggestion];
            if (suggestionValue !== undefined) {
              onQueryChange(suggestionValue);
            }
  ...
  const handleDateChange = (field: 'dateFrom' | 'dateTo', value: string) => {
    if (value.trim() === '') {
      onOptionsChange({ [field]: null } as Partial<SearchOptions>);
      return;
    }

    onOptionsChange({ [field]: value } as Partial<SearchOptions>);
  };
  ...
  const handleAmountChange = (field: 'amountMin' | 'amountMax', value: string) => {
    ...
    if (!Number.isNaN(numValue)) {
      onOptionsChange({ [field]: numValue } as Partial<SearchOptions>);
    }
  };
                ...
                {TRANSACTION_TYPES.map(type => (
                  <label key={type} className="flex items-center gap-2">
                    <input
                      ...
                      onChange={(e) => {
                        const currentTypes: TransactionType[] = options.types ?? [];
                        const nextTypes: TransactionType[] = e.target.checked
                          ? [...currentTypes, type]
                          : currentTypes.filter(t => t !== type);
                        onOptionsChange({ types: nextTypes });
                      }}
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (value === 'cleared') {
                    onOptionsChange({ cleared: true });
                  } else if (value === 'uncleared') {
                    onOptionsChange({ cleared: false });
                  } else {
                    onOptionsChange({ cleared: null } as Partial<SearchOptions>);
                  }
                }}
