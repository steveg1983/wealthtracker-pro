...
export const VirtualizedTransactionList: React.FC<VirtualizedTransactionListProps> = ({
  ...
  const listRef = useRef<FixedSizeList<TransactionRowProps['data']> | null>(null);
  
  // Memoize the data object to prevent unnecessary re-renders
  const itemData = useMemo<TransactionRowProps['data']>(() => {
    const base: TransactionRowProps['data'] = {
      transactions,
      formatCurrency
    };
          ...
          >
            {({ onItemsRendered, ref }) => (
              <FixedSizeListComponent
                ref={(list) => {
                  const listInstance = list as FixedSizeList<TransactionRowProps['data']> | null;

                  if (typeof ref === 'function') {
                    ref(listInstance);
                  } else if (ref) {
                    (ref as MutableRefObject<FixedSizeList<TransactionRowProps['data']> | null>).current = listInstance;
                  }
