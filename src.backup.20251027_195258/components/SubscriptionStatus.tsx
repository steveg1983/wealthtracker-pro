...
import type { SubscriptionTier } from '@app-types/subscription';
...
const PLAN_DETAILS: Record<SubscriptionTier, PlanDetails> = {
  free: {
    name: 'Free',
    icon: Zap,
    color: 'text-gray-600',
  ...
  business: {
    ...
    features: [
      ...
      { name: 'Consolidated dashboards', included: true },
      { name: 'Advanced workflows', included: true },
      { name: 'Role-based access control', included: true },
      { name: 'Dedicated success manager', included: true },
      { name: 'Custom SLA', included: true },
      { name: 'Enhanced security controls', included: true },
      { name: 'Priority phone support', included: true },
      { name: 'Audit-ready reporting', included: true },
    ]
  }
};

export default function SubscriptionStatus(): React.JSX.Element {
  const { isLoading, tier, subscription } = useSubscription();
  ...
  const updateSubscription = async (newTier: 'premium' | 'pro'): Promise<void> => {
    logger.info('Subscription upgrade requested', { tier: newTier });
  };
  ...
  if (!currentPlan) {
    return <div>Error: Unable to load subscription plan details</div>;
  }

  const Icon = currentPlan.icon;

  const handleUpgrade = async (newTier: 'premium' | 'pro' | 'business'): Promise<void> => {
    if (newTier === 'business') {
      logger.info('Business tier upgrade requested (not yet available)', { tier: newTier });
      return;
    }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
        ...
        <div className="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700 flex gap-3">
          {subscriptionTier === 'free' && (
            <>
              ...
              <button
                ...
                className="flex-1 px-4 py-2 border border-purple-900 text-purple-900 dark:text-purple-300 rounded-lg hover:bg-purple-100 dark:hover:bg-purple-900/30 transition-colors"
              >
                Upgrade to Business
              </button>
            </>
          ...
          {(subscriptionTier === 'pro' || subscriptionTier === 'business') && !cancelAtPeriodEnd && (
            <button
              onClick={handleCancel}
              className="px-4 py-2 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors"
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm text-gray-500">£39.99/mo</span>
              </div>
              <p className="text-xs text-gray-600 dark:text-gray-400">
                Tailored for finance teams with advanced controls and support
              </p>
            </div>
          </div>
        </div>
