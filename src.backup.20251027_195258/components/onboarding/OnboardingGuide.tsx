...
export default function OnboardingGuide({ onComplete }: OnboardingGuideProps): React.JSX.Element {
  ...
  const renderIcon = (iconName: OnboardingStep['iconName']) => {
    switch (iconName) {
      case 'banknotes':
        return <ProfessionalIcon name="banknote" size={48} className="text-gray-600" />;
      case 'upload':
        return <ProfessionalIcon name="upload" size={48} className="text-green-600" />;
      case 'chart':
        return <ProfessionalIcon name="chartBar" size={48} className="text-purple-600" />;
      default:
        return <ProfessionalIcon name="success" size={48} className="text-gray-600" />;
    }
  };
