...
export function VisualBudgetProgress({ 
  ...
  const velocity = useMemo((): SpendingVelocity => {
    ...
    const elapsedMs = Math.max(0, currentTime - normalizedStart.getTime());
  ...
  }, [spent, normalizedEnd, normalizedStart, budget.amount]);
  ...
  const percentage = budgetAmount > 0
    ...
    : 0;
  const percentageLabel = formatPercentageValue(percentage, 0);
  const periodElapsedLabel = formatPercentageValue(velocity.percentOfPeriodElapsed, 0);
            ...
            >
              {showTooltip === 'expected' && (
                <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 bg-gray-900 text-white text-xs rounded px-2 py-1 whitespace-nowrap">
                  {`Expected: ${periodElapsedLabel} of period elapsed`}
                </div>
              )}
            </div>
          )}
                ...
                <div className="text-sm">
                  ...
                  <p className="text-yellow-700 dark:text-yellow-300">
                    ...
                    {!velocity.willExceed && !velocity.isOnTrack && (
                      <>You're spending faster than planned. You've used {percentageLabel} of your budget 
                      but only {periodElapsedLabel} of the period has elapsed.</>
                    )}
                    {!velocity.willExceed && velocity.isOnTrack && percentage >= 90 && (
      ...
      {!compact && (
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <select
              value={sortBy}
              onChange={(e) => setSortBy(e.target.value as 'percentage' | 'amount' | 'name')}
