...
export default function BudgetSummaryWidget() {
  ...
  const monthlySpending = transactions
    ...
    .reduce((acc, t) => {
      ...
      const current = acc[category] ?? toDecimal(0);
      acc[category] = current.plus(toDecimal(t.amount));
      return acc;
  ...
  const warningBudgets = activeBudgets.filter(budget => {
    const spent = monthlySpending[budget.categoryId] ?? toDecimal(0);
    const budgetAmount = toDecimal(budget.amount);
    if (budgetAmount.isZero()) {
      return false;
    }
    const percentage = spent.dividedBy(budgetAmount).times(100);
    return percentage.greaterThanOrEqualTo(80);
  });
    ...
    <div className="bg-white p-6 rounded-lg shadow">
      ...
      <div className="space-y-3">
        {activeBudgets.slice(0, 3).map(budget => {
          const spent = monthlySpending[budget.categoryId] ?? toDecimal(0);
          const budgetAmount = toDecimal(budget.amount);
          const ratio = budgetAmount.isZero()
            ? toDecimal(0)
            : spent.dividedBy(budgetAmount);
          const percentage = Math.min(ratio.times(100).toNumber(), 100);
