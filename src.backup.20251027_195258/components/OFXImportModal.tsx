...
type OFXPreviewResult = Awaited<ReturnType<typeof ofxImportService.importTransactions>>;

type OFXImportResultState =
  | {
      success: true;
      imported: number;
      duplicates: number;
...
export default function OFXImportModal({ isOpen, onClose }: OFXImportModalProps): React.JSX.Element {
  ...
  const [parseResult, setParseResult] = useState<OFXPreviewResult | null>(null);
  const [importResult, setImportResult] = useState<OFXImportResultState | null>(null);
  ...
  const processImport = useCallback(async () => {
    ...
    try {
      ...
      if (selectedAccountId) {
        importOptions.accountId = selectedAccountId;
      }
      ...
      const matchedAccount = result.matchedAccount ?? accounts.find(a => a.id === selectedAccountId);

      setImportResult({
        success: true,
        imported: result.newTransactions,
        duplicates: result.duplicates,
        ...(matchedAccount ? { account: matchedAccount } : {})
      });
    } catch (error) {
