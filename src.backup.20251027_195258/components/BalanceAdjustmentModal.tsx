...
export default function BalanceAdjustmentModal({ 
  ...
  // Form state
  const [adjustmentDate, setAdjustmentDate] = useState<string>(getTodayIsoDate());
  ...
  const currentBalanceDecimal = toDecimal(currentBalance);
  const newBalanceDecimal = parseCurrencyDecimal(newBalance);
  const differenceDecimal = newBalanceDecimal.minus(currentBalanceDecimal);
  const adjustmentAmountDecimal = differenceDecimal.abs();
  const adjustmentAmount = toStorageNumber(adjustmentAmountDecimal);
  const isIncrease = differenceDecimal.greaterThan(0);
  
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    
    if (adjustmentAmountDecimal.lessThan(toDecimal(0.01))) {
    ...
    const adjustmentTransaction = {
      date: new Date(adjustmentDate || getTodayIsoDate()),
      description: description,
      amount: adjustmentAmount,
      type: isIncrease ? 'income' as const : 'expense' as const,
      category: category,
      accountId: accountId,
      notes: notes || `Manual balance adjustment from ${formatCurrency(currentBalanceDecimal, account.currency)} to ${formatCurrency(newBalanceDecimal, account.currency)}`,
      cleared: true,
            ...
            <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium text-gray-900 dark:text-white">
                    {formatCurrency(currentBalanceDecimal, account.currency)}
                  </p>
                </div>
                <div>
                  <p className="text-gray-600 dark:text-gray-400">New Balance</p>
                  <p className="font-medium text-gray-900 dark:text-white">
                    {formatCurrency(newBalanceDecimal, account.currency)}
                  </p>
                </div>
              </div>
              <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-600">
                ...
                <p className={`text-lg font-bold ${
                  ...
                  {isIncrease ? '+' : '-'}{formatCurrency(adjustmentAmountDecimal, account.currency)}
                </p>
              </div>
            </div>
