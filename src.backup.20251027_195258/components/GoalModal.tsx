...
type GoalCreatePayload = Omit<Goal, 'id' | 'createdAt' | 'updatedAt' | 'progress'>;
...
export default function GoalModal({ isOpen, onClose, goal }: GoalModalProps): React.JSX.Element {
  ...
  const { formData, updateField, handleSubmit, setFormData } = useModalForm<FormData>(
    ...
    },
    {
      onSubmit: async (data) => {
        ...
        const goalPayload: GoalCreatePayload = {
          name: trimmedName,
          type: data.type,
          targetAmount: parseCurrencyDecimal(data.targetAmount).toNumber(),
          currentAmount: parseCurrencyDecimal(data.currentAmount).toNumber(),
          targetDate,
          isActive: data.isActive
        };

        if (data.description.trim()) {
          goalPayload.description = data.description.trim();
        }

        if (data.linkedAccountIds.length > 0) {
          goalPayload.linkedAccountIds = data.linkedAccountIds;
        }
    ...
    if (goal) {
      ...
      if (goal.targetDate) {
        if (goal.targetDate instanceof Date) {
          ...
          targetDateString = dateStr.includes("T") ? dateStr.split("T")[0] || dateStr : dateStr;
        }
      }
      ...
      setFormData({
        ...
        targetDate: targetDateString || todayIso,
  ...
  }, [goal, isOpen, setFormData, todayIso]);
