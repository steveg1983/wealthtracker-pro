...
type TransactionFilterType = FilterOptions['types'][number];

const ALLOWED_FILTER_TYPES: readonly TransactionFilterType[] = ['income', 'expense', 'transfer'];

const isTransactionFilterType = (type: Transaction['type']): type is TransactionFilterType =>
  ALLOWED_FILTER_TYPES.includes(type as TransactionFilterType);
...
export default function BulkTransactionEdit({ 
  ...
  const filteredTransactions = useMemo(() => {
    return transactions.filter(t => {
      ...
      if (filters.types.length > 0) {
        if (!isTransactionFilterType(t.type) || !filters.types.includes(t.type)) {
          return false;
        }
      }
  ...
  const updateChangeField = useCallback(<K extends keyof BulkEditChanges>(
    key: K,
    value: BulkEditChanges[K] | undefined
  ) => {
    setChanges(prev => {
      const next: BulkEditChanges = { ...prev };
      if (value === undefined) {
        delete next[key];
      } else {
        next[key] = value;
  ...
  const handleUndo = () => {
    if (historyIndex > 0) {
      ...
      setSelectedIds(new Set(prevState.selectedIds));
  ...
  const handleRedo = () => {
    if (historyIndex < editHistory.length - 1) {
      ...
      setSelectedIds(new Set(nextState.selectedIds));
      setHistoryIndex(historyIndex + 1);
    }
  };
  ...
  const commonCategory = useMemo(() => {
    ...
    const firstCategory = firstTransaction.category;
  ...
  const getPreviewChanges = useMemo(() => {
    ...
    selectedTransactions.forEach(transaction => {
      ...
      const {
        ...
        tags: changeTags,
        cleared: changeCleared,
        notes: changeNotes,
        appendNote: shouldAppendNote,
        moveToAccount: changeMoveToAccount
      } = changes;

      if (changeCategory !== undefined && changeCategory !== transaction.category) {
        updates.push(`Category → ${changeCategory || 'Uncategorized'}`);
      }

      if (changeTags !== undefined) {
        updates.push(`Tags → ${changeTags.length > 0 ? changeTags.join(', ') : 'None'}`);
      }

      if (changeCleared !== undefined && changeCleared !== transaction.cleared) {
        updates.push(`Cleared → ${changeCleared ? 'Yes' : 'No'}`);
      }

      if (changeNotes !== undefined) {
        if (shouldAppendNote) {
          updates.push(`Note → Append "${changeNotes}"`);
        } else {
          updates.push(`Note → "${changeNotes}"`);
        }
      }

      if (changeMoveToAccount !== undefined && changeMoveToAccount !== transaction.accountId) {
        const account = accounts.find(a => a.id === changeMoveToAccount);
                    ...
                    <div className="space-y-1">
                      ...
                      <label className="flex items-center gap-2">
                        <input
                          ...
                          onChange={(e) => {
                            if (e.target.checked) {
                              setFilters({ ...filters, types: [...filters.types, 'transfer'] });
                            } else {
                              setFilters({ ...filters, types: filters.types.filter(t => t !== 'transfer') });
                            }
                          }}
                          className="rounded"
                        />
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={e => updateChangeField('category', e.target.value ? e.target.value : undefined)}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={e => {
                      ...
                      const nextTags = raw ? raw.split(',').map(tag => tag.trim()).filter(Boolean) : undefined;
                      updateChangeField('tags', nextTags && nextTags.length > 0 ? nextTags : undefined);
                    }}
                    className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg
                             bg-white dark:bg-gray-800"
                  />
                ...
                <div>
                  ...
                  <textarea
                    ...
                    value={changes.notes ?? ''}
                    onChange={e => updateChangeField('notes', e.target.value ? e.target.value : undefined)}
                    rows={3}
                    className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg
                             bg-white dark:bg-gray-800"
                  />
                  <label className="flex items-center gap-2 mt-2">
                    <input
                      ...
                      onChange={e => updateChangeField('appendNote', e.target.checked)}
                      className="rounded"
                    />
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={e => updateChangeField('moveToAccount', e.target.value ? e.target.value : undefined)}
                    className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg
                             bg-white dark:bg-gray-800"
                  >
