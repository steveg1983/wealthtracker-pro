...
import type { TooltipContentProps, BarProps } from 'recharts';
...
const MemoizedChart = memo((props: ChartRenderProps) => {
  ...
  if (type === 'line' || type === 'bar') {
    const chartData: LineBarChartData = data as LineBarChartData;
    const chartOptions: LineBarChartOptions = { ...(options ?? {}) };
    const showLegend = chartOptions.showLegend ?? true;
    const legendVerticalAlign = chartOptions.legendPosition === 'bottom' ? 'bottom' : 'top';
    const xTickFormatter = chartOptions.xAxisFormatter
      ? (value: string | number, _index: number) => chartOptions.xAxisFormatter!(value)
      : undefined;
    const yTickFormatter = chartOptions.yAxisFormatter
      ? (value: number, _index: number) => chartOptions.yAxisFormatter!(value)
      : undefined;

    const renderTooltipContent = ({ active, payload, label }: TooltipContentProps<ValueType, NameType>) => {
      if (!active || !payload?.length) {
        return null;
      }

      const labelString = typeof label === 'number' || typeof label === 'string'
        ? (xTickFormatter ? xTickFormatter(label, 0) : String(label))
        : '';
          ...
          <ResponsiveContainer>
            ...
            >
              ...
              {chartData.datasets.map((dataset, index) => {
                ...
                const stackId = chartOptions.stacked ? dataset.stackId ?? 'stack' : dataset.stackId;
                ...
                if (stackId !== undefined) {
                  (barProps as { stackId: NonNullable<BarProps['stackId']> }).stackId = stackId;
                }
  ...
  const chartData: PieChartDatum[] = data as PieChartDatum[];
  const chartOptions: PieChartOptions = { ...(options ?? {}) };
  const showLegend = chartOptions.showLegend ?? true;
  const legendVerticalAlign = chartOptions.legendPosition === 'bottom' ? 'bottom' : 'top';
  ...
  const renderPieTooltip = ({ active, payload }: TooltipContentProps<ValueType, NameType>) => {
    if (!active || !payload?.length) {
      return null;
    }
...
export const OptimizedChart: React.FC<OptimizedChartProps> = memo((props) => {
  ...
  const chartProps: ChartRenderProps = useMemo(() => {
    if (type === 'line' || type === 'bar') {
      const lineOptions = options as LineBarChartOptions | undefined;
      return lineOptions
        ? {
            type,
    ...
    const pieOptions = options as PieChartOptions | undefined;
    return pieOptions
      ? {
          type,
