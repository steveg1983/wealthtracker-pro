...
export default function TransactionSearch({ 
  ...
  const searchTransactions = useCallback((term: string, currentFilters: typeof filters): Transaction[] => {
    ...
    if (currentFilters.amountMin) {
      const minDecimal = parseCurrencyDecimal(currentFilters.amountMin);
      if (!minDecimal.isNaN()) {
        const minValue = minDecimal.toNumber();
        filtered = filtered.filter(t => Math.abs(t.amount) >= minValue);
      }
    }
    if (currentFilters.amountMax) {
      const maxDecimal = parseCurrencyDecimal(currentFilters.amountMax);
      if (!maxDecimal.isNaN()) {
        const maxValue = maxDecimal.toNumber();
        filtered = filtered.filter(t => Math.abs(t.amount) <= maxValue);
      }
    }
  ...
  const debouncedSearch = useMemo(
    ...
    [searchTransactions, onResultsChange]
  );
  ...
  categories.forEach((category) => categoryNames.add(category.name));
  transactions.forEach((transaction) => {
    if (transaction.category) {
      categoryNames.add(transaction.category);
    }
  });
  const availableCategories = Array.from(categoryNames).sort((a, b) => a.localeCompare(b));
