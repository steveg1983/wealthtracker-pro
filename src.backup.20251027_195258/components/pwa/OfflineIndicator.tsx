...
import type { OfflineQueueItem } from '../../pwa/offline-storage';
...
const isConflictSummary = (value: unknown): value is ConflictSummary => {
  if (!value || typeof value !== 'object') {
    return false;
  }

  const candidate = value as Partial<ConflictSummary>;
  return typeof candidate.id === 'string' && typeof candidate.entity === 'string';
};

export const OfflineIndicator: React.FC = () => {
  const offlineState = useOfflineState();
  const { sync, getConflicts } = useOfflineOperations();
  const [conflicts, setConflicts] = React.useState<ConflictSummary[]>([]);
  ...
  React.useEffect(() => {
    const loadConflicts = async () => {
      ...
      if (Array.isArray(conflictList)) {
        setConflicts(conflictList.filter(isConflictSummary));
      } else {
        setConflicts([]);
      }
    };
