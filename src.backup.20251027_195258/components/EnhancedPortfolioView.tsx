...
export default function EnhancedPortfolioView({ 
  ...
  const getAllocationPercent = useCallback((value: number, decimals: number = 1): number => {
  ...
  const formatSignedPercentage = (value: number, decimals: number = 2): string => {
    const decimalValue = new Decimal(value ?? 0);
    const sign = decimalValue.greaterThanOrEqualTo(0) ? '+' : '';
    return `${sign}${decimalValue.abs().toDecimalPlaces(decimals, Decimal.ROUND_HALF_UP).toNumber()}%`;
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
          ...
          <p className={`text-sm ${totalGain >= 0 ? 'text-green-600' : 'text-red-600'}`}>
            {totalCost > 0 ? formatSignedPercentage((totalGain / totalCost) * 100) : '+0.00%'}
          </p>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-right">
                      <div className="flex flex-col items-end">
                        ...
                        {livePrice && (
                          ...
                          {formatSignedPercentage(livePrice.changePercent ?? 0)}
                          </span>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-right">
                      {holding.gain !== undefined ? (
                        <div className="flex flex-col items-end">
                          ...
                          {formatSignedPercentage(holding.gainPercent || 0)}
                          </span>
                        </div>
                      ) : (
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-right">
                      <div className="flex items-center justify-end gap-2">
                        <div className="w-20 bg-gray-200 dark:bg-gray-600 rounded-full h-2">
                          {(() => {
                            const allocationPercent = getAllocationPercent(holding.marketValue || holding.value, 1);
                            return (
                              <div 
                        ...
                        <span className="text-gray-600 dark:text-gray-400 w-12 text-right">
                          {formatPercentage(getAllocationPercent(holding.marketValue || holding.value, 1), 1)}
                        </span>
                      </div>
                    </td>
                  </tr>
            ...
            return (
              ...
              >
                ...
                <div className="flex justify-between text-sm">
                  ...
                  {holding.gain !== undefined && (
                    <span className={`font-medium ${holding.gain >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                      {holding.gain >= 0 ? '+' : ''}{formatCurrency(holding.gain, currency)} ({formatSignedPercentage(holding.gainPercent || 0)})
                    </span>
                  )}
                </div>
              </div>
