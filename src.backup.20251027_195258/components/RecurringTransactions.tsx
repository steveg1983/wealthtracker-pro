...
export default function RecurringTransactions(): React.JSX.Element {
  ...
  const calculateNextDate = useCallback((currentDate: string, frequency: RecurrenceFrequency): string => {
  ...
  }, [addTransaction, calculateNextDate]);
  ...
  }, [processTemplate, processingTemplates, templates]);

  // Check and process due templates on mount and hourly
  useEffect(() => {
    processDueTemplates();
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
                    <div>
                      ...
                      <p className={`font-medium ${
                        parseCurrencyDecimal(template.amount).greaterThan(0) ? 'text-green-600' : 'text-red-600'
                      }`}>
                        {formatCurrency(parseCurrencyDecimal(template.amount).toNumber())}
                      </p>
                    </div>
                    <div>
