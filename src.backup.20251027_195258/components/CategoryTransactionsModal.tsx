...
export default function CategoryTransactionsModal({ 
  ...
  useEffect(() => {
    if (isOpen && categoryTransactions.length > 0) {
      ...
      if (!fromDate) {
        const [earliestDateString] = earliestDate.toISOString().split('T');
        if (earliestDateString) {
          setFromDate(earliestDateString);
        }
      }
      if (!toDate) {
        const [latestDateString] = latestDate.toISOString().split('T');
        if (latestDateString) {
          setToDate(latestDateString);
        }
      }
    }
