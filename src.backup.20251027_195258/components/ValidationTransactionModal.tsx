...
interface EditableTransactionValues {
  amount: string;
  category: string;
  description: string;
}
...
export default function ValidationTransactionModal({
  ...
  const [editValues, setEditValues] = useState<Record<string, EditableTransactionValues>>({});
  ...
  const handleSave = (transactionId: string): void => {
    ...
    if (values) {
      updateTransaction(transactionId, {
        amount: toStorageNumber(parseCurrencyDecimal(values.amount || '0')),
                          ...
                          <div>
                            ...
                            <input
                              ...
                              className={`w-full px-3 py-1 border rounded-lg
                                       bg-white dark:bg-gray-800 text-gray-900 dark:text-white
                                       ${parseCurrencyDecimal(values.amount || '0').lessThanOrEqualTo(0) 
                                         ? 'border-red-500' 
