...
export default function AddWidgetModal({
  ...
  const widgetOptions: WidgetType[] = useMemo(() => {
    if (availableWidgets) {
      return availableWidgets;
    }

    return WidgetRegistry.getAllWidgets().map(widget => ({
      id: widget.type,
      title: widget.title,
      icon: widget.icon,
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    {isSelected && !isAlreadyAdded && (
                      <div className="absolute top-2 right-2">
                        <ProfessionalIcon name="check" size={20} className="text-gray-600" />
                      </div>
                    )}
                  </div>
                </button>
        ...
        <div className="p-6 border-t border-gray-200 dark:border-gray-700 bg-blue-50 dark:bg-gray-900/50">
          <div className="flex items-center justify-between">
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {selectedWidget 
                ? `Selected: ${widgetOptions.find(w => w.id === selectedWidget)?.title ?? ''}`
