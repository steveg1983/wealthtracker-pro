...
export default function RecentAlertsWidget({ isCompact = false }: RecentAlertsWidgetProps): React.JSX.Element {
  ...
  const alerts = useMemo(() => {
    ...
    budgets.forEach(budget => {
      ...
      const budgetAmountDecimal = toDecimal(budget.amount);
      if (budgetAmountDecimal.lessThanOrEqualTo(0)) {
        return;
      }

      const spentDecimal = toDecimal(budget.spent);
      if (spentDecimal.greaterThan(budgetAmountDecimal)) {
        const overAmountDecimal = spentDecimal.minus(budgetAmountDecimal);
        const overPercentDecimal = overAmountDecimal.dividedBy(budgetAmountDecimal).times(100);
        
        alertsList.push({
          id: `budget-over-${budget.id}`,
          type: 'warning',
          title: 'Budget Exceeded',
          message: `${budget.categoryId} is ${formatPercentageValue(overPercentDecimal, 0)} over budget (${formatCurrency(overAmountDecimal)} over)`,
          timestamp: new Date(),
          actionUrl: '/budget',
          actionLabel: 'View Budget',
          icon: AlertTriangleIcon
        });
      } else if (spentDecimal.greaterThan(budgetAmountDecimal.times(0.9))) {
        const usagePercentDecimal = spentDecimal.dividedBy(budgetAmountDecimal).times(100);
        alertsList.push({
          id: `budget-warning-${budget.id}`,
          type: 'info',
          title: 'Budget Warning',
          message: `${budget.categoryId} is at ${formatPercentageValue(usagePercentDecimal, 0)} of budget`,
          timestamp: new Date(),
          actionUrl: '/budget',
          actionLabel: 'View Budget',
    ...
    const getCreditLimit = (account: Account): number | null => {
      const metadata = (account as { metadata?: unknown }).metadata;
      if (metadata && typeof metadata === 'object' && !Array.isArray(metadata)) {
        const creditLimit = (metadata as Record<string, unknown>).creditLimit;
    ...
    accounts
      ...
      .forEach(account => {
        const limit = getCreditLimit(account) ?? 0;
        const limitDecimal = toDecimal(limit);
        if (limitDecimal.lessThanOrEqualTo(0)) {
          return;
        }
        const balanceDecimal = toDecimal(account.balance).abs();
        const utilizationDecimal = balanceDecimal.dividedBy(limitDecimal).times(100);
        
        if (utilizationDecimal.greaterThan(90)) {
          alertsList.push({
            id: `high-util-${account.id}`,
            type: 'error',
            title: 'High Credit Utilization',
            message: `${account.name} is at ${formatPercentageValue(utilizationDecimal, 0)} utilization`,
            timestamp: new Date(),
            actionUrl: '/accounts',
            actionLabel: 'View Account',
            icon: CreditCardIcon
    ...
    goals.forEach(goal => {
      ...
      const progressDecimal = toDecimal(goal.currentAmount).dividedBy(goal.targetAmount).times(100);
      const progressValue = progressDecimal.toNumber();
      
      if (progressValue >= 100 && !goal.completedAt) {
        alertsList.push({
        ...
        alertsList.push({
          ...
          message: `"${goal.name}" is ${formatPercentageValue(progressDecimal, 0)} complete`,
          timestamp: new Date(),
          actionUrl: '/goals',
          actionLabel: 'View Goal',
          icon: TargetIcon
    ...
    const weeklySpendingDecimal = recentTransactions.reduce(
      (sum, t) => sum.plus(toDecimal(t.amount).abs()),
      toDecimal(0)
    );
    const avgTransactionDecimal = recentTransactions.length > 0
      ? weeklySpendingDecimal.dividedBy(recentTransactions.length)
      : toDecimal(0);
    
    recentTransactions.forEach(transaction => {
      const transactionAmountDecimal = toDecimal(transaction.amount).abs();
      if (avgTransactionDecimal.greaterThan(0) && transactionAmountDecimal.greaterThan(avgTransactionDecimal.times(3))) {
        alertsList.push({
          id: `unusual-spending-${transaction.id}`,
          type: 'info',
          title: 'Unusual Transaction',
          message: `Large expense: ${transaction.description} (${formatCurrency(transactionAmountDecimal)})`,
    ...
    const monthIncomeDecimal = monthTransactions
      .filter(t => t.type === 'income')
      .reduce((sum, t) => sum.plus(toDecimal(t.amount)), toDecimal(0));
    const monthExpensesDecimal = monthTransactions
      .filter(t => t.type === 'expense')
      .reduce((sum, t) => sum.plus(toDecimal(t.amount).abs()), toDecimal(0));
    
    if (monthIncomeDecimal.greaterThan(monthExpensesDecimal) && monthExpensesDecimal.greaterThan(0)) {
      const savingsRateDecimal = monthIncomeDecimal
        .minus(monthExpensesDecimal)
        .dividedBy(monthIncomeDecimal)
        .times(100);
      if (savingsRateDecimal.greaterThan(toDecimal(20))) {
        alertsList.push({
          id: 'high-savings',
          type: 'success',
          title: 'Great Savings Rate!',
          message: `You're saving ${formatPercentageValue(savingsRateDecimal, 0)} of your income this month`,
          timestamp: new Date(),
          icon: TrendingUpIcon
        });
      }
