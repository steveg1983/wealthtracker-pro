...
export default function BillRemindersWidget({ isCompact = false }: BillRemindersWidgetProps): React.JSX.Element {
  ...
  const upcomingBills = useMemo(() => {
    ...
    // Identify isRecurring bills from transaction history
    const isRecurringPatterns = new Map<string, { amount: number; dayOfMonth: number; lastPaid?: Date }>();
    ...
    transactions
      ...
      .forEach(t => {
        const normalizedAmountForKey = toDecimal(t.amount ?? 0)
          .abs()
          .toDecimalPlaces(0, Decimal.ROUND_HALF_UP)
          .toNumber();
        const key = `${t.description}-${normalizedAmountForKey}`;
        const date = new Date(t.date);
        
        if (!isRecurringPatterns.has(key) || 
            (isRecurringPatterns.get(key)?.lastPaid || new Date(0)) < date) {
          const roundedAmount = toDecimal(t.amount)
            .abs()
            .toDecimalPlaces(2, Decimal.ROUND_HALF_UP)
            .toNumber();
          isRecurringPatterns.set(key, {
    ...
    isRecurringPatterns.forEach((pattern, key) => {
  ...
  const summary = useMemo(() => {
    ...
    const totalDueDecimal = unpaidBills.reduce(
      (sum, bill) => sum.plus(toDecimal(bill.amount)),
      toDecimal(0),
    );
    const totalDue = totalDueDecimal.toDecimalPlaces(2, Decimal.ROUND_HALF_UP).toNumber();
  ...
  return (
    <div className="space-y-4">
      ...
      {/* View All Button */}
      <button
        onClick={() => navigate('/transactions?filter=isRecurring')}
