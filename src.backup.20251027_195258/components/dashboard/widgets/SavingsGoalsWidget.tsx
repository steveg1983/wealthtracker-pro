...
export default function SavingsGoalsWidget({ isCompact = false }: SavingsGoalsWidgetProps): React.JSX.Element {
  ...
  const goalsProgress = useMemo(() => {
    ...
    const activeGoals = goals
      ...
      .map(goal => {
        const targetAmountDecimal = toDecimal(goal.targetAmount);
        const currentAmountDecimal = toDecimal(goal.currentAmount);

        const progressDecimal = targetAmountDecimal.isZero()
          ? toDecimal(0)
          : currentAmountDecimal.dividedBy(targetAmountDecimal).times(100);
        const remainingDecimal = targetAmountDecimal.minus(currentAmountDecimal);
        ...
        const monthlyTargetDecimal = remainingDecimal.dividedBy(monthsRemaining);
        
        // Calculate recent savings rate (last 30 days)
        const thirtyDaysAgo = new Date();
        thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
        
        const recentSavingsDecimal = transactions
          .filter(t => 
        ...
        const isOnTrack = monthlyTargetDecimal.greaterThan(0)
          ? recentSavingsDecimal.greaterThanOrEqualTo(monthlyTargetDecimal.times(0.9))
          : true;
        ...
        return {
          ...
          currentAmount: currentAmountDecimal.toNumber(),
          progress: Math.min(100, progressDecimal.toNumber()),
          monthlyTarget: monthlyTargetDecimal.toNumber(),
          daysRemaining,
          isOnTrack,
          completedAt: progressDecimal.greaterThanOrEqualTo(100),
          targetDate,
  ...
  const summary = useMemo(() => {
    ...
    const completedGoals = goalsProgress.filter(g => g.completedAt).length;
    const onTrackGoals = goalsProgress.filter(g => g.isOnTrack && !g.completedAt).length;
    const totalSavedDecimal = goalsProgress.reduce(
      (sum, g) => sum.plus(toDecimal(g.currentAmount)),
      toDecimal(0)
    );
    const totalTargetDecimal = goalsProgress.reduce(
      (sum, g) => sum.plus(toDecimal(g.targetAmount)),
      toDecimal(0)
    );
    const overallProgressDecimal = totalTargetDecimal.greaterThan(0)
      ? totalSavedDecimal.dividedBy(totalTargetDecimal).times(100)
      : toDecimal(0);
    ...
    return {
      ...
      completedGoals,
      onTrackGoals,
      totalSaved: totalSavedDecimal.toNumber(),
      totalTarget: totalTargetDecimal.toNumber(),
      overallProgress: overallProgressDecimal.toNumber()
    };
  }, [goalsProgress]);
      ...
      <div className="bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 rounded-lg p-4">
        ...
        <div className="flex items-center justify-between mt-2 text-xs">
          <span className="text-gray-600 dark:text-gray-400">
            {formatPercentageValue(summary.overallProgress, 0)} complete
          </span>
          <span className="text-gray-600 dark:text-gray-400">
            ...
            <div className="flex items-start justify-between mb-2">
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  {goal.isOnTrack && !goal.completedAt && (
              ...
              <div className="text-right">
                <div className="text-lg font-bold text-gray-900 dark:text-white">
                  {formatPercentageValue(goal.progress, 0)}
                </div>
              </div>
            </div>
