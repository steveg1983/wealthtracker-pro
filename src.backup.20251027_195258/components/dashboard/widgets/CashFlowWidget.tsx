  ...
  const cashFlowData = useMemo(() => {
    ...
    const calculatePeriodFlow = (days: number) => {
      ...
      const savingsRate = income.greaterThan(0)
        ? net.dividedBy(income).times(100)
        : toDecimal(0);
    ...
    const dailyAvgIncome = monthFlow.income.dividedBy(daysInMonth);
    const dailyAvgExpenses = monthFlow.expenses.dividedBy(daysInMonth);
    ...
    for (let i = 3; i >= 0; i--) {
      ...
      weeklyData.push({
        ...
        income: weekIncome.toNumber(),
        expenses: weekExpenses.toNumber(),
        net: weekIncome.minus(weekExpenses).toNumber()
      });
    }
    ...
    return {
      ...
      dailyAvgExpenses,
      weeklyData,
      trend: monthFlow.net.greaterThan(0) ? 'positive' : 'negative'
    };
  }, [transactions]);

  const weeklyChartData = cashFlowData.weeklyData;
  const monthSavingsPercent = Decimal.min(
    Decimal.max(cashFlowData.month.savingsRate.times(100), new Decimal(0)),
    new Decimal(100)
  ).toNumber();
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          <div>
            <div className="flex items-center gap-1">
              {cashFlowData.month.net.greaterThanOrEqualTo(0) ? (
            ...
            <div className={`text-lg font-bold ${
              cashFlowData.month.net.greaterThanOrEqualTo(0) ? 'text-green-600' : 'text-red-600'
            }`}>
              {cashFlowData.month.net.greaterThanOrEqualTo(0) ? '+' : ''}{formatCurrency(cashFlowData.month.net)}
            </div>
          </div>
        </div>
        
        {/* Savings Rate */}
        {cashFlowData.month.savingsRate.greaterThan(0) && (
          <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
            <div className="flex items-center justify-between">
              <span className="text-xs text-gray-600 dark:text-gray-400">Savings Rate</span>
              <span className="text-sm font-semibold text-green-600">
                {formatPercentageValue(cashFlowData.month.savingsRate.times(100), 1)}
              </span>
            </div>
            <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 mt-1">
              <div 
                className="bg-green-500 h-2 rounded-full"
                style={{ width: `${monthSavingsPercent}%` }}
              />
            </div>
          </div>
        )}
          ...
          <div className="h-32">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={weeklyChartData}>
                ...
                <YAxis tickFormatter={(value) => formatCurrency(Number(value))} tick={{ fill: '#6B7280' }} />
                <Tooltip formatter={(value: number) => formatCurrency(Number(value))} cursor={{ fill: 'rgba(148, 163, 184, 0.1)' }} />
                <Legend />
                <Bar dataKey="income" fill="rgba(34, 197, 94, 0.8)" />
                <Bar dataKey="expenses" fill="rgba(239, 68, 68, 0.8)" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="text-center p-2 bg-gray-50 dark:bg-gray-800 rounded">
          ...
          <div className={`text-sm font-semibold ${
            cashFlowData.week.net.greaterThanOrEqualTo(0) ? 'text-green-600' : 'text-red-600'
          }`}>
            {cashFlowData.week.net.greaterThanOrEqualTo(0) ? '+' : ''}{formatCurrency(cashFlowData.week.net)}
          </div>
        </div>
        ...
        <div className="text-center p-2 bg-gray-50 dark:bg-gray-800 rounded">
          ...
          <div className={`text-sm font-semibold ${
            cashFlowData.quarter.net.greaterThanOrEqualTo(0) ? 'text-green-600' : 'text-red-600'
          }`}>
            {cashFlowData.quarter.net.greaterThanOrEqualTo(0) ? '+' : ''}{formatCurrency(cashFlowData.quarter.net)}
          </div>
        </div>
      </div>
