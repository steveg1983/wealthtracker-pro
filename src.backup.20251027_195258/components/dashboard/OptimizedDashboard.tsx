...
interface WidgetContentProps {
  widget: { type: string; isCompact?: boolean };
  metrics: { totalAssets: number; totalLiabilities: number; netWorth: number };
  accounts: Account[];
...
const WidgetContent = memo(({ widget, metrics, accounts, transactions, budgets, navigate, formatCurrencyWithSymbol }: WidgetContentProps) => {
  switch (widget.type) {
    case 'netWorth':
      return (
    ...
    case 'savingsGoals':
      return <SavingsGoalsWidget isCompact={Boolean(widget.isCompact)} />;
      
    case 'debtTracker':
      return <DebtTrackerWidget isCompact={Boolean(widget.isCompact)} />;
      
    case 'billReminders':
      return <BillRemindersWidget isCompact={Boolean(widget.isCompact)} />;
      
    case 'investmentPerformance':
      return <InvestmentPerformanceWidget isCompact={Boolean(widget.isCompact)} />;
      
    case 'cashFlow':
      return <CashFlowWidget isCompact={Boolean(widget.isCompact)} />;
      
    case 'recentAlerts':
      return <RecentAlertsWidget isCompact={Boolean(widget.isCompact)} />;
      
    case 'netWorthTrend':
      return <NetWorthTrendWidget isCompact={Boolean(widget.isCompact)} />;
      
    case 'expenseCategories':
      return <ExpenseCategoriesWidget isCompact={Boolean(widget.isCompact)} />;
...
const WIDGET_TYPES = {
  netWorth: { title: 'Net Worth', icon: createIcon('wallet'), defaultSize: 'medium', description: 'Track your total net worth' },
  netWorthTrend: { title: 'Net Worth Trend', icon: createIcon('trendingUp'), defaultSize: 'large', description: 'Historical net worth chart' },
  accounts: { title: 'Accounts Overview', icon: createIcon('banknote'), defaultSize: 'large', description: 'View all your accounts' },
  transactions: { title: 'Recent Transactions', icon: createIcon('creditCard'), defaultSize: 'large', description: 'Latest transactions' },
  budgets: { title: 'Budget Status', icon: createIcon('chartPie'), defaultSize: 'medium', description: 'Budget tracking' },
  savingsGoals: { title: 'Savings Goals', icon: createIcon('goals'), defaultSize: 'large', description: 'Track savings goals progress' },
  debtTracker: { title: 'Debt Tracker', icon: createIcon('creditCard'), defaultSize: 'medium', description: 'Monitor debts and loans' },
  billReminders: { title: 'Bill Reminders', icon: createIcon('calendar'), defaultSize: 'medium', description: 'Upcoming bills and payments' },
  investmentPerformance: { title: 'Investment Performance', icon: createIcon('trendingUp'), defaultSize: 'large', description: 'Portfolio performance' },
  cashFlow: { title: 'Cash Flow', icon: createIcon('arrowsLeftRight'), defaultSize: 'large', description: 'Income vs expenses' },
  recentAlerts: { title: 'Recent Alerts', icon: createIcon('warning'), defaultSize: 'medium', description: 'Important notifications' },
  expenseCategories: { title: 'Expense Categories', icon: createIcon('chartPie'), defaultSize: 'medium', description: 'Spending by category' }
};
...
export default function OptimizedDashboard(): React.JSX.Element {
  ...
  const { layout, isLoading, addWidget, removeWidget, updateWidget, reorderWidgets } = useDashboardLayout();
  ...
  const metrics = useMemo(() => {
    ...
    return {
      totalAssets: totalAssetsDecimal.toNumber(),
      totalLiabilities: totalLiabilitiesDecimal.toNumber(),
      netWorth: netWorthDecimal.toNumber(),
    };
  }, [accounts]);
    ...
    <div className="p-6 space-y-6">
        ...
        <div className="flex items-center gap-4">
          {isEditMode ? (
            <>
              ...
              >
                <ProfessionalIcon name="save" size={20} />
                Save Layout
              </button>
            </>
          ) : (
      ...
      {layout.widgets.length === 0 && (
        <div className="text-center py-12 bg-gray-50 dark:bg-gray-800 rounded-lg">
          <ProfessionalIcon name="grid" size={48} className="mx-auto text-gray-400 mb-4" />
