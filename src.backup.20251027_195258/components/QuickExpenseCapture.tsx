...
const formatDistanceMiles = (distance: number | null | undefined): string | null => {
  if (distance === null || distance === undefined) {
    return null;
  }

  return `${toDecimal(distance).toDecimalPlaces(1).toNumber()} mi`;
};
...
export default function QuickExpenseCapture({ isOpen, onClose, onExpenseCreated }: QuickExpenseCaptureProps) {
  ...
  const handleSaveExpense = async () => {
    ...
    try {
      ...
      const expense = selectedMerchant
        ? { ...baseExpense, notes: `Location: ${selectedMerchant.address}` }
        : baseExpense;
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-2 max-h-60 overflow-y-auto">
        {nearbyMerchants.map((merchant, index) => {
          const formattedDistance = formatDistanceMiles(merchant.distance);
          return (
            <button
              key={index}
