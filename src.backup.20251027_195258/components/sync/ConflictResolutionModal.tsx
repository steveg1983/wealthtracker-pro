...
type ConflictType = 'account' | 'transaction' | 'budget' | 'category';
...
type ConflictValue = Record<string, unknown>;
...
export default function ConflictResolutionModal({
  ...
  const getTypeIcon = (type: ConflictType): React.JSX.Element => {
    switch (type) {
      case 'account':
  ...
  const renderValue = (value: ConflictValue, type: ConflictType): React.JSX.Element => {
    if (type === 'transaction') {
      const transaction = value as TransactionConflict;
      const amount = typeof transaction.amount === 'number' ? transaction.amount : 0;
      const transactionDate = transaction.date ? new Date(transaction.date) : undefined;
    ...
    if (type === 'account') {
      const account = value as AccountConflict;
    ...
    if (type === 'budget') {
      const budget = value as BudgetConflict;
      const amount = typeof budget.amount === 'number' ? budget.amount : 0;

      return (
        <div className="space-y-1">
          <div className="font-medium">{budget.categoryName || 'Budget'}</div>
          <div className="text-sm text-gray-600 dark:text-gray-400">
            {formatCurrency(amount)} {budget.period ? `/ ${budget.period}` : ''}
          </div>
        </div>
      );
    }
