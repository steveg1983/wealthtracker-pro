...
export default function CSVImportWizard({ isOpen, onClose, type }: CSVImportWizardProps): React.JSX.Element {
  ...
  const ensureString = (value: JsonValue | undefined, fallback = ''): string => {
  ...
  const parseNumericValue = (value: JsonValue | undefined): number => {
    if (value === null || value === undefined) {
      return 0;
    }
    ...
    const decimalValue = parseCurrencyDecimal(value);
    if (decimalValue.isNaN()) {
      return 0;
    }

    return toStorageNumber(decimalValue);
  };

  const normalizeAccountType = (rawType: string): Account['type'] => {
  ...
  const updateMapping = (index: number, updates: Partial<ColumnMapping>) => {
    setMappings(prevMappings => prevMappings.map((mapping, mapIndex) => {
      if (mapIndex !== index) {
        return mapping;
      }
  ...
  const processImport = async () => {
    ...
    const mappingByTarget = new Map<string, ColumnMapping>();

    mappings.forEach(mapping => {
      const columnIndex = headers.indexOf(mapping.sourceColumn);
      if (columnIndex >= 0) {
        columnIndexMap.set(mapping.targetField, columnIndex);
        mappingByTarget.set(mapping.targetField, mapping);
      }
    });

    const resolveValue = (row: string[], targetField: string): JsonValue | undefined => {
      const mapping = mappingByTarget.get(targetField);
      if (!mapping) {
        return undefined;
      }

      const columnIndex = columnIndexMap.get(targetField);
    ...
    try {
      if (type === 'transaction') {
        ...
        const importOutcome = await enhancedCsvImportService.importTransactions(
          csvContent,
          mappings,
          transactions,
          accountMap,
        ...
        for (const item of importOutcome.items) {
          const candidate = item as Partial<Transaction>;

          if (!(candidate.date instanceof Date)) {
            continue;
          }
          ...
          if (typeof candidate.accountId !== 'string' || candidate.accountId.trim().length === 0) {
            continue;
          }

          if (candidate.type !== 'income' && candidate.type !== 'expense' && candidate.type !== 'transfer') {
            continue;
          }
          ...
          const transactionPayload: Omit<Transaction, 'id'> = {
            ...
            type: candidate.type,
            cleared: typeof candidate.cleared === 'boolean' ? candidate.cleared : false,
            isImported: true
          };

          if (Array.isArray(candidate.tags)) {
            transactionPayload.tags = candidate.tags.filter((tag): tag is string => typeof tag === 'string');
          }

          if (typeof candidate.notes === 'string' && candidate.notes.trim().length > 0) {
            transactionPayload.notes = candidate.notes;
          }

          if (candidate.reconciledWith) {
            transactionPayload.reconciledWith = candidate.reconciledWith;
          }

          if (candidate.reconciledDate instanceof Date) {
            transactionPayload.reconciledDate = candidate.reconciledDate;
          }

          if (typeof candidate.reconciledNotes === 'string') {
            transactionPayload.reconciledNotes = candidate.reconciledNotes;
          }

          if (typeof candidate.bankReference === 'string') {
            transactionPayload.bankReference = candidate.bankReference;
          }

          if (typeof candidate.isRecurring === 'boolean') {
            transactionPayload.isRecurring = candidate.isRecurring;
          }

          if (typeof candidate.isSplit === 'boolean') {
            transactionPayload.isSplit = candidate.isSplit;
          }

          if (typeof candidate.pending === 'boolean') {
            transactionPayload.pending = candidate.pending;
          }

          if (typeof candidate.plaidTransactionId === 'string') {
            transactionPayload.plaidTransactionId = candidate.plaidTransactionId;
          }

          if (typeof candidate.merchant === 'string') {
            transactionPayload.merchant = candidate.merchant;
          }

          if (typeof candidate.paymentChannel === 'string') {
            transactionPayload.paymentChannel = candidate.paymentChannel;
          }

          if (candidate.location) {
            transactionPayload.location = candidate.location;
          }

          if (typeof candidate.goalId === 'string') {
            transactionPayload.goalId = candidate.goalId;
          }

          if (typeof candidate.recurringTransactionId === 'string') {
            transactionPayload.recurringTransactionId = candidate.recurringTransactionId;
          }

          if (typeof candidate.addedBy === 'string') {
            transactionPayload.addedBy = candidate.addedBy;
          }

          if (typeof candidate.linkedTransferId === 'string') {
            transactionPayload.linkedTransferId = candidate.linkedTransferId;
          }

          if (candidate.transferMetadata) {
            transactionPayload.transferMetadata = candidate.transferMetadata;
          }

          if (candidate.investmentData) {
            transactionPayload.investmentData = candidate.investmentData;
          }
        ...
        const accountResult: ImportResult = {
          success: 0,
          failed: 0,
          duplicates: 0,
          items: [],
        ...
        for (let rowIndex = 0; rowIndex < data.length; rowIndex++) {
          ...
          try {
            const mappedName = ensureString(resolveValue(row, 'name')).trim();
            const mappedType = ensureString(resolveValue(row, 'type')).trim();

            if (!mappedName || !mappedType) {
              throw new Error('Missing account name or type');
            }

            const balanceValue = parseNumericValue(resolveValue(row, 'balance'));
            const rawCurrency = ensureString(resolveValue(row, 'currency'));
            const currencyValue = rawCurrency.trim() || 'GBP';
            const institutionValue = ensureString(resolveValue(row, 'institution')).trim();

            const payload: Omit<Account, 'id'> & { initialBalance?: number } = {
              name: mappedName,
              type: normalizeAccountType(mappedType),
            ...
            accountResult.success += 1;
            accountResult.items.push({
              name: mappedName,
              type: payload.type,
              balance: balanceValue,
            ...
            accountResult.failed += 1;
            accountResult.errors.push({
              row: rowIndex + 2,
              error: error instanceof Error ? error.message : 'Unknown account import error'
            });
          }
        }

        setImportResult(accountResult);
      }
                      ...
                      <tr key={rowIndex}>
                        {targetFields
                          ...
                          .map(field => {
                            ...
                            if (!mapping) {
                              return <td key={field} className="px-4 py-2 text-sm" />;
                            }

                            const columnIndex = headers.indexOf(mapping.sourceColumn);
                            const rawValue = columnIndex >= 0 ? row[columnIndex] ?? '' : '';
                            const transformed = mapping.transform ? mapping.transform(rawValue) : rawValue;
                            const displayValue = transformed === null || transformed === undefined ? '' : String(transformed);
