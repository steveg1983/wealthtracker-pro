...
type SerializedSavedSearch = Omit<SavedSearch, 'createdAt' | 'filters'> & {
  createdAt: string;
  filters: Partial<{
    categories?: string[];
    accounts?: string[];
...
const isSerializedSavedSearchArray = (
  value: unknown
): value is SerializedSavedSearch[] =>
  Array.isArray(value) &&
  value.every(item =>
    typeof item === 'object' &&
    item !== null &&
    typeof (item as SerializedSavedSearch).id === 'string' &&
    typeof (item as SerializedSavedSearch).name === 'string' &&
    typeof (item as SerializedSavedSearch).query === 'string' &&
    typeof (item as SerializedSavedSearch).createdAt === 'string' &&
    typeof (item as SerializedSavedSearch).usageCount === 'number'
  );

const deserializeSavedSearch = (serialized: SerializedSavedSearch): SavedSearch => {
  const { filters } = serialized;
  const normalizedFilters: Partial<SavedSearchFilters> = {};

  if (filters.categories) {
    normalizedFilters.categories = filters.categories;
  }

  if (filters.accounts) {
    normalizedFilters.accounts = filters.accounts;
  }

  if (filters.amountRange) {
    normalizedFilters.amountRange = filters.amountRange;
  }

  if (filters.types) {
    normalizedFilters.types = filters.types;
  }
...
const serializeSavedSearch = (savedSearch: SavedSearch): SerializedSavedSearch => {
  const { filters } = savedSearch;

  const serializedFilters: SerializedSavedSearch['filters'] = {};

  if (filters.categories) {
    serializedFilters.categories = filters.categories;
  }

  if (filters.accounts) {
    serializedFilters.accounts = filters.accounts;
  }

  if (filters.amountRange) {
    serializedFilters.amountRange = filters.amountRange;
  }

  if (filters.types) {
    serializedFilters.types = filters.types;
  }

  if (filters.dateRange) {
    serializedFilters.dateRange = {
...
export function QuickFiltersAndSearch({ 
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        if (isSerializedSavedSearchArray(parsed)) {
          setSavedSearches(parsed.map(deserializeSavedSearch));
        }
      } catch (error) {
        logger.error('Failed to parse saved searches:', error);
      }
    }
  ...
  useEffect(() => {
    const serialized = savedSearches.map(serializeSavedSearch);
    localStorage.setItem('wealthtracker_savedSearches', JSON.stringify(serialized));
  }, [savedSearches]);
  ...
  const searchSuggestions = useMemo(() => {
    ...
    transactions.forEach(t => {
      if (t.description.toLowerCase().includes(query)) {
        ...
        if (merchant && merchant.toLowerCase().includes(query)) {
