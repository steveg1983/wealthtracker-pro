...
type ResultGroupKey = SearchResultType | 'other';
...
type SearchItem = SearchResult | GroupHeader;

const isGroupHeader = (item: SearchItem): item is GroupHeader =>
  (item as GroupHeader).isHeader === true;
...
export function VirtualizedSearchResults({
  ...
  const processedResults = useMemo<SearchItem[]>(() => {
    if (!groupByType) return results;

    const grouped = results.reduce<Record<ResultGroupKey, SearchResult[]>>((acc, result) => {
      const resultType: ResultGroupKey = result.type ?? 'other';
    ...
    const entries = Object.entries(grouped).filter(([, items]) => items.length > 0) as Array<[
      ResultGroupKey,
      SearchResult[]
    ]>;
    ...
    entries.forEach(([type, items]) => {
      const header: GroupHeader = { isHeader: true, type, count: items.length };
  ...
  const highlightMatch = useCallback((text: string, currentQuery: string) => {
  ...
  const renderResult = useCallback(
    (item: SearchItem, _index: number, isScrolling = false) => {
                ...
                <div className="flex-shrink-0 text-right">
                  ...
                  {showRelevanceScore && result.relevanceScore !== undefined && (
                    <div className="text-xs text-gray-400 mt-1">
                      {formatPercentageFromRatio(result.relevanceScore, 0)} match
                    </div>
                  )}
                </div>
              </div>
...
export function VirtualizedQuickSearch({
  ...
  const renderQuickResult = useCallback((result: SearchResult) => (
    <div
