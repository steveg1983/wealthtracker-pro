...
export default function AccountReconciliationModal({ 
  ...
  // Form state
  const defaultReconciliationDate = (() => {
    const isoString = new Date().toISOString();
    const [datePart] = isoString.split('T');
    return datePart ?? isoString;
  })();

  const [reconciliationDate, setReconciliationDate] = useState<string>(defaultReconciliationDate);
  const [statementBalance, setStatementBalance] = useState<string>('');
  const [category, setCategory] = useState<string>('cat-blank'); // Default to Blank category
  const [subCategory, setSubCategory] = useState<string>('sub-other-expense');
  ...
  const statementBalanceDecimal = parseCurrencyDecimal(statementBalance);
  const statementBalanceNum = statementBalanceDecimal.toNumber();
  const differenceDecimal = statementBalanceDecimal.minus(toDecimal(systemBalance));
  const difference = differenceDecimal.toNumber();
  const differenceAbsDecimal = differenceDecimal.abs();
  const differenceAbs = differenceAbsDecimal.toNumber();
  
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    
    if (differenceDecimal.abs().lessThan(toDecimal(0.01))) {
    ...
    const adjustmentTransaction = {
      ...
      amount: toStorageNumber(differenceDecimal.abs()),
      type: differenceDecimal.greaterThan(0) ? 'income' as const : 'expense' as const,
      category: category,
      accountId: accountId,
  ...
  const transactionType = differenceDecimal.greaterThan(0) ? 'income' : 'expense';
      ...
      <div className="bg-white dark:bg-gray-800 rounded-t-lg sm:rounded-lg shadow-xl w-full sm:max-w-lg max-h-[90vh] sm:max-h-[85vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6">
              ...
              <div className="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                ...
                {differenceAbs >= 0.01 && (
                  <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">
                    A {differenceDecimal.greaterThan(0) ? 'positive' : 'negative'} adjustment of {formatCurrency(differenceAbs, account.currency)} will be added to reconcile.
                  </p>
                )}
              </div>
            )}
          ...
          <div className="flex gap-3 mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
            ...
            >
              {differenceAbs < 0.01 ? 'Close' : 'Reconcile & Adjust'}
            </button>
          </div>
        </form>
