      ...
      switch (filter.id) {
        ...
        case 'amount_min':
          if (filter.value && (typeof filter.value === 'string' || typeof filter.value === 'number')) {
            const minDecimal = parseCurrencyDecimal(String(filter.value));
            if (!minDecimal.isNaN()) {
              const minAmount = minDecimal.toNumber();
        ...
        case 'amount_max':
          if (filter.value && (typeof filter.value === 'string' || typeof filter.value === 'number')) {
            const maxDecimal = parseCurrencyDecimal(String(filter.value));
            if (!maxDecimal.isNaN()) {
              const maxAmount = maxDecimal.toNumber();
    ...
    setFilters(prev => {
      ...
      search.filters.forEach(savedFilter => {
        ...
        if (index !== -1) {
          ...
          if (currentFilter) {
            updated[index] = { ...currentFilter, value: savedFilter.value };
          }
        }
      });
      return updated;
