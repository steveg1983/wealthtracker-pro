...
import type { AppContextType, AppState, Tag } from './AppContextSupabase.types';
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  const { user: rawUser, isLoaded } = useUser() as {
  ...
  const updateDebounceRef = useRef<TimeoutHandle | null>(null);
  ...
  useEffect(() => {
    ...
    const handleIdReconciled = (event: Event): void => {
      const detail = (event as CustomEvent<{ entity: 'account'; tempId: string; permanentId: string }>).detail;
      if (!detail || detail.entity !== 'account') {
        return;
      }
      ...
      setAccounts(prev => {
        ...
        const next = prev.map(account => {
          if (account.id === detail.tempId) {
            changed = true;
            return { ...account, id: detail.permanentId };
          }
          return account;
        });
      ...
      setTransactions(prev => {
        ...
        const next = prev.map(transaction => {
          if (transaction.accountId === detail.tempId) {
            changed = true;
            return { ...transaction, accountId: detail.permanentId };
          }
          return transaction;
        });
        return changed ? next : prev;
      ...
      setCategories(prev => {
        ...
        const next = prev.map(category => {
          if (category.accountId === detail.tempId) {
            changed = true;
            return { ...category, accountId: detail.permanentId };
          }
          return category;
        });
        return changed ? next : prev;
      ...
      setGoals(prev => {
        ...
        const next = prev.map(goal => {
          ...
          if (goal.accountId === detail.tempId) {
            updates.accountId = detail.permanentId;
          }

          if (goal.linkedAccountIds?.some(id => id === detail.tempId)) {
            updates.linkedAccountIds = goal.linkedAccountIds.map(id =>
              id === detail.tempId ? detail.permanentId : id,
            );
          }
    ...
    const listener: EventListener = (event) => {
      handleIdReconciled(event);
    };

    window.addEventListener('auto-sync-id-reconciled', listener);
    return () => {
      window.removeEventListener('auto-sync-id-reconciled', listener);
    };
  }, [setAccounts, setTransactions, setCategories, setGoals]);
    ...
    const initializeData = async () => {
      ...
      try {
        ...
        if (user) {
          ...
          if (!clerkId) {
            logger.warn('[AppContext] Clerk user ID missing, skipping initialization');
            setAccounts([]);
            setInvestments([]);
            return;
          }

          const activeClerkId = clerkId as string;
          ...
          if (databaseId) {
            ...
            await AutoSyncService.initialize(activeClerkId);
        ...
        if (DataService.isUsingSupabase() && clerkId) {
          const resolvedClerkId = clerkId;
          ...
          const debouncedUpdate = (updateType: string, updateFn: () => Promise<void>) => {
            ...
            if (updateDebounceRef.current) {
              globalThis.clearTimeout(updateDebounceRef.current);
            }
            
            // Set a new debounced update
            updateDebounceRef.current = globalThis.setTimeout(async () => {
          ...
          const unsubscribeAccountsPromise = SimpleAccountService.subscribeToAccountChanges(
            ...
            async (payload: RealtimePostgresChangesPayload<AccountRow>) => {
              ...
              debouncedUpdate('account', async () => {
                ...
                const updatedAccounts = await SimpleAccountService.getAccounts(resolvedClerkId);
  ...
  const addAccount = useCallback(async (account: Omit<Account, 'id'> & { initialBalance?: number }) => {
    try {
      ...
      const newAccount = await SimpleAccountService.createAccount(clerkId, accountToCreate);
  ...
  const normalizeDate = useCallback((value: Date | string | undefined): Date | undefined => {
    if (!value) {
      return undefined;
    }
