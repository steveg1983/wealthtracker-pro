...
import type { RealtimeSyncContextType, RealtimeSyncProviderProps } from './RealtimeSyncProvider.types';
...
export function RealtimeSyncProvider({ 
  ...
  const portfolioValueRef = useRef<number | null>(null);
  ...
  const checkForAnomalies = useCallback(async (newTransaction?: Transaction) => {
    try {
      ...
      if (transactions.length > 0 && (newTransaction || timeSinceLastCheck > 5 * 60 * 1000)) {
        ...
        // Filter to only show high and medium severity anomalies
        ...
        const newAnomalies = significantAnomalies.filter(
          a => !recentAnomalies.some(existing => existing.id === a.id)
        );
        
        // Show notifications for new anomalies
        if (showNotifications && newAnomalies.length > 0) {
          newAnomalies.forEach(anomaly => {
            const message = `Unusual spending detected: ${anomaly.description}`;
  ...
  const updatePortfolioMetrics = useCallback(() => {
    try {
      ...
      const investmentAccounts = accounts.filter(
        acc => acc.type === 'investment' || acc.type === 'asset'
      );
      
      if (investmentAccounts.length > 0) {
        const currentValueDecimal = investmentAccounts.reduce(
          (sum: DecimalInstance, acc: Account) => sum.plus(toDecimal(acc.balance)),
          toDecimal(0)
        );
        
        // Calculate 24h change if we have a previous value
        if (portfolioValueRef.current && portfolioValueRef.current !== 0) {
          const previousValueDecimal = toDecimal(portfolioValueRef.current);
          if (!previousValueDecimal.isZero()) {
            const changeDecimal = currentValueDecimal
              .minus(previousValueDecimal)
              .dividedBy(previousValueDecimal)
              .times(100);
            const changeRounded = changeDecimal.toDecimalPlaces(2, Decimal.ROUND_HALF_UP);
            setPortfolioChange24h(changeRounded.toNumber());
            
            // Show notification for significant changes
            if (showNotifications && changeDecimal.abs().greaterThan(2)) {
              const isPositive = changeDecimal.greaterThanOrEqualTo(0);
              const message = `Portfolio ${isPositive ? 'up' : 'down'} ${changeDecimal.abs()
        ...
        portfolioValueRef.current = currentValueDecimal.toNumber();
      }
    } catch (error) {
