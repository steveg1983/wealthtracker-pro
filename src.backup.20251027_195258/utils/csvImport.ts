...
export function importTransactionsFromCSV(
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    try {
      ...
      const amountDecimal = parseCurrencyDecimal(amountStr).abs();
...
export function importAccountsFromCSV(content: string): Omit<Account, 'id' | 'lastUpdated'>[] {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    try {
      ...
      const balanceStrRaw = balanceIndex >= 0 ? row[balanceIndex] ?? '0' : '0';
      const balanceDecimal = parseCurrencyDecimal(balanceStrRaw);
      const balanceUsesParentheses = balanceStrRaw.includes('(') && balanceStrRaw.includes(')');
      const normalizedBalanceDecimal = balanceUsesParentheses ? balanceDecimal.abs() : balanceDecimal;
      const balance = toStorageNumber(normalizedBalanceDecimal);
