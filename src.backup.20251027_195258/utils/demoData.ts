...
export const generateDemoTransactions = (count: number = 50) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const baseRandom = toDecimal(Math.random());
    const expenseAmountDecimal = baseRandom
      .times(toDecimal(500).minus(10))
      .plus(10)
      .toDecimalPlaces(2, Decimal.ROUND_HALF_UP);
    const incomeAmountDecimal = baseRandom
      .times(toDecimal(3000).minus(1000))
      .plus(1000)
      .toDecimalPlaces(2, Decimal.ROUND_HALF_UP);
    const amount = (isExpense ? expenseAmountDecimal.negated() : incomeAmountDecimal).toNumber();
