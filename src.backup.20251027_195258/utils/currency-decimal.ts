...
const CURRENCY_SANITIZE_REGEX = /[^0-9.-]/g;

const isTrivialNumericString = (value: string): boolean => {
  const trimmed = value.trim();
  return trimmed === '' || trimmed === '-' || trimmed === '.' || trimmed === '-.';
};

const normalizeDecimalPoint = (value: string): string => {
  const firstDot = value.indexOf('.');
  if (firstDot === -1) {
    return value;
  }
  const beforeDot = value.slice(0, firstDot + 1);
...
export function parseCurrencyDecimal(value: string | number | null | undefined): DecimalInstance {
  ...
  const withoutSpaces = withoutParens.replace(/\s+/g, '');
  const usesCommaAsDecimal = withoutSpaces.includes(',') && !withoutSpaces.includes('.');
  const withNormalizedSeparators = usesCommaAsDecimal
    ? withoutSpaces.replace(/,/g, '.')
    : withoutSpaces.replace(/,/g, '');
  const stripped = withNormalizedSeparators.replace(CURRENCY_SANITIZE_REGEX, '');

  if (isTrivialNumericString(stripped)) {
    return new Decimal(0);
  }

  const hadLeadingMinus = stripped.startsWith('-');
  let magnitude = hadLeadingMinus ? stripped.slice(1) : stripped;
  magnitude = magnitude.replace(/-/g, '');
  magnitude = normalizeDecimalPoint(magnitude);

  if (isTrivialNumericString(magnitude)) {
    return new Decimal(0);
  }

  let normalized = magnitude;
  if (hadLeadingMinus || hasParentheses) {
  ...
  try {
    ...
    logger.warn('Failed to parse currency value', { value, normalized, error: String(error) });
    return new Decimal(0);
  }
}

export function parseCurrencyToNumber(value: string | number | null | undefined): number {
  return parseCurrencyDecimal(value).toNumber();
}
