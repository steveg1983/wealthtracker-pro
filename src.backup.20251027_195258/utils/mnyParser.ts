...
import { Decimal, toDecimal, toStorageNumber } from '@wealthtracker/utils';
...
export async function parseMNY(arrayBuffer: ArrayBuffer): Promise<ParseResult> {
  ...
  for (let offset = 0; offset < arrayBuffer.byteLength - recordSize && potentialRecords.length < maxRecords; offset += recordSize) {
    ...
    if (offset % (1024 * 1024) === 0 && offset > 0) {
      const progressDecimal = toDecimal(offset)
        .dividedBy(1024 * 1024)
        .toDecimalPlaces(1, Decimal.ROUND_HALF_UP);
      logger.debug('Scan progress', { mb: progressDecimal.toNumber() });
...
export function applyMappingToData(rawData: Array<Record<string, unknown>>, mapping: FieldMapping): { accounts: ParsedAccount[], transactions: ParsedTransaction[] } {
  ...
  rawData.forEach(record => {
    try {
      ...
      // Get amount
      const rawAmountValue = typeof amountField === 'number' ? amountField : String(amountField ?? '');
      const hasNumericContent =
        typeof rawAmountValue === 'number' ||
        (typeof rawAmountValue === 'string' && /[0-9]/.test(rawAmountValue));
      if (!hasNumericContent) {
        return;
      }
      const amountDecimal = parseCurrencyDecimal(rawAmountValue);
      if (amountDecimal.isNaN()) return;
      const amount = toStorageNumber(amountDecimal);
...
export async function parseMBF(arrayBuffer: ArrayBuffer): Promise<ParseResult> {
  ...
  logger.debug('Readable text percentage', {
    percent: toDecimal(textPercentage)
      .toDecimalPlaces(1, Decimal.ROUND_HALF_UP)
      .toNumber(),
  });
