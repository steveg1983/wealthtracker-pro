...
import { parseCurrencyDecimal } from './currency-decimal';
    ...
    if (line === '!Account') {
      ...
      while (i < lines.length && !lines[i]?.startsWith('!')) {
        ...
        if (accountLine.startsWith('N')) {
          ...
          balance = toStorageNumber(parseCurrencyDecimal(balanceStr));
    ...
    if (line.startsWith('^')) {
      ...
      if (currentTransaction.date && currentTransaction.amount !== undefined) {
        ...
        if (currentTransaction.category) {
          ...
          if (colonMatch && colonMatch[1] && colonMatch[1].length < 50) {
            accountName = colonMatch[1];
            currentTransaction.category = currentTransaction.category.substring(colonMatch[0]?.length || 0).trim();
          }
        }
        ...
        const transaction: ParsedTransaction = {
          ...
          ...(currentTransaction.payee && { payee: currentTransaction.payee }),
      ...
      currentTransaction.amount = toStorageNumber(parseCurrencyDecimal(amountStr));
...
function parseQIFDate(dateStr: string): Date {
  ...
  if (!date || isNaN(date.getTime())) {
    ...
    if (dashParts.length === 3 && dashParts[0] && dashParts[1] && dashParts[2]) {
  ...
  if (!date || isNaN(date.getTime())) {
    ...
    if (ukParts.length === 3 && ukParts[0] && ukParts[1] && ukParts[2]) {
  ...
  if (!date || isNaN(date.getTime())) {
    ...
    if (isoParts.length === 3 && isoParts[0] && isoParts[0].length === 4 && isoParts[1] && isoParts[2]) {
