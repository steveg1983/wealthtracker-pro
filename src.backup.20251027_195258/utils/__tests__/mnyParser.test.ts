...
describe('mnyParser', () => {
  ...
  describe('parseMNY', () => {
    ...
    it('handles large files with progress logging', async () => {
      ...
      expect(debugSpy).toHaveBeenCalledWith(
        'Scan progress',
        expect.objectContaining({ mb: expect.any(Number) })
      );
    });
  ...
  describe('applyMappingToData', () => {
    ...
    it('handles errors gracefully', () => {
      const badData: Array<typeof mockRawData[number] | null | undefined | Record<string, unknown>> = [
        ...mockRawData,
        null,
        undefined,
        {}
  ...
  describe('parseMBF', () => {
    ...
    it('logs file header for debugging', async () => {
      ...
      const headerCall = debugSpy.mock.calls.find(([message]) => message === 'MBF file header');
    ...
    it('calculates text percentage in file', async () => {
      ...
      const percentageCall = debugSpy.mock.calls.find(
        ([message, payload]) =>
          message === 'Readable text percentage' &&
          typeof payload === 'object' &&
          payload !== null &&
          'percent' in payload &&
          Math.abs(Number((payload as { percent: number }).percent) - 50) < 0.5
      );
    ...
    it('tries pattern-based extraction as fallback', async () => {
      ...
      const patternCall = debugSpy.mock.calls.find(
        ([message]) => message === 'Trying pattern-based extraction...'
      );
