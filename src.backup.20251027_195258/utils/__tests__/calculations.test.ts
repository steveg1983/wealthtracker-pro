...
import { createMockAccount, createMockTransaction, createMockBudget, createMockGoal } from '../../test/utils/renderWithProviders';
...
describe('Calculation Utilities', () => {
  ...
  describe('calculateBudgetProgress', () => {
    it('calculates budget progress percentage correctly', () => {
      const budget = createMockBudget({ category: 'groceries', amount: 500, spent: undefined });
      const transactions = [
    ...
    it('handles overspending', () => {
      ...
      expect(calculateBudgetProgress(budget, transactions).percentage).toBe(100);
    });

    it('returns 0 when budget amount is 0', () => {
      const budget = createMockBudget({ category: 'groceries', amount: 0, spent: undefined });
      const transactions = [
        createMockTransaction({ category: 'groceries', amount: 100, type: 'expense' })
      ];
  ...
  describe('calculateGoalProgress', () => {
    ...
    it('returns 0 when target amount is 0', () => {
      ...
      expect(calculateGoalProgress(goal).percentage).toBe(0);
    });
  });
  ...
  describe('getAccountTypeIcon', () => {
    ...
    it('returns default icon for unknown account types', () => {
      expect(getAccountTypeIcon('unknown-type')).toBe('💰');
    });
  });
});

describe.skip('Pending aggregate calculation helpers', () => {
  it('awaits fixture coverage for advanced aggregation helpers', () => {
    expect(calculateAverageTransactionAmount).toBeDefined();
    expect(calculateMonthlyAverage).toBeDefined();
    expect(groupTransactionsByDate).toBeDefined();
    expect(calculateAccountTotals).toBeDefined();
    expect(getRecentTransactions).toBeDefined();
    expect(getTopCategories).toBeDefined();
    expect(calculateDailyBalance).toBeDefined();
    expect(calculateMonthlyTrends).toBeDefined();
    expect(calculateCategoryTrends).toBeDefined();
  });
});
