...
export class ColorContrastChecker {
   ...
   */
  static hexToRgb(hex: string | null | undefined): RGB {
    if (!hex) {
    ...
    return {
      r: parseInt(rStr ?? '00', 16),
      g: parseInt(gStr ?? '00', 16),
      b: parseInt(bStr ?? '00', 16)
    };
  }
   ...
   */
  static getContrastRatio(color1: string | null | undefined, color2: string | null | undefined): number {
   ...
   */
  static checkContrast(foreground: string | null | undefined, background: string | null | undefined): ContrastResult {
    if (!foreground || !background) {
      return {
        ratio: 0,
        passes: {
          normal: { aa: false, aaa: false },
          large: { aa: false, aaa: false }
        },
        recommendation: 'Provide both foreground and background colors to evaluate contrast.'
      };
    }

    const ratio = this.getContrastRatio(foreground, background);
    
    const ratioDecimal = toDecimal(ratio).toDecimalPlaces(2, Decimal.ROUND_HALF_UP);
    const result: ContrastResult = {
      ratio: ratioDecimal.toNumber(),
      passes: {
        normal: {
          aa: ratio >= 4.5,
          aaa: ratio >= 7
...
interface WindowWithColorTools extends Window {
...
if (process.env.NODE_ENV === 'development' && typeof window !== 'undefined') {
  const globalWindow = window as WindowWithColorTools;

  globalWindow.ColorContrastChecker = ColorContrastChecker;
  globalWindow.auditColorContrast = auditColorContrast;
}
