...
export function exportTransactionsToCSV(
  ...
  const rows = transactions.map(t => {
    ...
    const amountValue = typeof t.amount === 'number' ? t.amount : t.amount?.toString();
    const amountDecimal = toDecimal(amountValue);
    ...
    return [
      ...
      t.category,
      t.type,
      amountDecimal.toDecimalPlaces(2).toString(), // Always export with 2 decimal places
...
export function exportAccountsToCSV(accounts: Account[] | DecimalAccount[]): string {
  ...
  const rows = accounts.map(a => {
    const balanceValue = typeof a.balance === 'number' ? a.balance : a.balance?.toString();
    const balanceDecimal = toDecimal(balanceValue);
    
    return [
      a.name,
      a.type,
      balanceDecimal.toDecimalPlaces(2).toString(),
      a.currency || 'GBP',
      a.institution || '',
