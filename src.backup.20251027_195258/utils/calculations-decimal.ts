...
import { sumDecimals } from '@wealthtracker/utils';
...
export function calculateSpendingByCategory(
  ...
  filtered.forEach(t => {
    ...
    const bucket = spending[key] ?? (spending[key] = new Decimal(0));
    spending[key] = bucket.plus(t.amount);
  });
...
export function calculateAccountTotals(accounts: DecimalAccount[]): Record<string, DecimalInstance> {
  ...
  accounts.forEach(account => {
    ...
    const currentTotal = totals[key] ?? new Decimal(0);
    totals[key] = currentTotal.plus(account.balance);
  });
...
export function getTopCategories(
  ...
  transactions
    ...
    .forEach(t => {
      ...
      const bucket = categoryTotals[key] ?? (categoryTotals[key] = { total: new Decimal(0), count: 0 });
