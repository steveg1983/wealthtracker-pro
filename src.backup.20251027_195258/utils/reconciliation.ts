...
export const getReconciliationSummary = (accounts: Account[], transactions: Transaction[]): ReconciliationSummary[] => {
  ...
  const reconciliationDetails = accounts.map(account => {
    ...
    const totalToReconcileDecimal = unclearedTransactions.reduce((sum, transaction) => {
      const amountDecimal = parseCurrencyDecimal(transaction.amount);
      if (amountDecimal.isNaN()) {
        return sum;
      }
      return sum.plus(amountDecimal.abs());
    }, parseCurrencyDecimal(0));
    const totalToReconcile = toNumber(totalToReconcileDecimal);
