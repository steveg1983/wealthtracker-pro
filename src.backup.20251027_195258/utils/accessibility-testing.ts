...
export class AccessibilityTester {
  ...
  private static checkHeadingHierarchy(root: HTMLElement): AccessibilityIssue[] {
    ...
    headings.forEach((heading, index) => {
      ...
      const level = parseInt(headingElement.tagName.slice(1), 10);
  ...
  private static checkColorContrast(root: HTMLElement): AccessibilityIssue[] {
    ...
    textElements.forEach(element => {
      ...
      if (bgColor && fgColor && bgColor !== 'transparent' && fgColor !== 'transparent') {
        ...
        const fontSizeMatch = styles.fontSize.match(/\d+(?:\.\d+)?/);
        const fontSize = fontSizeMatch ? toDecimal(fontSizeMatch[0]).toNumber() : 0;
        ...
        if (contrast.ratio < requiredRatio) {
          issues.push({
            ...
            message: `Insufficient color contrast ratio: ${toDecimal(contrast.ratio)
              .toDecimalPlaces(2, Decimal.ROUND_HALF_UP)
              .toString()}:1 (required: ${requiredRatio}:1)`,
   ...
   */
  static calculateContrast(foreground: string | null | undefined, background: string | null | undefined): ColorContrastResult {
   ...
   */
  private static parseColor(color: string | null | undefined): { r: number; g: number; b: number } {
  ...
  private static checkTables(root: HTMLElement): AccessibilityIssue[] {
    ...
    tables.forEach(table => {
      ...
      headers.forEach(header => {
        ...
        if (!headerEl.hasAttribute('scope')) {
          issues.push({
            element: headerEl,
            type: 'warning',
            category: 'Tables',
  ...
  static generateReport(issues: AccessibilityIssue[]): string {
    const grouped = issues.reduce((acc, issue) => {
      ...
      const categoryArray = acc[issue.category];
      if (categoryArray) {
