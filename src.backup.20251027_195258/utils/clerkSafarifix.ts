...
export const initClerkSafariCompat = async (): Promise<SafariCompatInfo> => {
  if (!isSafari()) {
  ...
  const results: SafariCompatInfo = {
    safari: true,
  ...
  if (!Promise.allSettled) {
    Promise.allSettled = function<T>(promises: Iterable<T | PromiseLike<T>>): Promise<PromiseSettledResult<Awaited<T>>[]> {
      const wrapped = Array.from(promises, (p) =>
        Promise.resolve(p)
          .then<PromiseSettledResult<Awaited<T>>>(value => ({ status: 'fulfilled', value: value as Awaited<T> } as const))
          .catch<PromiseSettledResult<Awaited<T>>>(reason => ({ status: 'rejected', reason } as const))
      );
