...
${r.name}:
  Mean: ${formatMs(r.mean)}ms
  Min: ${formatMs(r.min)}ms
  Max: ${formatMs(r.max)}ms
  P95: ${formatMs(r.p95)}ms
`).join('\n')}
`;
}
const formatMs = (value: number): string =>
  toDecimal(value).toDecimalPlaces(2, Decimal.ROUND_HALF_UP).toString();
