...
import { createTestStore } from '@wealthtracker/testing/create-test-store';
...
describeIfSupabase('Real-time Analytics Integration - REAL', () => {
  let store: AppStore;
  ...
  beforeEach(async () => {
    ...
    if (!TEST_EMAIL || !TEST_PASSWORD) {
      throw new Error('Set VITEST_SUPABASE_EMAIL and VITEST_SUPABASE_PASSWORD to run real-time analytics tests');
    }
    ...
    if (signInError) {
      throw new Error(`Failed to sign in realtime analytics test user: ${signInError.message}`);
    }

    const session = await supabase.auth.getSession();
    authUserId = signInData.user?.id ?? session.data.session?.user?.id ?? '';
    if (!authUserId) {
      throw new Error('Unable to resolve authenticated user id for realtime analytics tests');
    }

    const timestamp = new Date().toISOString();
    testDataIds.userId = authUserId;
    ...
    if (userError && userError.code !== '23505') {
      console.error('Failed to ensure test user:', userError);
    }
  ...
  describeSupabase('Anomaly Detection with REAL Data', () => {
