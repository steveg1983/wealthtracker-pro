...
import { formatDecimalFixed, toDecimal } from '@wealthtracker/utils';
...
const renderCurrency = (value: number): string => `£${formatDecimalFixed(value, 2)}`;
  ...
  return (
    <div>
      ...
      <div data-testid="accounts-list">
        {accounts.map(account => (
          <div key={account.id} data-testid={`account-${account.id}`}>
            <span>{account.name}: {renderCurrency(account.balance)}</span>
            <button 
      ...
      <div data-testid="total-balance">
        Total: {renderCurrency(accounts.reduce((sum, acc) => sum + acc.balance, 0))}
      </div>
    </div>
  );
};
  ...
  const addTransaction = () => {
    if (description && amount) {
      const newTransaction: Transaction = {
        ...
        amount: toDecimal(amount || 0).toNumber(),
        type,
      ...
      <div data-testid="transactions-list">
        {transactions.map(transaction => (
          <div key={transaction.id} data-testid={`transaction-${transaction.id}`}>
            {transaction.type}: {transaction.description} - {renderCurrency(transaction.amount)}
          </div>
        ))}
      </div>
      <div data-testid="transaction-summary">
        <div>Income: {renderCurrency(totalIncome)}</div>
        <div>Expenses: {renderCurrency(totalExpenses)}</div>
        <div>Net: {renderCurrency(totalIncome - totalExpenses)}</div>
      </div>
    </div>
  );
};
        ...
        {budgets.map(budget => {
          ...
          return (
            <div key={budget.id} data-testid={`budget-${budget.category}`}>
              <div>{budget.category}: {renderCurrency(budget.amount)}/month</div>
              <div>Spent: {renderCurrency(spent)} ({formatDecimalFixed(percentage, 0)}%)</div>
              <div>Remaining: {renderCurrency(remaining)}</div>
              <button 
...
describe('Critical Workflows Integration Tests', () => {
  ...
  describe('Budget Management Workflow', () => {
    it('should allow creating budgets and tracking spending', async () => {
      ...
      expect(groceriesBudget).toHaveTextContent('groceries: £500.00/month');
      ...
      expect(entertainmentBudget).toHaveTextContent('entertainment: £200.00/month');
