...
const logger = lazyLogger.getLogger('OfflineStorage');
...
const isSyncError = (value: unknown): value is SyncError => {
...
class OfflineStorageService {
    ...
    try {
      ...
      for (const operation of operations) {
        try {
          ...
          // Handle conflict
          if (isSyncError(error) && error.status === 409) {
  ...
  private resolveAccountConflict(client: Account, server: Account): ConflictResolution<Account> {
    ...
    // Check if client has an original balance snapshot
    const clientWithSnapshot = client as Account & { originalBalance?: number };
    const originalBalance = clientWithSnapshot.originalBalance ?? 0;
...
export const useOfflineOperations = () => {
  ...
  const resolveConflict = React.useCallback(
    (conflictId: string, resolution: unknown) =>
