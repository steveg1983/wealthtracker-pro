...
import { mapSupabaseAccountToDomain } from '../../services/api/accountService';
    ...
    const fetchDeletedAccounts = async () => {
      ...
      try {
        ...
        if (error) {
          ...
          const transformedAccounts = (data || []).map(mapSupabaseAccountToDomain);
  ...
  return (
    <PageWrapper title="Deleted Accounts">
      <div className="mb-6">
        ...
        >
          <ProfessionalIcon name="arrowLeft" size={20} />
          Back to Settings
        </button>
      </div>
      ...
      <div className="bg-amber-50 dark:bg-amber-900/20 rounded-xl p-6 shadow-md border-l-4 border-amber-400 dark:border-amber-600 mb-6">
        <div className="flex items-start gap-3">
          <ProfessionalIcon name="archive" size={24} className="text-amber-600 dark:text-amber-400 mt-1" />
          <div>
        ...
        <div className="p-6">
          {loading || userIdLoading ? (
            ...
            <div className="text-center py-12">
              <ProfessionalIcon name="archive" size={48} className="mx-auto text-gray-300 dark:text-gray-600 mb-4" />
              ...
              {deletedAccounts.map((account) => (
                ...
                >
                  ...
                  <button
                    ...
                    `}
                  >
                    <ProfessionalIcon name="refresh" size={16} className={restoringId === account.id ? 'animate-spin' : ''} />
      ...
      {deletedAccounts.length > 0 && (
        <div className="mt-6 p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg">
          <div className="flex items-start gap-3">
            <ProfessionalIcon name="warning" size={20} className="text-yellow-600 dark:text-yellow-400 mt-1" />
            <div>
              <p className="text-sm text-gray-600 dark:text-gray-400">
