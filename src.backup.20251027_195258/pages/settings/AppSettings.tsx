...
import type { ProfessionalIconName } from '../../components/icons/ProfessionalIcons';
...
export default function AppSettings() {
  ...
  const themeOptions: Array<{ value: 'light' | 'dark' | 'auto' | 'scheduled'; label: string; iconName: ProfessionalIconName }> = [
    { value: 'light', label: 'Light', iconName: 'sun' },
    { value: 'dark', label: 'Dark', iconName: 'moon' },
    { value: 'auto', label: 'Auto', iconName: 'desktop' },
    { value: 'scheduled', label: 'Scheduled', iconName: 'clock' },
  ];
  ...
  const handleCheckTaxUpdates = async () => {
    ...
    try {
      ...
      console.error('Tax update check failed', error);
      setTaxUpdateResult({
        status: 'error',
  ...
  return (
    <PageWrapper 
      ...
      rightContent={
        ...
        >
          <ProfessionalIcon name="arrowLeft" size={16} />
        </button>
      }
    >
      ...
      <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl rounded-2xl shadow-lg border border-white/20 dark:border-gray-700/50 p-6 mb-6 mt-6">
        <div className="flex items-center gap-3 mb-4">
          <ProfessionalIcon name="globe" className="text-gray-600 dark:text-gray-400" size={20} />
      ...
      <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl rounded-2xl shadow-lg border border-white/20 dark:border-gray-700/50 p-6 mb-6">
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-3 gap-3">
            {themeOptions.map(({ value, label, iconName }) => (
              <button
                key={value}
              ...
              >
                <ProfessionalIcon name={iconName} size={20} />
                <span>{label}</span>
              </button>
            ))}
          </div>
        ...
        {theme === 'scheduled' && (
          <div className="mb-6 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
            <div className="flex items-center gap-2 mb-3">
              <ProfessionalIcon name="clock" size={20} className="text-primary" />
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                ...
                {taxUpdateResult.verificationStatus === 'valid' && (
                  <ProfessionalIcon name="success" size={16} className="text-green-500" />
                )}
                {taxUpdateResult.verificationStatus === 'needs_update' && (
                  <ProfessionalIcon name="warning" size={16} className="text-yellow-500" />
                )}
                {taxUpdateResult.verificationStatus === 'invalid' && (
                  <ProfessionalIcon name="close2" size={16} className="text-red-500" />
                )}
              </div>
