...
import { ArrowLeftIcon, SearchIcon, PlusIcon, CalendarIcon, BanknoteIcon, FileTextIcon, TagIcon, ArrowRightLeftIcon, XIcon, SettingsIcon, MinimizeIcon, MaximizeIcon } from '../components/icons';
import EditTransactionModal from '../components/EditTransactionModal';
import CategorySelector from '../components/CategorySelector';
import { usePreferences } from '../contexts/PreferencesContext';
import { VirtualizedTable } from '../components/VirtualizedTable';
import type { Column, VirtualizedTableProps } from '../components/VirtualizedTable';
import type { Transaction } from '../types';

type TransactionWithBalance = Transaction & { balance: number };

const TransactionTable = VirtualizedTable as React.ComponentType<VirtualizedTableProps<TransactionWithBalance>>;
...
export default function AccountTransactions() {
  ...
  const transactionsWithBalance = useMemo<TransactionWithBalance[]>(() => {
    if (!account) {
      return [] as TransactionWithBalance[];
    }
    ...
    return result as TransactionWithBalance[];
  ...
  const handleTransactionClick = useCallback((transaction: TransactionWithBalance) => {
  ...
  const handleQuickAdd = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!account || !quickAddForm.description || !quickAddForm.amount || !quickAddForm.date) {
      return;
    }
    ...
    const parsedAmount = parseCurrencyDecimal(quickAddForm.amount);
    if (parsedAmount.isNaN()) {
      return;
    }

    let amountDecimal = parsedAmount;
    if (quickAddForm.type === 'expense') {
      amountDecimal = amountDecimal.abs().negated();
    ...
    const baseCategory = quickAddForm.type === 'transfer'
      ? 'transfer'
    ...
    if (quickAddForm.notes) {
      transactionData.notes = quickAddForm.notes;
    }

    if (quickAddForm.type === 'transfer') {
      transactionData.transferMetadata = {
    ...
    if (quickAddForm.type === 'transfer' && quickAddForm.category && newTransaction) {
      await addTransaction({
        ...
        category: 'transfer',
        tags: quickAddForm.tags,
        notes: `Transfer from ${account.name}${quickAddForm.notes ? '\n' + quickAddForm.notes : ''}`,
        cleared: false,
        pending: false
      });
    }
