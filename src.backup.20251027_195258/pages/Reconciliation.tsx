...
export default function Reconciliation() {
  ...
  const unclearedTransactions = useMemo(() => {
    ...
    if (dateTo) {
      filtered = filtered.filter(t => {
        ...
        return rawDate <= dateTo;
      });
    }
    
    // Apply amount filters
    if (amountMin) {
      const minDecimal = parseCurrencyDecimal(amountMin).abs();
      filtered = filtered.filter(t => toDecimal(t.amount).abs().greaterThanOrEqualTo(minDecimal));
    }
    if (amountMax) {
      const maxDecimal = parseCurrencyDecimal(amountMax).abs();
      filtered = filtered.filter(t => toDecimal(t.amount).abs().lessThanOrEqualTo(maxDecimal));
    }
  ...
  const updateSplitItem = (id: string, field: 'description' | 'category' | 'amount', value: string | number): void => {
    setSplitItems(splitItems.map(item => {
      ...
      if (field === 'amount') {
        const amountDecimal = typeof value === 'number' ? toDecimal(value) : parseCurrencyDecimal(value);
        return {
          ...item,
          amount: amountDecimal.toDecimalPlaces(2, Decimal.ROUND_HALF_UP).toNumber()
        };
      }
  ...
  const splitItemsTotal = useMemo(() => {
    return splitItems.reduce((sum, item) => sum.plus(toDecimal(item.amount)), toDecimal(0));
  }, [splitItems]);

  const splitDifferenceDecimal = useMemo(() => {
    if (!splittingTransaction) {
      return toDecimal(0);
    }
    return toDecimal(splittingTransaction.amount).minus(splitItemsTotal);
  }, [splittingTransaction, splitItemsTotal]);

  // Validate split amounts
  const validateSplitAmounts = (): boolean => {
    if (!splittingTransaction) return false;
    return splitDifferenceDecimal
      .abs()
      .lessThanOrEqualTo(toDecimal(0.01)); // Allow for small rounding differences
  };
                    ...
                    <div className="flex-1 grid grid-cols-12 gap-3">
                      ...
                      <div className="col-span-12 md:col-span-5">
                        ...
                        <CategorySelect
                          ...
                          className="w-full px-2 py-2 text-sm border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded focus:outline-none focus:ring-2 focus:ring-primary"
                          disabled={Boolean(isSplit)}
                          showMultiple={Boolean(isSplit)}
                        />
                      </div>
                    </div>
                ...
                <div className="space-y-1">
                  ...
                  <div className="text-sm">
                    ...
                    <span className={`font-medium ${
                      splitDifferenceDecimal.abs().lessThanOrEqualTo(toDecimal(0.01))
                        ? 'text-green-600' 
                        : 'text-red-600'
                    }`}>
                      {formatCurrency(splitDifferenceDecimal)}
                    </span>
                  </div>
                </div>
