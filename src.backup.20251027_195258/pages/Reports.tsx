...
export default function Reports() {
  ...
  const summary = useMemo(() => {
    const roundCurrency = (value: ReturnType<typeof toDecimal>) =>
    ...
    return {
      income: roundCurrency(incomeDecimal),
      expenses: roundCurrency(expensesDecimal),
      netIncome: roundCurrency(netIncomeDecimal),
  ...
  const exportToPDF = async (includeCharts: boolean = true) => {
    ...
    try {
      ...
      if (includeCharts) {
        ...
        generateSimplePDFReport(reportData);
      }
    } catch (error) {
  ...
  const handleBudgetExport = (data: BudgetExportRow[]) => {
    const csv = data.map(row => ({
      Category: row.categoryName,
      Budgeted: row.budgeted,
      Actual: row.actual,
      Variance: row.variance,
      'Variance %': formatPercentageValue(row.variancePercent, 1),
      Status: row.status
    }));
    ...
    <div>
        ...
        <div className="flex gap-2">
          {activeTab === 'overview' ? (
            <>
              ...
              >
                <ProfessionalIcon name="pdf" size={20} />
      ...
      <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl rounded-2xl shadow-lg border border-white/20 dark:border-gray-700/50 p-1 mb-6">
        <div className="flex space-x-1">
          ...
          >
            <ProfessionalIcon name="trendingUp" size={20} />
            Overview
          </button>
          <button
          ...
          >
            <ProfessionalIcon name="chartBar" size={20} />
            Budget Comparison
          </button>
          <button
      ...
      <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl rounded-2xl shadow-lg border border-white/20 dark:border-gray-700/50 p-4 mb-6">
        <div className="flex flex-wrap gap-4 items-center">
          <div className="flex items-center gap-2">
            <ProfessionalIcon name="calendar" className="text-gray-500" size={20} />
            <select
              value={dateRange}
          ...
          <div className="flex items-center gap-2">
            <ProfessionalIcon name="filter" className="text-gray-500" size={20} />
            <select
      ...
      <div className="grid gap-6">
        ...
        <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl rounded-2xl shadow-lg border border-white/20 dark:border-gray-700/50 p-6">
          ...
          <p className={`text-2xl font-bold ${
            summary.savingsRateDecimal.greaterThanOrEqualTo(20) ? 'text-green-600 dark:text-green-400' : 'text-yellow-600 dark:text-yellow-400'
          }`}>
            {isLoading ? (
              <SkeletonText className="w-20 h-8" />
            ) : (
              formatPercentageValue(summary.savingsRateDecimal, 1)
            )}
          </p>
        </div>
        </div>
        ...
        <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl rounded-2xl shadow-lg border border-white/20 dark:border-gray-700/50 p-6">
          <h2 className="text-lg font-semibold mb-4 flex items-center gap-2 text-theme-heading dark:text-white">
            <ProfessionalIcon name="chartPie" size={20} />
            Expense Categories
          </h2>
