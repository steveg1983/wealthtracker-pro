...
import { ProfessionalIcon, type ProfessionalIconName } from '../components/icons/ProfessionalIcons';
...
type DashboardLayouts = Record<'lg' | 'md' | 'sm', Layout[]>;
...
export default function DashboardDraggable(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    if (savedLayouts) {
      const parsed = JSON.parse(savedLayouts) as Partial<DashboardLayouts>;
      const mergedLayouts: DashboardLayouts = {
        lg: DEFAULT_LAYOUTS.lg.map(defaultItem => {
          const savedItem = parsed.lg?.find(item => item.i === defaultItem.i);
          return savedItem ? { ...savedItem, h: defaultItem.h } : defaultItem;
        }),
        md: DEFAULT_LAYOUTS.md.map(defaultItem => {
          const savedItem = parsed.md?.find(item => item.i === defaultItem.i);
          return savedItem ? { ...savedItem, h: defaultItem.h } : defaultItem;
        }),
        sm: DEFAULT_LAYOUTS.sm.map(defaultItem => {
          const savedItem = parsed.sm?.find(item => item.i === defaultItem.i);
          return savedItem ? { ...savedItem, h: defaultItem.h } : defaultItem;
        })
      };
  ...
  const budgetStatus = useMemo(() => {
    ...
    const percentageDecimal = activeBudgets.length > 0
      ? toDecimal(overBudget.length)
          .dividedBy(activeBudgets.length)
          .times(100)
          .toDecimalPlaces(0)
      : toDecimal(0);
  ...
  const widgetIconMap: Record<string, ProfessionalIconName> = {
    accounts: 'creditCard',
  ...
  const getWidgetIcon = (widgetId: string) => {
    ...
    return <ProfessionalIcon name={iconName} size={20} className="text-gray-400" />;
  };

  const handleLayoutChange = (_layout: Layout[], updatedLayouts: DashboardLayouts) => {
    ...
    if (widget.id === 'addTransaction') {
      return (
        <div 
          ...
          onClick={handleClick}
        >
          <ProfessionalIcon name="add" size={48} className="mb-3" />
      ...
      >
        ...
        {widget.empty ? (
          <div className="flex flex-col items-center justify-center py-8 text-gray-400">
            <ProfessionalIcon name="warning" size={32} className="mb-3" />
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <ProfessionalIcon
                name="success"
                size={16}
                className="text-secondary dark:text-primary flex-shrink-0"
              />
              <div>
