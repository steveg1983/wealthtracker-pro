...
export default function Accounts({ onAccountClick }: { onAccountClick?: (accountId: string) => void }) {
  ...
  const decimalAccounts = useMemo(() => accounts.map(account => {
    const { openingBalance, balance, holdings, ...rest } = account;
    ...
    const decimalHoldings = holdings?.map(holding => {
      ...
      return {
        ...holdingRest,
        shares: toDecimal(shares ?? 0),
        value: toDecimal(value ?? 0),
        ...(averageCost !== undefined ? { averageCost: toDecimal(averageCost) } : {}),
        ...(currentPrice !== undefined ? { currentPrice: toDecimal(currentPrice) } : {}),
        ...(marketValue !== undefined ? { marketValue: toDecimal(marketValue) } : {}),
        ...(gain !== undefined ? { gain: toDecimal(gain) } : {}),
        ...(gainPercent !== undefined ? { gainPercent: toDecimal(gainPercent) } : {})
      };
    });
    ...
    return {
      ...
      ...(openingBalance !== undefined
        ? {
            ...
            initialBalance: toDecimal(openingBalance)
          }
        : {}),
      ...(decimalHoldings ? { holdings: decimalHoldings } : {})
    };
  }), [accounts]);
        ...
        <BalanceAdjustmentModal
          ...
          onClose={() => {
            ...
            if (balanceAdjustmentData.accountId) {
              const newBalance = parseCurrencyDecimal(balanceAdjustmentData.newBalance).toNumber();
