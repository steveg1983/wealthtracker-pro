...
class EnhancedEncryptionService {
   ...
   * Get stored key data
   */
  private isStoredKeyData(value: unknown): value is StoredKeyData {
    if (!value || typeof value !== 'object') {
      return false;
    }

    const candidate = value as Partial<StoredKeyData>;
    return typeof candidate.key === 'string' &&
      typeof candidate.salt === 'string' &&
      typeof candidate.timestamp === 'number';
  }

  private async getStoredKeyData(): Promise<StoredKeyData | null> {
    try {
      // Try IndexedDB first
      const stored = await this.getFromIndexedDB('encryption_keys', 'master');
      if (this.isStoredKeyData(stored)) {
        return stored;
      }
    } catch {
    ...
    const storedValue = sessionStorage.getItem('wt_master_key');
    if (!storedValue) {
      return null;
    }
    ...
    try {
      ...
      return this.isStoredKeyData(parsed) ? parsed : null;
    } catch {
      return null;
    }
  }
   ...
   * Decrypt stored key
   */
  private async decryptStoredKey(keyData: StoredKeyData): Promise<CryptoJS.lib.WordArray> {
    const deviceKey = await this.getDeviceKey();
   ...
   */
  private async storeInIndexedDB(storeName: string, key: string, value: StoredKeyData): Promise<void> {
