...
export const getCSPDirectives = (nonce?: string): Record<string, string[]> => {
  const directives: Record<string, string[]> = {
    ...
    'script-src': [
      ...
      (typeof import.meta !== 'undefined' && import.meta.env && import.meta.env.MODE === 'development') ? "'unsafe-inline'" : '', // Only for dev
...
const logger = lazyLogger.getLogger('CSP');
...
export const setupCSPReporting = (): void => {
  ...
  document.addEventListener('securitypolicyviolation', (event) => {
    if (!(event instanceof SecurityPolicyViolationEvent)) {
      logger.error('CSP Violation:', event);
      return;
    }
    ...
    try {
      captureMessage('CSP Violation', 'warning', {
        ...
        documentURI: event.documentURI,
        effectiveDirective: event.effectiveDirective,
        lineNumber: String(event.lineNumber),
        columnNumber: String(event.columnNumber),
        sourceFile: event.sourceFile,
...
type MiddlewareHandler = (req: { url?: string }, res: { setHeader: (name: string, value: string) => void; locals?: Record<string, unknown> }, next: MiddlewareNext) => void;
...
export const viteCSPPlugin = (): ViteCSPPlugin => {
  return {
    name: 'csp-header',
...
export const cspMiddleware = (_req: { method?: string }, res: NodeResponse, next: NodeNext): void => {
  const nonce = generateNonce();
  const cspHeader = getCSPHeader(nonce);
  const securityHeaders = getSecurityHeaders();
