...
interface ClientData {
  clientId: string;
  lastSeen: Date;
  pendingOperations: OperationData[];
}
...
type SyncOperation = OperationData & {
  id: string;
...
type StoredEntityRecord = OperationData & {
  lastModified: Date;
};
...
class MockSyncServer {
  ...
  private dataStore: Map<string, StoredEntityRecord> = new Map();
  ...
  private setupSocketHandlers(): void {
    ...
    this.io.on('connection', (socket: Socket) => {
      ...
      socket.on('sync-operation', (operation: SyncOperation, callback: (result: { success: boolean; error?: string; operationId?: string }) => void) => {
        this.handleSyncOperation(socket, operation, callback);
      });

      // Handle conflict resolution
      socket.on('resolve-conflict', (resolution: ConflictResolutionPayload, callback: (result: { success: boolean }) => void) => {
  ...
  private checkForConflict(entityId: string, clientId: string, version: number): ConflictOperation | null {
  ...
  private applyOperation(operation: OperationData): void {
