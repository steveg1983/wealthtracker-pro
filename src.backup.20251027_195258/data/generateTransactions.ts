...
function randomBetween(min: number, max: number): number {
  const randomFraction = toDecimal(Math.random());
  const rangeDecimal = toDecimal(max).minus(toDecimal(min));
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    Object.entries(SPENDING_PATTERNS).forEach(([category, pattern]) => {
      ...
      if (shouldHappen && Math.random() < 0.8) {
        ...
        const cardId = pattern.cards[Math.floor(Math.random() * pattern.cards.length)] || 'default-account';
