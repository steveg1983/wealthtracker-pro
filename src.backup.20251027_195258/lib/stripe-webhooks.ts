...
async function handleCheckoutSessionCompleted(
  ...
  if (userId) {
    ...
    logger.warn('Checkout session completed without matching user', {
...
async function handleSubscriptionUpdate(
  ...
  await logSubscriptionEvent(user.id, event.type, subscription);
...
async function handleSubscriptionDeleted(
  ...
  // Log event
  await logSubscriptionEvent(user.id, 'subscription.canceled', subscription);
...
async function handleInvoicePaid(
  ...
  if (!stripeCustomerId) {
    logger.warn('Invoice paid webhook missing customer identifier', { invoiceId: invoice.id });
    return { success: true, message: 'Payment recorded (no customer mapping)' };
  }
  ...
  if (userLookupError || !user) {
    logger.warn('Invoice paid webhook could not resolve user', {
      invoiceId: invoice.id,
      stripeCustomerId
    });
    return { success: true, message: 'Payment recorded (user not found)' };
  }

  const invoiceId = typeof invoice.id === 'string' ? invoice.id : null;
  if (!invoiceId) {
    logger.warn('Invoice paid webhook missing invoice id', { invoice });
    return { success: true, message: 'Payment recorded (missing invoice id)' };
  }
  const invoiceCurrency: string = invoice.currency ?? 'usd';
  const invoiceStatus: string = invoice.status ?? 'paid';
...
async function handlePaymentFailed(
  ...
  // Log event
  await logSubscriptionEvent(user.id, 'payment.failed', invoice);
...
async function handlePaymentMethodDetached(
  _event: Stripe.PaymentMethodDetachedEvent
): Promise<WebhookHandlerResult> {
  // Note: event.data.object.customer is null after detachment
...
type UserSubscriptionStatus = Database['public']['Tables']['users']['Row']['subscription_status'];

function mapStripeStatus(stripeStatus: Stripe.Subscription.Status): UserSubscriptionStatus {
  switch (stripeStatus) {
    case 'active':
      return 'active';
    case 'trialing':
...
function getPlanFromSubscription(subscription: Stripe.Subscription): 'free' | 'pro' | 'business' {
  ...
  if (priceId && PRICE_IDS.pro.includes(priceId)) return 'pro';
  if (priceId && PRICE_IDS.business.includes(priceId)) return 'business';
  return 'free';
}
...
async function logSubscriptionEvent(
  ...
  try {
    ...
    const payload: Database['public']['Tables']['subscription_events']['Insert'] = {
      user_id: userId,
      event_type: eventType,
      payload: toJsonPayload(eventData),
...
const toJsonPayload = (value: unknown): Json => {
  ...
  if (Array.isArray(value)) {
    return value.map(item => toJsonPayload(item));
  }
  ...
  if (typeof value === 'object') {
    ...
    for (const [key, nested] of entries) {
      result[key] = toJsonPayload(nested);
    }
    return result;
  }
