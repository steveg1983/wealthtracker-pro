...
export function useSwipeGestures<T extends HTMLElement = HTMLElement>(
  ...
  const longPressTimer = useRef<TimeoutHandle>();
  ...
  const handleTouchStart = useCallback((e: TouchEvent) => {
    if (e.touches.length === 1) {
      ...
      longPressTimer.current = globalThis.setTimeout(() => {
...
export function useSwipeableItem(
  ...
  const { ref, swipeDistance, swipeDirection } = useSwipeGestures(swipeHandlers, {
    threshold: 75,
    preventScrollOnSwipe: true
  });
