...
import type { PreferenceValue } from '../types/cache-types';
...
export function useCachedSearch<T>({
  ...
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);
  const debounceTimer = useRef<TimeoutHandle>();
...
export function useCacheStats() {
  ...
  return {
    ...stats,
    clearCache,
    hitRate: `${toDecimal(stats.avgHitRate)
      .times(100)
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      const results = await Promise.allSettled(
        operations.map(op => 
          smartCache.get(op.key, op.fetcher, op.ttl ? { ttl: op.ttl } : {})
        )
      );
