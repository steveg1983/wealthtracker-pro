...
interface UseOfflineDataOptions<TEntity extends OfflineEntityType> {
  entity: TEntity;
  onConflict?: (conflict: OfflineConflictRecord<TEntity>) => void;
}

interface UseOfflineDataReturn<TEntity extends OfflineEntityType> {
  save: (data: OfflineEntityData<TEntity>) => Promise<void>;
  update: (
    id: string,
    updates: Partial<OfflineEntityData<TEntity>>
  ) => Promise<void>;
  remove: (id: string) => Promise<void>;
  isOffline: boolean;
}

export function useOfflineData<TEntity extends OfflineEntityType>(
  options: UseOfflineDataOptions<TEntity>
): UseOfflineDataReturn<TEntity> {
  const { isOffline } = useOffline();

  const save = useCallback(
    async (data: OfflineEntityData<TEntity>): Promise<void> => {
    ...
    if (options.entity === 'transaction') {
      await offlineService.saveTransaction(data, isOffline);
    }
    // Add other entity types as needed
  }, [isOffline, options.entity]);

  const update = useCallback(
    async (id: string, updates: Partial<OfflineEntityData<TEntity>>): Promise<void> => {
    ...
    if (options.entity === 'transaction') {
      await offlineService.updateTransaction(id, updates as Partial<Transaction>, isOffline);
    }
    // Add other entity types as needed
  }, [isOffline, options.entity]);
