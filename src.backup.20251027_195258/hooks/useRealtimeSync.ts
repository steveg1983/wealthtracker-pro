...
declare global {
  interface Window {
    __updateRealtimeActionTimestamp?: () => void;
  }
}
...
const getUpdatedAtTimestamp = (record: unknown): number | undefined => {
  if (!record || typeof record !== 'object') {
    return undefined;
  }

  const withUpdatedAt = record as {
    updatedAt?: string | Date | number;
    updated_at?: string | Date | number;
    updatedAtMs?: number;
  };

  const value = withUpdatedAt.updatedAt ?? withUpdatedAt.updated_at ?? withUpdatedAt.updatedAtMs;
  if (!value) {
    return undefined;
  }
...
export function useRealtimeSync(options: UseRealtimeSyncOptions = {}): UseRealtimeSyncReturn {
  ...
  const handleTransactionsEvent: RealtimeCallback<Transaction> = useCallback((event) => {
    if (isEchoUpdate(getUpdatedAtTimestamp(event.new))) {
      return;
    }
  ...
  useEffect(() => {
    ...
    window.__updateRealtimeActionTimestamp = updateLastActionTimestamp;
    
    return () => {
      delete window.__updateRealtimeActionTimestamp;
    };
