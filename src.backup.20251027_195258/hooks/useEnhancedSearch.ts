...
import type { SearchOptions, SearchResponse } from '../services/searchService';
...
interface SerializedSavedSearch extends Omit<SavedSearch, 'createdAt'> {
  createdAt: string;
}
...
export function useEnhancedSearch({
  ...
  useEffect(() => {
    ...
    try {
      const parsed = JSON.parse(saved) as SerializedSavedSearch[];
      const normalised = parsed.map(search => ({
        ...search,
        createdAt: new Date(search.createdAt)
      }));
      setSavedSearches(normalised);
    } catch (error) {
