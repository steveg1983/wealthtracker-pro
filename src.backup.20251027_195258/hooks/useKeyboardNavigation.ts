...
export function useKeyboardNavigation(options: KeyboardNavigationOptions = {}) {
  ...
  const scrollToSelected = (index: number) => {
    const selected = document.querySelector(`[data-index="${index}"]`) as HTMLElement;
    if (selected) {
  ...
  ], [navigate, isSearchFocused]);

  const vimShortcuts = useMemo<KeyboardShortcut[]>(() => {
    if (!enableVimMode) {
      return [];
    }
  ...
  }, [commandBuffer, enableVimMode, selectedIndex, transactions.length]);
  ...
  const allShortcuts = useMemo(
    ...
    [customShortcuts, defaultShortcuts, registeredShortcuts, vimShortcuts]
  );
    ...
    navigateByIndex: (direction: 'up' | 'down' | 'first' | 'last') => {
      ...
      switch (direction) {
        case 'up':
          ...
          break;
        case 'down':
          nextIndex = Math.min(selectedIndex + 1, transactions.length - 1);
          break;
        case 'first':
          nextIndex = 0;
          break;
        case 'last':
          nextIndex = transactions.length - 1;
          break;
      }
