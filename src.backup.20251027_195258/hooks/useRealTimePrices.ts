...
export function useRealTimePrices({
  ...
  useEffect(() => {
    ...
    // Handle errors
    const handleError = (payload: JsonValue) => {
      if (isJsonObject(payload)) {
        const symbol = typeof payload.symbol === 'string' ? payload.symbol : 'Unknown symbol';
        const message = typeof payload.error === 'string' ? payload.error : 'Unknown error';
        setError(`Failed to fetch price for ${symbol}: ${message}`);
      } else if (typeof payload === 'string') {
        setError(payload);
      } else {
        setError('Failed to fetch price update.');
      }
      setIsLoading(false);
    };
