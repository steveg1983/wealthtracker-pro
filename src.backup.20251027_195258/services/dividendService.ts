...
class DividendService {
  ...
  updateDividend(dividendId: string, updates: Partial<Dividend>): boolean {
    ...
    const current = this.dividends[index];
  ...
  getDividendSummary(startDate?: Date, endDate?: Date): DividendSummary {
    ...
    const filteredDividends = this.getDividends(filters);
    ...
    filteredDividends.forEach(div => {
      ...
      const symbolSummary = summary.bySymbol[div.symbol];
      if (symbolSummary) {
        symbolSummary.total = symbolSummary.total.plus(amount);
        symbolSummary.count++;
        if (div.paymentDate > symbolSummary.lastPayment) {
          symbolSummary.lastPayment = div.paymentDate;
        }
      }
      ...
      const frequencyTotal = summary.byFrequency[div.frequency];
      if (frequencyTotal) {
        summary.byFrequency[div.frequency] = frequencyTotal.plus(amount);
      }
      ...
      const typeTotal = summary.byType[div.type];
      if (typeTotal) {
        summary.byType[div.type] = typeTotal.plus(amount);
      }
      ...
      const monthTotal = summary.byMonth[monthKey];
      if (monthTotal) {
        summary.byMonth[monthKey] = monthTotal.plus(amount);
      }
    });
  ...
  private calculateProjectedAnnualIncome(): DecimalInstance {
    ...
    recentDividends.forEach(div => {
      ...
      const symbolData = symbolFrequency[div.symbol];
      if (symbolData) {
        symbolData.total = symbolData.total.plus(toDecimal(div.amount));
        symbolData.count++;
      }
    });
    ...
    Object.values(symbolFrequency).forEach(data => {
  ...
  getDividendProjections(holdings: Array<{ symbol: string; shares: number; currentValue: number }>): DividendProjection[] {
    ...
    holdings.forEach(holding => {
      ...
      if (recentDividends.length > 0) {
        ...
        const yieldValue = this.yieldData[holding.symbol];
        if (yieldValue) {
          const annualYield = yieldValue / 100;
  ...
  exportToCSV(): string {
    ...
    const formatDecimalString = (value: number | undefined, decimals: number): string => {
      if (value === undefined) {
        return '';
      }
    ...
    const rows = this.dividends.map(div => [
      ...
      div.symbol,
      formatDecimalString(div.amount, 2),
      formatDecimalString(div.amountPerShare, 4),
      toDecimal(div.shares).toString(),
      div.type,
      div.frequency,
      formatDecimalString(div.taxWithheld, 2),
