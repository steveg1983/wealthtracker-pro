...
class BusinessService {
  ...
  getDefaultVATRate(): VATRate {
    const defaultRate = this.vatRates.find(rate => rate.isDefault) || this.vatRates[0];
    if (!defaultRate) {
      throw new Error('No VAT rates available');
    }
    return defaultRate;
  }
  ...
  updateVATRate(id: string, updates: Partial<Omit<VATRate, 'id'>>): VATRate | null {
    ...
    const existing = this.vatRates[index];
    if (!existing) {
      return null;
    }
  ...
  updateMileageEntry(id: string, updates: Partial<Omit<MileageEntry, 'id' | 'createdAt'>>): MileageEntry | null {
    const index = this.mileageEntries.findIndex(entry => entry.id === id);
    if (index === -1) return null;

    const existing = this.mileageEntries[index];
    if (!existing) {
      return null;
    }
    ...
    if (updates.distance !== undefined || updates.rate !== undefined) {
      ...
      entry.amount = new Decimal(distance).times(rate).toNumber();
    }
  ...
  updateBusinessExpense(id: string, updates: Partial<Omit<BusinessExpense, 'id' | 'createdAt'>>): BusinessExpense | null {
    const index = this.businessExpenses.findIndex(expense => expense.id === id);
    if (index === -1) return null;

    const existing = this.businessExpenses[index];
    if (!existing) {
      return null;
    }
    ...
    this.businessExpenses[index] = updated;
    this.saveData();
    return updated;
  }
