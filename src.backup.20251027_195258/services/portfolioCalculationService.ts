  ...
  calculateRebalanceRecommendations(
    ...
    holdings.forEach(holding => {
      ...
      if (Math.abs(difference) < 1) {
        ...
        const priceDecimal = new Decimal(holding.currentPrice || 0);
        const sharesDecimal = priceDecimal.greaterThan(0)
          ? new Decimal(valueDifference).abs().dividedBy(priceDecimal)
          : new Decimal(0);
        ...
        recommendations.push({
          ...
          shares: sharesDecimal.toDecimalPlaces(2, Decimal.ROUND_HALF_UP).toNumber(),
