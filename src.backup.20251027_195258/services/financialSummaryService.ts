...
import { formatPercentageValue, formatSignedPercentageValue, toDecimal } from '@wealthtracker/utils';
...
class FinancialSummaryService {
   ...
   */
  formatSummaryText(summary: SummaryData, currency: string = 'GBP'): string {
    const formatCurrencyValue = (value: DecimalInstance | number): string =>
      formatCurrencyDecimal(value, currency);
    const formatPercent = (value: DecimalInstance | number, decimals: number = 1): string =>
      formatPercentageValue(value, decimals);
    const formatSignedPercent = (value: DecimalInstance | number, decimals: number = 1): string =>
      formatSignedPercentageValue(value, decimals);
    ...
    // Overview
    text += `**Income:** ${formatCurrencyValue(summary.totalIncome)}\n`;
    text += `**Expenses:** ${formatCurrencyValue(summary.totalExpenses)}\n`;
    text += `**Net:** ${formatCurrencyValue(summary.netIncome)}\n`;
    text += `**Savings Rate:** ${formatPercent(summary.savingsRate, 1)}\n\n`;
    ...
    if (summary.comparison.incomeChange !== 0 || summary.comparison.expenseChange !== 0) {
      ...
      text += `- Income: ${formatSignedPercent(summary.comparison.incomeChange, 1)}\n`;
      text += `- Expenses: ${formatSignedPercent(summary.comparison.expenseChange, 1)}\n\n`;
    }
    ...
    if (summary.topCategories.length > 0) {
      ...
      summary.topCategories.forEach(cat => {
        text += `- ${cat.category}: ${formatCurrencyValue(cat.amount)} (${formatPercent(cat.percentage, 1)})\n`;
      });
      text += '\n';
    }
    ...
    if (overBudget.length > 0) {
      ...
      overBudget.forEach(b => {
        text += `- ${b.budgetName}: ${formatCurrencyValue(b.spent)} / ${formatCurrencyValue(b.limit)} (${formatPercent(b.percentage, 0)})\n`;
      });
      text += '\n';
    }
    ...
    if (activeGoals.length > 0) {
      ...
      activeGoals.forEach(g => {
        text += `- ${g.goalName}: ${formatPercent(g.progress, 1)} (+${formatCurrencyValue(g.amountAdded)} this ${summary.period === 'weekly' ? 'week' : 'month'})\n`;
      });
    }
