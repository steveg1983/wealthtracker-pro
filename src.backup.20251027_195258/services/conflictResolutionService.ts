...
export class ConflictResolutionService {
  ...
  static analyzeConflict(
    ...
    if (analysis.hasConflict && analysis.canAutoResolve) {
      const conflictId =
        (clientData && typeof clientData === 'object' ? clientData.id : undefined) ??
        (serverData && typeof serverData === 'object' ? serverData.id : undefined);

      analyticsEngine.track('conflict_auto_resolved', {
        entity: entityType,
        conflictId,
   ...
   */
  private static getChangedFields(obj1: EntityRecord, obj2: EntityRecord): string[] {
  ...
  private static categorizeChanges(
    ...
    for (const field of changedFields) {
      ...
      if (this.isNonConflictingChange(field, clientValue, serverValue)) {
  ...
  private static performSmartMerge(
    ...
    const merged: EntityRecord = { ...serverData }; // Start with server as base
    ...
    for (const field of analysis.nonConflictingFields) {
      ...
      merged[field] = this.mergeFieldValue(field, clientValue, serverValue, clientTimestamp, serverTimestamp);
    }
