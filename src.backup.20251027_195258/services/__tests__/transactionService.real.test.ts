...
import { describeSupabase, shouldRunSupabaseRealTests } from '@wealthtracker/testing/supabaseRealTest';

// Use real Supabase test instance
const supabaseUrl = import.meta.env?.VITE_SUPABASE_URL ?? process.env.VITE_SUPABASE_URL;
const supabaseKey = import.meta.env?.VITE_SUPABASE_ANON_KEY ?? process.env.VITE_SUPABASE_ANON_KEY;

const testSupabase = shouldRunSupabaseRealTests && supabaseUrl && supabaseKey
  ? createClient(supabaseUrl, supabaseKey)
  : null;

const requireSupabase = () => {
  if (!testSupabase) {
    throw new Error('Supabase client not configured for transaction real tests');
  }
  return testSupabase;
};
...
describeIfSupabase('TransactionService - REAL Database Tests', () => {
  beforeEach(async () => {
  ...
  describe('createTransaction - REAL', () => {
    it('should create a real transaction in the database', async () => {
      ...
      if (result.id) {
        ...
        const { data: verifyData } = await requireSupabase()
          .from('transactions')
          .select('*')
          .eq('id', result.id)
          .single();
        ...
        expect(toDecimal(verifyData?.amount ?? 0).toNumber()).toBe(-150.50);
      }
    });
    ...
    it('should handle real database constraints for transactions', async () => {
      ...
      await expect(
        TransactionService.createTransaction(testUserId, invalidTransaction as TransactionInput)
      ).rejects.toThrow();
    });
  ...
  describe('getTransactions - REAL', () => {
    ...
    it('should return empty array for user with no transactions', async () => {
      ...
      // Cleanup
      await requireSupabase().from('users').delete().eq('id', emptyUserId);
    });
  });
  ...
  describe('updateTransaction - REAL', () => {
    it('should update a real transaction in the database', async () => {
      ...
      expect(toDecimal(verifyData?.amount ?? 0).toNumber()).toBe(-125.00);
    });
  });
  ...
  describe('Real Database Transaction Filters', () => {
    ...
    it('should handle concurrent transaction operations correctly', async () => {
      ...
      const { data: allTransactions } = await requireSupabase()
        .from('transactions')
        .select('*')
        .in('id', createdTransactionIds);
...
export class TransactionTestDataFactory {
  private readonly supabase = shouldRunSupabaseRealTests && supabaseUrl && supabaseKey
    ? createClient(supabaseUrl, supabaseKey)
    : null;
...
type TransactionInput = Parameters<typeof TransactionService.createTransaction>[1];
