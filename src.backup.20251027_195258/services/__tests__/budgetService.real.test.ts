...
const requireSupabase = () => {
  if (!testSupabase) {
    throw new Error('Supabase client not configured for budget real tests');
  }
  return testSupabase;
};
...
describeIfSupabase('BudgetService - REAL Database Tests', () => {
  beforeEach(async () => {
  ...
  describe('createBudget - REAL', () => {
    it('should create a real budget in the database', async () => {
      ...
      if (result.id) {
        ...
        expect(toDecimal(verifyData?.amount ?? 0).toNumber()).toBe(500);
      }
    });
  ...
  describe('updateBudget - REAL', () => {
    it('should update a real budget in the database', async () => {
      ...
      expect(toDecimal(verifyData?.amount ?? 0).toNumber()).toBe(350);
      expect(toDecimal(verifyData?.spent ?? 0).toNumber()).toBe(125);
    });
  });
  ...
  describe('Real Database Budget Constraints', () => {
    it('should handle concurrent budget operations correctly', async () => {
      ...
      const { data: allBudgets } = await requireSupabase()
        .from('budgets')
        .select('*')
        .in('id', createdBudgetIds);
