...
class RealTimePriceService {
  private intervals: Map<string, IntervalHandle> = new Map();
  ...
  private marketCheckInterval: IntervalHandle | null = null;
  private eventListeners: Map<string, Set<(data: JsonValue) => void>> = new Map();
  
  constructor() {
    this.checkMarketStatus();
    // Check market status every 5 minutes
    this.marketCheckInterval = globalThis.setInterval(() => this.checkMarketStatus(), 5 * 60 * 1000);
  }
   ...
   */
  dispose(): void {
    this.intervals.forEach(interval => globalThis.clearInterval(interval));
    this.intervals.clear();
    this.subscriptions.clear();
    if (this.marketCheckInterval) {
      globalThis.clearInterval(this.marketCheckInterval);
    }
