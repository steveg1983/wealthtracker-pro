...
class MobileService {
  ...
  private async processImage(capture: CameraCapture): Promise<CameraCapture> {
    ...
    const suggestedAmount = amountMatch ? toDecimal(amountMatch[1] || '0').toNumber() : undefined;
    
    // Determine merchant and category
    const merchantMatch = mockText.match(/([A-Z\s]+)(?=\n)/);
    const suggestedMerchant = merchantMatch?.[1]?.trim();
    ...
    budgets.forEach(budget => {
      ...
      if (percentage >= 90) {
        this.sendNotification(
          'Budget Alert',
          `You've spent ${formatPercentageValue(percentage, 0)} of your ${budget.categoryId} budget`,
          { type: 'budget_alert', budgetId: budget.id, category: budget.categoryId }
        );
      }
    });
  }
    ...
    recurringExpenses.forEach(expense => {
      ...
      if (daysSinceLastPayment >= 25 && daysSinceLastPayment <= 35) {
        this.sendNotification(
          ...
          { type: 'bill_reminder', transactionId: expense.id }
        );
      }
    });
  }
