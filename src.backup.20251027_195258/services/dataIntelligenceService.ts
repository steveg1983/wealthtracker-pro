...
export interface DataIntelligenceStats {
  ...
  monthlySubscriptionCost: DecimalInstance;
  topMerchants: Array<{
    name: string;
  ...
  categoryAccuracy: DecimalInstance;
...
class DataIntelligenceService {
  ...
  private getMerchantTransactionCount(_merchantName: string): number {
  ...
  private performBasicEnrichment(description: string): MerchantEnrichment {
    ...
    // If no pattern matches, return basic enrichment
    return {
      originalName: description,
      cleanName,
      category: 'Other',
  ...
  detectSubscriptions(transactions: Transaction[]): Subscription[] {
    ...
    Object.entries(merchantGroups).forEach(([merchant, merchantTransactions]) => {
      ...
      for (let i = 1; i < merchantTransactions.length; i++) {
        const currentTransaction = merchantTransactions[i];
        const previousTransaction = merchantTransactions[i-1];
        if (currentTransaction && previousTransaction) {
          const daysDiff = toDecimal(currentTransaction.date.getTime())
            .minus(previousTransaction.date.getTime())
      ...
      if (intervals.length === 0) return; // Skip if no intervals

      const sortedIntervals = [...intervals].sort((a, b) => a - b);
      const medianInterval = sortedIntervals[Math.floor(intervals.length / 2)];

      if (typeof medianInterval !== 'number') return; // Skip if median is undefined
      ...
      if (isRegular && isAmountConsistent && medianInterval >= 7 && medianInterval <= 370) {
        ...
        if (!existingSubscription) {
          ...
          potentialSubscriptions.push({
            ...
            frequency,
            nextPaymentDate,
            category: enrichment.category || firstTransaction.category || 'Other',
            status: 'active',
    ...
    return potentialSubscriptions.sort((a, b) => {
      const subA = a as Subscription & { confidence?: number };
      const subB = b as Subscription & { confidence?: number };
      const confA = subA.confidence ?? 0.5;
      const confB = subB.confidence ?? 0.5;
  ...
  analyzeSpendingPatterns(transactions: Transaction[]): SpendingPattern[] {
    ...
    Object.entries(categoryGroups).forEach(([category, categoryTransactions]) => {
      ...
      for (let i = 1; i < categoryTransactions.length; i++) {
        const currentTransaction = categoryTransactions[i];
        const previousTransaction = categoryTransactions[i-1];
        if (currentTransaction && previousTransaction) {
          const daysDiff = toDecimal(currentTransaction.date.getTime())
            .minus(previousTransaction.date.getTime())
            .dividedBy(1000 * 60 * 60 * 24)
  ...
  generateInsights(transactions: Transaction[]): SpendingInsight[] {
    ...
    upcomingRenewals.forEach(sub => {
      const daysUntilRenewal = Math.ceil((sub.nextPaymentDate.getTime() - Date.now()) / (1000 * 60 * 60 * 24));
      const subscriptionCurrency = sub.currency ? sub.currency.toUpperCase() : 'USD';
      const renewalAmount = formatCurrencyDecimal(toDecimal(sub.amount), subscriptionCurrency);
      newInsights.push({
        id: `insight-renewal-${sub.id}`,
        type: 'subscription_alert',
        title: 'Subscription Renewal Due',
        description: `${sub.merchantName} (${renewalAmount}) renews in ${daysUntilRenewal} day${daysUntilRenewal !== 1 ? 's' : ''}`,
        severity: daysUntilRenewal <= 3 ? 'high' : 'medium',
        category: sub.category,
        merchant: sub.merchantName,
        amount: toDecimal(sub.amount).toDecimalPlaces(2, Decimal.ROUND_HALF_UP).toNumber(),
    ...
    Object.entries(currentMonthByCategory).forEach(([category, currentAmountDecimal]) => {
      const lastAmountDecimal = lastMonthByCategory[category] ?? toDecimal(0);
      if (lastAmountDecimal.greaterThan(0)) {
        const increasePercentageDecimal = currentAmountDecimal
          .minus(lastAmountDecimal)
          .dividedBy(lastAmountDecimal)
          .times(100);
        
        if (increasePercentageDecimal.greaterThan(30)) {
          newInsights.push({
            id: `insight-spike-${category}`,
            type: 'spending_spike',
            title: 'Unusual Spending Detected',
            description: `${category} spending is ${formatPercentageValue(increasePercentageDecimal, 0)} higher than last month`,
            severity: increasePercentageDecimal.greaterThan(50) ? 'high' : 'medium',
            category,
            amount: currentAmountDecimal.toDecimalPlaces(2, Decimal.ROUND_HALF_UP).toNumber(),
  ...
  private groupTransactionsByCategory(transactions: Transaction[]): Record<string, DecimalInstance> {
    return transactions.reduce((groups, transaction) => {
      const category = transaction.category;
      if (!groups[category]) {
        groups[category] = toDecimal(0);
      }
      groups[category] = groups[category].plus(toDecimal(transaction.amount).abs());
      return groups;
    }, {} as Record<string, DecimalInstance>);
  }
  ...
  getSmartCategories(): SmartCategory[] {
    return this.smartCategories.map(category => ({ ...category }));
  }
  ...
  getStats(): DataIntelligenceStats {
    ...
    const weeksPerMonth = toDecimal(4.33);
    const monthlySubscriptionCost = this.subscriptions
      .filter(s => s.status === 'active')
      .reduce<DecimalInstance>((sum, s) => {
        const amountDecimal = toDecimal(s.amount);
        switch (s.frequency) {
          case 'monthly':
            return sum.plus(amountDecimal);
          case 'yearly':
            return sum.plus(amountDecimal.dividedBy(12));
          case 'quarterly':
            return sum.plus(amountDecimal.dividedBy(3));
          case 'weekly':
            return sum.plus(amountDecimal.times(weeksPerMonth));
          default:
            return sum.plus(amountDecimal);
        }
      }, toDecimal(0));
    ...
    const topMerchants = this.merchants
      ...
      .map(m => ({
        name: m.name,
        amount: toDecimal(m.avgTransactionAmount ?? 0),
        transactionCount: 0,
        category: m.category
      }));
    ...
    return {
      ...
      topMerchants,
      categoryAccuracy: totalMerchants > 0
        ? toDecimal(enrichedMerchants).dividedBy(toDecimal(totalMerchants)).times(100)
        : toDecimal(0),
