...
class PushNotificationService {
  ...
  async showNotification(options: NotificationOptions): Promise<void> {
    ...
    const notificationOptions: globalThis.NotificationOptions = {
      ...
      ...(options.requireInteraction !== undefined && { requireInteraction: options.requireInteraction })
    };
   ...
   */
  static getNotificationConfig(type: string, rawData: NotificationPayload = {}): NotificationOptions {
    const data = { ...rawData };
    ...
    switch (type) {
      case 'budget-alert': {
        const percentage = typeof data.percentage === 'number'
          ? data.percentage
          : Number.parseFloat(String(data.percentage ?? '0'));
        const category = typeof data.category === 'string' ? data.category : 'your';
      ...
      case 'bill-reminder': {
        const name = typeof data.name === 'string' ? data.name : 'Bill';
        const amount = typeof data.amount === 'number'
          ? data.amount
          : Number.parseFloat(String(data.amount ?? '0'));
        const daysUntilDue = typeof data.daysUntilDue === 'number'
          ? data.daysUntilDue
          : Number.parseInt(String(data.daysUntilDue ?? '0'), 10);
        const id = typeof data.id === 'string' ? data.id : Date.now().toString();

        return {
          title: '📅 Bill Due Soon',
          body: `${name} - $${amount} due in ${daysUntilDue} days`,
          tag: `bill-${id}`,
      ...
      case 'goal-achieved': {
        const name = typeof data.name === 'string' ? data.name : 'your goal';
        const id = typeof data.id === 'string' ? data.id : Date.now().toString();

        return {
          title: '🎉 Goal Achieved!',
          body: `Congratulations! You've reached your "${name}" goal`,
          tag: `goal-${id}`,
      ...
      case 'investment-alert': {
        const message = typeof data.message === 'string'
          ? data.message
          : 'Check your portfolio for important updates';
      ...
      case 'unusual-spending': {
        const category = typeof data.category === 'string' ? data.category : 'a category';
        const percentage = typeof data.percentage === 'number'
          ? data.percentage
          : Number.parseFloat(String(data.percentage ?? '0'));
        ...
        return {
          ...
          body: `Spending in ${category} is ${percentage}% higher than usual`,
          tag: 'unusual-spending',
          data: { type: 'unusual-spending', ...data },
          requireInteraction: true,
          actions: [
            { action: 'view-transactions', title: 'Review Transactions' },
            { action: 'dismiss', title: 'Dismiss' }
          ]
        };
      }
