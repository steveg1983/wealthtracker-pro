...
class StateTaxService {
  ...
  calculateStateTax(
    ...
    Object.values(incomeComponents).forEach(amount => {
      if (amount > 0) {
        totalIncome += amount;
      }
    });
  ...
  private calculateTax(
    ...
    if (stateInfo.incomeTax.type === 'flat' && stateInfo.incomeTax.rate) {
      tax = toDecimal(taxableIncome).times(stateInfo.incomeTax.rate ?? 0);
      breakdown.push({
        income: 'State Income Tax',
        deductions: '',
        taxableAmount: `$${taxableIncome.toLocaleString()}`,
        tax: formatCurrencyDecimal(tax, 'USD'),
        rate: formatPercentageValue(toDecimal(stateInfo.incomeTax.rate ?? 0).times(100), 2)
      });
      ...
      for (const bracket of brackets) {
        if (taxableIncome > bracket.min) {
          ...
          const bracketTax = toDecimal(taxableInBracket).times(bracket.rate);
          ...
          if (taxableInBracket > 0) {
            breakdown.push({
              ...
              tax: formatCurrencyDecimal(bracketTax, 'USD'),
              rate: formatPercentageValue(toDecimal(bracket.rate).times(100), 2)
            });
          }
        }
      }
