...
class AlphaVantageService {
  ...
  async getQuote(symbol: string): Promise<StockQuote | null> {
    ...
    try {
      ...
      const stockQuote: StockQuote = {
        ...
        changePercent: toDecimal(quote['10. change percent']?.replace('%', '') || '0').toNumber(),
  ...
  async getHistoricalPrices(
    ...
    try {
      ...
      const prices: HistoricalPrice[] = Object.entries(timeSeries as Record<string, Record<string, string>>)
        .map(([date, values]) => {
          const basePrice = {
            date: new Date(date),
          ...
          return values['7. dividend amount']
            ? { ...basePrice, dividendAmount: toDecimal(values['7. dividend amount']) }
            : basePrice;
        })
  ...
  async getDividendInfo(symbol: string): Promise<DividendInfo | null> {
    ...
    try {
      ...
      if (recentDividends.length >= 2) {
        const first = recentDividends[0];
        const second = recentDividends[1];
      ...
      const mostRecent = recentDividends[0];
      if (!mostRecent) return null;

      const dividendInfo: DividendInfo = {
        symbol,
        exDividendDate: mostRecent.date,
        paymentDate: new Date(mostRecent.date.getTime() + 14 * 24 * 60 * 60 * 1000), // Estimate
        amount: mostRecent.dividendAmount || new Decimal(0),
        frequency,
        yieldPercentage
      };
  ...
  private getFallbackHistoricalPrices(_symbol: string): HistoricalPrice[] {
