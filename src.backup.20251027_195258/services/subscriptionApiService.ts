...
import { SupabaseSubscriptionService } from '@wealthtracker/core';
...
const ensureUserId = (userProfile: { id?: unknown } | null): string | null => {
  if (userProfile && typeof userProfile.id === 'string' && userProfile.id.trim()) {
...
export class SubscriptionApiService {
  private static stripe = import.meta.env.VITE_STRIPE_PUBLISHABLE_KEY
  ...
  static async getCurrentSubscription(clerkUserId: string): Promise<UserSubscription | null> {
    try {
      ...
      if (!userId) {
        logger.warn('User profile missing id when fetching subscription');
        return null;
      }

      return await SupabaseSubscriptionService.getCurrentSubscription(userId);
    } catch (error) {
  ...
  static async getBillingHistory(clerkUserId: string): Promise<BillingHistory> {
    try {
      ...
      const [paymentMethods, invoices] = await Promise.all([
        SupabaseSubscriptionService.getPaymentMethods(userId),
        SupabaseSubscriptionService.getInvoices(userId)
      ]);

      const subscription = await SupabaseSubscriptionService.getCurrentSubscription(userId);
      ...
      return {
        ...
        ...(subscription?.currentPeriodEnd && { nextBillingDate: subscription.currentPeriodEnd })
      };
    } catch (error) {
  ...
  static async cancelDemoSubscription(clerkUserId: string): Promise<UserSubscription> {
    try {
      ...
      const currentSubscription = await SupabaseSubscriptionService.getCurrentSubscription(userId);
  ...
  static async initializeUserProfile(
    ...
    try {
      ...
      // Initialize usage tracking when the Supabase profile includes a usable id
      const userProfile = await SupabaseSubscriptionService.getUserProfile(clerkUserId);
      const userId = ensureUserId(userProfile);
      if (!userId) {
        logger.warn('initializeUserProfile could not resolve user id from Supabase profile');
        return;
      }

      await SupabaseSubscriptionService.refreshUsageCounts(userId);
    } catch (error) {
  ...
  static async checkFeatureAccess(clerkUserId: string, feature: string): Promise<boolean> {
    try {
      ...
      const userId = ensureUserId(userProfile);
      if (!userId) {
        logger.warn('checkFeatureAccess missing user id');
        return false;
      }

      return await SupabaseSubscriptionService.hasFeatureAccess(userId, feature);
    } catch (error) {
  ...
  static async getUsageInfo(clerkUserId: string) {
    try {
      ...
      const [subscription, usage] = await Promise.all([
        SupabaseSubscriptionService.getCurrentSubscription(userId),
        SupabaseSubscriptionService.getSubscriptionUsage(userId)
      ]);
