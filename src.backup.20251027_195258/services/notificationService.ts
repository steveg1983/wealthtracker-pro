...
class NotificationService {
  ...
  checkBudgetAlerts(budgets: Budget[], transactions: Transaction[], categories: Category[]): Notification[] {
    ...
    budgets.forEach(budget => {
      ...
      const budgetAmountDecimal = new Decimal(budget.amount || 0);
      const percentageDecimal = budget.amount > 0
        ? spentDecimal.dividedBy(budgetAmountDecimal).times(100)
        : new Decimal(0);
      const percentage = percentageDecimal.toNumber();
      ...
      applicableRules.forEach(rule => {
        if (this.evaluateConditions(rule.conditions, { 
          ...
          const notification = this.createNotificationFromRule(rule, {
            ...
            percentage: percentageDecimal.toDecimalPlaces(0, Decimal.ROUND_HALF_UP).toNumber(),
            spent,
  ...
  private createNotificationFromRule(rule: NotificationRule, context: UnknownObject): Notification | null {
    ...
    if (action.config.actionButton) {
      const { label, action: target } = action.config.actionButton;
      notification.action = {
        label,
        onClick: () => {
          if (target) {
  ...
  updateRule(ruleId: string, updates: Partial<NotificationRule>): boolean {
    ...
    if (existingRule) {
      this.rules[ruleIndex] = { ...existingRule, ...updates };
    }
    this.saveRules();
    return true;
  }
