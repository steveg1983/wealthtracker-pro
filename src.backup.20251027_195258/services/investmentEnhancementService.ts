...
class InvestmentEnhancementService {
  ...
  getRebalancingSuggestions(
    ...
    Object.entries(targetAllocations).forEach(([category, targetPercent]) => {
      ...
      if (Math.abs(difference) > 5) { // 5% threshold
        ...
        if (difference > 0) {
          const differenceDisplay = formatPercentageValue(Math.abs(difference), 1);
          // Need to buy more
          suggestions.push({
            symbol: category,
            name: `${category} ETF`,
  ...
  generateInsights(
    ...
    if (totalDividends.greaterThan(0)) {
      const totalDividendsDisplay = totalDividends
        .toDecimalPlaces(2)
        .toNumber()
        .toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
      insights.push(`💰 Projected annual dividend income: $${totalDividendsDisplay}`);
    }
