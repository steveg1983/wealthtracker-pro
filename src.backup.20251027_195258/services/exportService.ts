...
import type { Transaction, Account, Investment, Budget, Goal } from '../types';
import type {
  ExportableAccount,
  ExportableBudget,
  ExportableData,
...
type ReportDateInput = Date | string;

interface FinancialReportDateRange {
  start: ReportDateInput;
  end: ReportDateInput;
}
...
type FinancialReportBudgetPerformance = Budget & {
  remaining: number;
  percentUsed: number;
  isOverBudget: boolean;
};

type FinancialReportGoalProgress = Goal & {
  progress: number;
  remaining: number;
  isCompleted: boolean;
};

type FinancialReportTransaction = Transaction & {
  categoryId?: string;
};

interface FinancialReportDataset {
  income: number;
  expenses: number;
  cashFlow: number;
  savingsRate: number;
  ...
  budgetPerformance: FinancialReportBudgetPerformance[];
  goalProgress: FinancialReportGoalProgress[];
  filteredTransactions: FinancialReportTransaction[];
}
...
class ExportService {
  ...
  async generatePDFReport(exportData: FinancialReportExport, _options: ExportOptions): Promise<void> {
    const doc = await this.createPdfDocument();
    let yPosition = 20;
    const { summary, sections, data } = exportData;
    ...
    if (summary) {
      ...
      doc.setFontSize(10);
      const summaryItems = [
        `Net Worth: ${this.formatCurrency(summary.netWorth)}`,
        `Total Assets: ${this.formatCurrency(summary.totalAssets)}`,
        `Total Liabilities: ${this.formatCurrency(summary.totalLiabilities)}`,
        `Monthly Income: ${this.formatCurrency(summary.monthlyIncome)}`,
        `Monthly Expenses: ${this.formatCurrency(summary.monthlyExpenses)}`,
        `Cash Flow: ${this.formatCurrency(summary.cashFlow)}`,
        `Savings Rate: ${formatPercentageValue(summary.savingsRate, 1)}`
      ];
    ...
    sections.forEach(section => {
      ...
      if (section.id === 'budgets' && data.budgetPerformance?.length) {
        doc.setFontSize(10);
        data.budgetPerformance.forEach(budget => {
          ...
          const remaining = new Decimal(budget.remaining);
          const percentUsed = budget.percentUsed;
          doc.text(
            `${budget.categoryId}: ${this.formatCurrency(spent)} / ${this.formatCurrency(amount)} ` +
            `(${formatPercentageValue(percentUsed, 0)}) • Remaining ${this.formatCurrency(remaining)}`,
            30,
            yPosition
          );
          yPosition += 6;
      ...
      if (section.id === 'goals' && data.goalProgress?.length) {
        doc.setFontSize(10);
        data.goalProgress.forEach(goal => {
          ...
          doc.text(`${goal.name}: ${formatPercentageValue(goal.progress ?? 0, 0)} complete`, 30, yPosition);
          yPosition += 6;
        });
      }
    });
  ...
  async generateExcelReport(exportData: FinancialReportExport, options: ExportOptions): Promise<void> {
  ...
  private async createComprehensiveExcelWorkbook(
    exportData: FinancialReportExport,
    options: ExportOptions
    ...
    if (exportData.data.budgetPerformance && exportData.data.budgetPerformance.length > 0) {
      ...
      exportData.data.budgetPerformance.forEach(budget => {
        budgetData.push([
          budget.categoryId,
          budget.amount,
          budget.spent,
    ...
    if (exportData.data.goalProgress && exportData.data.goalProgress.length > 0) {
      ...
      exportData.data.goalProgress.forEach(goal => {
        goalsData.push([
          goal.name,
          goal.targetAmount,
    ...
    if (options.includeTransactions && exportData.data.filteredTransactions) {
      ...
      exportData.data.filteredTransactions.forEach(transaction => {
  ...
  private downloadExcel(workbook: import('xlsx').WorkBook, filename: string): void {
    importXLSX()
      .then(XLSX => {
  ...
  async generateCSVReport(exportData: FinancialReportExport, options: ExportOptions): Promise<void> {
    // Create CSV content
    ...
    csvContent += `Net Worth,"${this.formatCurrency(exportData.summary.netWorth)}"\n`;
    csvContent += `Total Assets,"${this.formatCurrency(exportData.summary.totalAssets)}"\n`;
    csvContent += `Total Liabilities,"${this.formatCurrency(exportData.summary.totalLiabilities)}"\n`;
    csvContent += `Monthly Income,"${this.formatCurrency(exportData.summary.monthlyIncome)}"\n`;
    csvContent += `Monthly Expenses,"${this.formatCurrency(exportData.summary.monthlyExpenses)}"\n`;
    csvContent += `Cash Flow,"${this.formatCurrency(exportData.summary.cashFlow)}"\n`;
    csvContent += `Savings Rate,"${formatPercentageValue(exportData.summary.savingsRate, 1)}"\n\n`;
    ...
    if (exportData.data.budgetPerformance && exportData.data.budgetPerformance.length > 0) {
      ...
      exportData.data.budgetPerformance.forEach(budget => {
        csvContent += `${budget.categoryId},"${this.formatCurrency(budget.amount)}","${this.formatCurrency(budget.spent)}","${this.formatCurrency(budget.remaining)}","${formatPercentageValue(budget.percentUsed, 0)}"\n`;
      });
      csvContent += '\n';
    }
    ...
    if (exportData.data.goalProgress && exportData.data.goalProgress.length > 0) {
      ...
      exportData.data.goalProgress.forEach(goal => {
        csvContent += `${goal.name},"${this.formatCurrency(goal.targetAmount)}","${this.formatCurrency(goal.currentAmount ?? 0)}","${this.formatCurrency(goal.remaining)}","${formatPercentageValue(goal.progress ?? 0, 0)}"\n`;
      });
      csvContent += '\n';
    }
  ...
  private async addInvestmentsSummaryToPDF(
    ...
    investments.forEach(investment => {
      ...
      const gainLossPercentLabel = formatPercentageValue(gainLossPercent, 2);
      const signedPercent = gainLoss.gte(0) ? `+${gainLossPercentLabel}` : gainLossPercentLabel;

      doc.text(
        `${investment.symbol}: ${this.formatCurrency(currentValue)} (${signedPercent})`,
        20,
        yPosition
      );
      yPosition += 5;
  ...
  private async addBudgetsSummaryToPDF(
    ...
    budgets.forEach(budget => {
      ...
      doc.text(
        `${budget.categoryId}: ${this.formatCurrency(spent)} / ${this.formatCurrency(budgeted)} ` +
        `(${formatPercentageValue(percentSpent, 1)}) • Remaining ${this.formatCurrency(remaining)}`,
        20,
        yPosition
      );
      yPosition += 5;
  ...
  private formatCurrency(amount: DecimalInstance | number | string | null | undefined, currency: string = 'USD'): string {
    return formatCurrencyDecimal(toDecimal(amount ?? 0), currency);
  }
  ...
  exportToQIF(data: { transactions: Transaction[]; accounts: Account[] }): string {
    ...
    for (const account of data.accounts) {
      ...
      const accountBalance = toDecimal(account.balance ?? 0).toDecimalPlaces(2).toString();
      qifContent += `$${accountBalance}\n`;
      qifContent += '^\n';
      ...
      if (accountTransactions.length > 0) {
        ...
        for (const transaction of accountTransactions) {
          ...
          const transactionAmount = toDecimal(transaction.amount ?? 0).abs().toDecimalPlaces(2).toString();
          const sign = transaction.type === 'expense' ? '-' : '';
          qifContent += `T${sign}${transactionAmount}\n`;
    ...
    switch (format) {
      case 'csv': {
        const primarySource =
          ...
          data.budgets ??
          [];
        const exportSources = primarySource as ExportSource[];
        exportContent = await this.exportToCSV(exportSources, options);
