...
class SearchService {
    ...
    if (options.query && this.transactionIndex) {
      ...
      searchResults = fuseResults.map(result => ({
        ...
        matches: (result.matches || []).map(match => ({
          field: match.key || '',
          value: match.value || '',
          indices: Array.from(match.indices || []) as [number, number][]
        }))
      }));
  ...
  private applyTransactionFilters(
    ...
    // Amount range filter
    if (options.amountMin != null || options.amountMax != null) {
      filtered = filtered.filter(t => {
        if (options.amountMin != null && t.amount < options.amountMin) return false;
        if (options.amountMax != null && t.amount > options.amountMax) return false;
        return true;
      });
    }
    ...
    if (this.transactionIndex && data.transactions) {
      ...
      results.transactions = transactionResults.map(r => ({
        ...
        matches: (r.matches || []).map(m => ({
          field: m.key || '',
          value: m.value || '',
          indices: Array.from(m.indices || []) as [number, number][]
        }))
      }));
    }
  ...
  parseNaturalLanguageQuery(query: string): SearchOptions {
    ...
    if (amountMatch && amountMatch[1] && amountMatch[0]) {
      options.amountMin = toDecimal(amountMatch[1]).toNumber();
      query = query.replace(amountMatch[0], '').trim();
    }

    const underMatch = query.match(/(?:under|below|less than|<)\s*\$?(\d+(?:\.\d+)?)/i);
    if (underMatch && underMatch[1] && underMatch[0]) {
      options.amountMax = toDecimal(underMatch[1]).toNumber();
