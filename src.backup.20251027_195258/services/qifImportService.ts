...
export class QIFImportService {
  ...
  private parseAmount(value: string): number {
    ...
    return toStorageNumber(amountDecimal);
  }
  ...
  async importTransactions(
    ...
    for (const qifTrx of parseResult.transactions) {
      ...
      const transaction: Omit<Transaction, 'id'> = {
        ...
        isRecurring: false,
        ...(qifTrx.checkNumber && { notes: `Check #: ${qifTrx.checkNumber}` })
      };
      ...
      if (options.autoCategorize && options.categories && !transaction.category) {
        ...
        const suggestions = smartCategorizationService.suggestCategories({
          ...transaction,
          id: '' // temporary id for categorization
        }, 1);
