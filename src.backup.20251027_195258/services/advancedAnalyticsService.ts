...
import { subMonths, differenceInDays, format, addMonths } from 'date-fns';
...
const roundToInt = (value: DecimalInstance | number): number =>
  toDecimal(value).toDecimalPlaces(0).toNumber();

const clampAndRoundToInt = (value: DecimalInstance | number): number => {
  const decimal = toDecimal(value);
  const nonNegative = decimal.lessThan(0) ? toDecimal(0) : decimal;
  return nonNegative.toDecimalPlaces(0).toNumber();
};
...
class AdvancedAnalyticsService {
  ...
  detectSpendingAnomalies(
    ...
    recentTransactions.forEach(transaction => {
      ...
      if (Math.abs(zScore) > threshold && amount.greaterThan(stats.mean)) {
        const percentageAboveDecimal = stats.mean.greaterThan(0)
          ? amount.minus(stats.mean).dividedBy(stats.mean).times(100)
          : toDecimal(0);
        const percentageAboveNumber = percentageAboveDecimal.toNumber();
        const roundedPercentageAbove = clampAndRoundToInt(percentageAboveDecimal);
        ...
        if (amount.greaterThan(stats.mean.plus(minimumAnomalyAmount))) {
          anomalies.push({
            ...
            severity: this.determineSeverity(zScore, percentageAboveNumber, amount.toNumber()),
            reason: this.getAnomalyReason(transaction, zScore, percentageAboveDecimal),
            percentageAboveNormal: roundedPercentageAbove
          });
        }
      }
    });
  ...
  private getAnomalyReason(
    ...
    zScore: number,
    percentageAboveDecimal: DecimalInstance
  ): string {
    const safePercentageDecimal = percentageAboveDecimal.lessThan(0)
      ? toDecimal(0)
      : percentageAboveDecimal;
    const percentageAboveDisplay = formatPercentageValue(safePercentageDecimal, 0);
    ...
    if (description.includes('annual') || description.includes('yearly') || description.includes('subscription')) {
      return `Annual or subscription payment - ${percentageAboveDisplay} above typical ${category} spending`;
    }
    ...
    if (zScore > 3.5) {
      return `Extremely high ${category} expense - ${percentageAboveDisplay} above your typical spending pattern`;
    } else if (zScore > 2.5) {
      return `Significantly above average ${category} spending - worth reviewing for accuracy`;
    } else {
      return `Higher than typical ${category} expense - ${percentageAboveDisplay} above your average`;
    }
  }
      ...
      if (trans.length >= 3) {
        ...
        if (firstDate && lastDate && firstTransaction && dates.length > 1) {
          ...
          if (daysDiff >= 25 && daysDiff <= 35) {
            ...
            const subscriptionAmount = toDecimal(Math.abs(firstTransaction.amount));
            const subscription: Subscription = {
              id: `sub-${Date.now()}-${Math.random()}`,
              merchant: firstTransaction.merchant ?? firstTransaction.description,
              amount: subscriptionAmount,
              frequency: 'monthly',
              category: firstTransaction.category || 'Other',
              lastChargeDate: lastDate,
              nextChargeDate: addMonths(lastDate, 1),
              isActive: true,
  ...
  private analyzeCategorySpending(transactions: Transaction[]): Map<string, CategorySpendingStats> {
    ...
    categoryGroups.forEach((categoryTrans, category) => {
      ...
      if (olderMonthlyAvg.greaterThan(0)) {
        const changeRatio = recentMonthlyAvg.minus(olderMonthlyAvg).dividedBy(olderMonthlyAvg);
        const changeNumber = changeRatio.toNumber();
        
        if (changeNumber > 0.1) {
          trend = 'increasing';
        } else if (changeNumber < -0.1) {
          trend = 'decreasing';
        }

        percentageIncrease = roundToInt(changeRatio.times(100));
      }
  ...
  private findDuplicateServices(transactions: Transaction[]): DuplicateService[] {
    ...
    serviceCategories.forEach((services, category) => {
      ...
      if (merchants.size >= 2) {
        ...
        // Note: Recommendations could be added to DuplicateService interface in future
        // Currently just identifying duplicate services without recommendation text
  ...
  private calculateSpendingVelocity(transactions: Transaction[]): SpendingVelocity {
    ...
    const percentageIncreaseDecimal = previousTotal.greaterThan(0)
      ? recentTotal.minus(previousTotal).dividedBy(previousTotal).times(100)
      : recentTotal.greaterThan(0) ? toDecimal(100) : toDecimal(0);
    const percentageIncreaseNumber = percentageIncreaseDecimal.toNumber();

    let trend: SpendingVelocity['trend'] = 'stable';
    if (percentageIncreaseNumber > 10) {
      trend = 'accelerating';
    } else if (percentageIncreaseNumber < -10) {
      trend = 'decelerating';
    }
    ...
    return {
      ...
      trend,
      isAccelerating: percentageIncreaseDecimal.greaterThan(toDecimal(5)),
      percentageIncrease: roundToInt(percentageIncreaseDecimal)
    };
  }

  private analyzeSavingsBehavior(transactions: Transaction[], _accounts: Account[]): SavingsBehavior {
    ...
    const monthlyRates = sortedKeys.map((key) => {
      const bucket = monthlyTotals.get(key)!;
    ...
    return {
      monthlySavingsRate: clampAndRoundToInt(currentSavingsRate),
      averageMonthlySavings,
      savingsStreak: consecutivePositive,
      totalSaved: totalSavings.greaterThan(0) ? totalSavings : toDecimal(0),
      savingsTrend,
      consistentSaving,
      averagePercentage: clampAndRoundToInt(averagePercentage)
    };
  }
  ...
  private analyzeBudgetPerformance(budget: Budget, transactions: Transaction[]): BudgetPerformance {
    ...
    const utilizationPercentageDecimal = budgetAmount.greaterThan(0)
      ? totalSpent.dividedBy(budgetAmount.times(Math.max(1, monthsTracked))).times(100)
      : toDecimal(0);
    const averageUtilization = clampAndRoundToInt(utilizationPercentageDecimal);

    const adherenceRateDecimal = monthsTracked > 0
      ? toDecimal(monthsTracked - overBudgetMonths).dividedBy(monthsTracked).times(100)
      : toDecimal(100);
    const adherenceRate = clampAndRoundToInt(adherenceRateDecimal);
    ...
    const averageUsage = clampAndRoundToInt(utilizationPercentageDecimal);
