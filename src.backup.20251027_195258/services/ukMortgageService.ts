...
class UKMortgageService {
  ...
  calculateMortgage(
    ...
    interestRate: number,
    termYears: number,
    _mortgageType: 'fixed' | 'variable' | 'tracker' = 'fixed',
    _fixedTermYears?: number
  ...
  calculateStampDuty(
    ...
    const residential = regionalData.residential;
    const thresholds = residential.thresholds;
    let totalTax = 0;
    ...
    if (firstTimeBuyer && region === 'england') {
      const ftbData = 'firstTimeBuyer' in residential ? residential.firstTimeBuyer : null;
      if (ftbData) {
