...
class AnalyticsEngine {
  ...
  detectSeasonalPatterns(
    ...
    const regressionData: Array<[number, number]> = values.map((y, x) => [x, y]);
    const result: RegressionResult = regression.linear(regressionData);
    
    // Detrend the data
    const detrended = values.map((y, x) => {
    ...
    return {
      ...
      slope,
      rSquared: result.r2,
      changeRate: (slope / safeBaseline) * 100,
      seasonality: {
  ...
  executeQuery(
    ...
    if (query.orderBy) {
      results.sort((a, b) => {
        ...
        const diff = (typeof aVal === 'number' ? aVal : 0) - (typeof bVal === 'number' ? bVal : 0);
  ...
  private applyFilters(transactions: Transaction[], filters: SegmentFilter[]): Transaction[] {
    ...
    const toComparableString = (input: unknown): string | null => {
    ...
    return transactions.filter(transaction => {
      return filters.every(filter => {
        ...
        switch (filter.operator) {
          ...
          case 'contains':
            {
              const base = toComparableString(value);
              const search = toComparableString(filterValue);
              return base !== null && search !== null ? base.includes(search) : false;
            }
          case 'greater':
            {
              const numericValue = parseNumber(value);
              const numericFilter = parseNumber(filterValue);
              return numericValue !== null && numericFilter !== null ? numericValue > numericFilter : false;
            }
          case 'less':
            {
              const numericValue = parseNumber(value);
          ...
          case 'between':
            {
              ...
              const [minRaw, maxRaw] = filterValue;
              const numericValue = parseNumber(value);
              const min = parseNumber(minRaw);
              const max = parseNumber(maxRaw);
              return numericValue !== null && min !== null && max !== null
          ...
          case 'in':
            {
              ...
              return filterValue.some(candidate => candidate === value);
            }
          default:
            return true;
        }
  ...
  private calculateCustomMetric(
    ...
    if (metric === 'income' || metric === 'expenses' || metric === 'net' || metric === 'count') {
      return this.calculateMetric(transactions, metric);
    }
