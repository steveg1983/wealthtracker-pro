...
class CashFlowForecastService {
  ...
  private analyzeTransactionGroup(transactions: Transaction[]): RecurringPattern | null {
    ...
    for (let i = 1; i < sorted.length; i++) {
      ...
      const millisDiff = new Date(current.date).getTime() - new Date(previous.date).getTime();
      const days = toDecimal(millisDiff)
    ...
    const nextOccurrence = this.calculateNextOccurrence(lastDate, frequency);
    ...
    const pattern: RecurringPattern = {
      ...
      frequency,
      confidence: toDecimal(confidence).toDecimalPlaces(0).toNumber(),
    ...
    if (frequency === 'monthly') {
      pattern.dayOfMonth = lastDate.getDate();
    }

    if (frequency === 'weekly') {
      pattern.dayOfWeek = lastDate.getDay();
    }
