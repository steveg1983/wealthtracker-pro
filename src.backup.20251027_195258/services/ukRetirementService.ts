...
interface PersonalPensionProjection {
  currentValue: number;
...
export interface UKRetirementProjection {
  ...
  personalPension?: PersonalPensionProjection;
  isa?: ISAProjection;
...
class UKRetirementService {
  ...
  calculateFullRetirementProjection(params: {
    ...
    return {
      ...
      taxOnRetirement: {
        incomeTax: 'incomeTax' in taxResult ? taxResult.incomeTax : 0,
        effectiveRate: taxResult.effectiveRate,
        netIncome: totalAnnualIncome - ('incomeTax' in taxResult ? taxResult.incomeTax : 0)
      },
      ...(workplacePension && { workplacePension }),
      ...(isa && { isa })
    };
  }
  ...
  calculateNIContributions(annualIncome: number, selfEmployed: boolean = false): {
    ...
    if (selfEmployed) {
      ...
      const nationalInsurance = 'nationalInsurance' in result ? result.nationalInsurance : 0;
      return {
        weeklyContribution: nationalInsurance / 52,
        annualContribution: nationalInsurance,
