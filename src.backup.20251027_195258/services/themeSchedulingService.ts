...
class ThemeSchedulingService {
  ...
  private schedulingInterval: IntervalHandle | null = null;
  ...
  updateSchedule(
    id: string,
    updates: Partial<Omit<ThemeSchedule, 'id' | 'createdAt' | 'lastUpdated'>>
  ): ThemeSchedule | null {
    ...
    if (this.currentSchedule?.id === id) {
      this.currentSchedule = this.schedules[index] || null;
    }
  ...
  updatePreset(id: string, updates: Partial<ThemePreset>): ThemePreset | null {
    ...
    if (existingPreset) {
      this.presets[index] = { ...existingPreset, ...updates };
    }
    this.saveData();
  ...
  private startScheduling(): void {
    // Check every minute
    this.schedulingInterval = globalThis.setInterval(() => {
  ...
  cleanup(): void {
    if (this.schedulingInterval) {
      globalThis.clearInterval(this.schedulingInterval);
