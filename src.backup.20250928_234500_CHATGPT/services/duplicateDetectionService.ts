...
export class DuplicateDetectionService {
  ...
  private static compareTransactions(
    ...
    if (Math.abs(parseFloat(String(tx1.amount)) - parseFloat(String(tx2.amount))) < 0.01) {
      confidence += 40;
      matchReasons.push('Exact amount match');
    } else if (Math.abs(parseFloat(String(tx1.amount)) - parseFloat(String(tx2.amount))) < 1) {
      confidence += 20;
