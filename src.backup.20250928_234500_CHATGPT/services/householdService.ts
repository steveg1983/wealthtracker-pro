...
class HouseholdService {
  ...
  private loadData() {
    try {
      ...
      if (householdData) {
        ...
        this.household = this.normalizeHousehold(parsed);
      }
      ...
      if (invitesData) {
        ...
        this.invites = Array.isArray(parsedInvites)
          ? parsedInvites.map(invite => this.normalizeInvite(invite))
          : [];
      }
      ...
      if (activitiesData) {
        ...
        this.activities = Array.isArray(parsedActivities)
          ? parsedActivities.map(activity => this.normalizeActivity(activity))
          : [];
      }
    } catch (error) {
  ...
  private normalizeHousehold(raw: any): Household {
    const members: HouseholdMember[] = Array.isArray(raw?.members)
      ? raw.members.map((member: any, index: number) => this.normalizeMember(member, index))
      : [];

    return {
      id: typeof raw?.id === 'string' ? raw.id : this.generateId(),
      name: typeof raw?.name === 'string' ? raw.name : 'Household',
      createdAt: raw?.createdAt ? new Date(raw.createdAt) : new Date(),
      createdBy: typeof raw?.createdBy === 'string' ? raw.createdBy : members[0]?.id ?? this.generateId(),
      members,
      settings: this.normalizeSettings(raw?.settings)
    };
  }

  private normalizeMember(raw: any, index: number): HouseholdMember {
    const role: HouseholdMember['role'] = ['owner', 'admin', 'member', 'viewer'].includes(raw?.role)
      ? raw.role
      : 'member';

    const permissions: MemberPermissions = raw?.permissions
      ? {
          canViewAccounts: Boolean(raw.permissions.canViewAccounts),
          canEditAccounts: Boolean(raw.permissions.canEditAccounts),
          canViewTransactions: Boolean(raw.permissions.canViewTransactions),
          canEditTransactions: Boolean(raw.permissions.canEditTransactions),
          canViewBudgets: Boolean(raw.permissions.canViewBudgets),
          canEditBudgets: Boolean(raw.permissions.canEditBudgets),
          canViewGoals: Boolean(raw.permissions.canViewGoals),
          canEditGoals: Boolean(raw.permissions.canEditGoals),
          canInviteMembers: Boolean(raw.permissions.canInviteMembers),
          canRemoveMembers: Boolean(raw.permissions.canRemoveMembers)
        }
      : this.getDefaultPermissions(role);

    const member: HouseholdMember = {
      id: typeof raw?.id === 'string' ? raw.id : this.generateId(),
      name: typeof raw?.name === 'string' ? raw.name : 'Member',
      email: typeof raw?.email === 'string' ? raw.email : '',
      role,
      color: typeof raw?.color === 'string' ? raw.color : this.generateMemberColor(index),
    ...
    if (raw?.lastActive) {
      member.lastActive = new Date(raw.lastActive);
    }

    return member;
  }

  private normalizeSettings(raw: any): HouseholdSettings {
    return {
      currency: typeof raw?.currency === 'string' ? raw.currency : 'USD',
      timezone: typeof raw?.timezone === 'string'
        ? raw.timezone
        : Intl.DateTimeFormat().resolvedOptions().timeZone,
      fiscalYearStart: typeof raw?.fiscalYearStart === 'number' ? raw.fiscalYearStart : 1,
      allowMemberExpenses: Boolean(raw?.allowMemberExpenses ?? true),
      ...
      largeTransactionThreshold: typeof raw?.largeTransactionThreshold === 'number'
        ...
        : 1000,
      sharedCategories: Boolean(raw?.sharedCategories ?? true),
      memberVisibility: ['all', 'summary', 'none'].includes(raw?.memberVisibility)
        ? raw.memberVisibility
        : 'all'
    };
  }

  private normalizeInvite(raw: any): HouseholdInvite {
    return {
      id: typeof raw?.id === 'string' ? raw.id : this.generateId(),
      householdId: typeof raw?.householdId === 'string' ? raw.householdId : this.household?.id ?? this.generateId(),
      email: typeof raw?.email === 'string' ? raw.email : '',
      role: ['owner', 'admin', 'member', 'viewer'].includes(raw?.role) ? raw.role : 'member',
      invitedBy: typeof raw?.invitedBy === 'string' ? raw.invitedBy : '',
      invitedAt: raw?.invitedAt ? new Date(raw.invitedAt) : new Date(),
      expiresAt: raw?.expiresAt ? new Date(raw.expiresAt) : new Date(Date.now() + 7 * 24 * 60 * 60 * 1000),
      status: ['pending', 'accepted', 'declined', 'expired'].includes(raw?.status)
        ? raw.status
        : 'pending',
      token: typeof raw?.token === 'string' ? raw.token : this.generateToken()
    };
  }

  private normalizeActivity(raw: any): MemberActivity {
    const activity: MemberActivity = {
      id: typeof raw?.id === 'string' ? raw.id : this.generateId(),
      memberId: typeof raw?.memberId === 'string' ? raw.memberId : '',
      memberName: typeof raw?.memberName === 'string' ? raw.memberName : 'Unknown',
    ...
    if (typeof raw?.entityId === 'string') {
      activity.entityId = raw.entityId;
    }

    if (['transaction', 'account', 'budget', 'goal'].includes(raw?.entityType)) {
      activity.entityType = raw.entityType;
    }

    return activity;
  }

  private normalizeActivityAction(action: any): MemberActivity['action'] {
    ...
    return actions.includes(action) ? action : 'joined_household';
  }
