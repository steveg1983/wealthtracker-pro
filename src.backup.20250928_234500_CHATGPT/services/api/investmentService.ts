...
class InvestmentService {
    ...
    try {
      ...
      if (!currentPrice && investment.symbol) {
        try {
          ...
          if (priceData) {
            currentPrice = typeof (priceData.price as any)?.toNumber === 'function'
              ? (priceData.price as any).toNumber()
  ...
  private async updateInvestmentPrices(investments: any[]): Promise<void> {
    ...
    try {
      ...
      for (const investment of investments) {
        ...
        if (quote) {
          const price = typeof (quote.price as any)?.toNumber === 'function'
            ? (quote.price as any).toNumber()
            : Number(quote.price);

          investment.current_price = price;
          investment.market_value = Number(investment.quantity) * price;
        }
      }
    } catch (error) {
  ...
  private mapInvestment(data: any): Investment {
    ...
    const costBasis = typeof data.cost_basis === 'number' ? data.cost_basis : Number(data.cost_basis ?? 0);
    const marketValue = typeof data.market_value === 'number'
      ? data.market_value
      : Number(data.market_value ?? costBasis);
    const averageCost = quantity > 0 ? costBasis / quantity : undefined;
    ...
    if (data.account_id) {
      investment.accountId = data.account_id;
    }
    if (typeof data.current_price === 'number' || data.current_price) {
      investment.currentPrice = Number(data.current_price);
    }
    if (data.exchange) investment.exchange = data.exchange;
    if (data.purchase_date) investment.purchaseDate = new Date(data.purchase_date);
    if (typeof data.purchase_price === 'number' || data.purchase_price) {
      investment.purchasePrice = Number(data.purchase_price);
    }
    if (data.last_updated) investment.lastUpdated = new Date(data.last_updated);
    if (data.notes) investment.notes = data.notes;
