  ...
  createExpenseSplit(
    ...
    if (splitMethod === 'equal') {
      ...
      splits = activeMembers.map(member => {
        ...
        return member.id === 'current-user'
          ? { ...baseSplit, paidAt: new Date() }
          : baseSplit;
      });
      ...
      splits = customSplits.map(split => {
        ...
        return split.memberId === 'current-user'
          ? { ...baseSplit, paidAt: new Date() }
          : baseSplit;
      });
      ...
      splits = customSplits.map(split => {
        ...
        const splitWithPercentage = split.percentage !== undefined
          ? { ...baseSplit, percentage: split.percentage }
          : baseSplit;

        return split.memberId === 'current-user'
          ? { ...splitWithPercentage, paidAt: new Date() }
          : splitWithPercentage;
      });
    } else {
      return null;
