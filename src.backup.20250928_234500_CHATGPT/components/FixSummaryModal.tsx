...
function FixSummaryModal({
  ...
  const groupedChanges = changes.reduce<Record<string, ChangeRecord[]>>((acc, change) => {
    const issueType = change.issueType || 'Other';
    const bucket = acc[issueType] ?? (acc[issueType] = []);
    bucket.push(change);
    return acc;
  }, {});
