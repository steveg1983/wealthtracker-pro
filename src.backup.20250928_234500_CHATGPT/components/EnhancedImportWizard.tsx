        ...
        try {
          ...
          switch (fileInfo.type) {
            case 'csv': {
              ...
              // Apply import rules
              const processedTransactions = preview.transactions
                .map((transaction) => importRulesService.applyRules(transaction))
              ...
              for (const transaction of processedTransactions) {
                ...
                if (!Number.isFinite(numericAmount)) {
                  logger.warn('Skipping transaction with invalid amount:', transaction);
                  continue;
                }

                const isDuplicate = transactions.some(existing =>
                  existing.date.getTime() === normalizedDate.getTime() &&
            ...
            case 'qif': {
              ...
              if (!defaultAccountId) {
                logger.warn('Skipping QIF import because no default account could be determined');
                break;
              }
