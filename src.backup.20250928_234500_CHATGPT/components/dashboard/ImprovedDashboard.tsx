  ...
  const metrics = useMemo(() => {
    ...
    const accountsNeedingAttention = accounts.filter(acc => {
      ...
      if (acc.type === 'credit' && creditLimit && creditLimit > 0) {
        const utilization = Math.abs(acc.balance) / creditLimit;
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                      ...
                      {account.type === 'credit' && creditLimit && creditLimit > 0 && Math.abs(account.balance) / creditLimit > 0.7 ? 'High utilization' : ''}
                    </p>
                  </div>
                </div>
