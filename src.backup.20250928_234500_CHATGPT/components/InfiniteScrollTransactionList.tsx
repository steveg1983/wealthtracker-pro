...
export const InfiniteScrollTransactionList = memo(function InfiniteScrollTransactionList({
  ...
  useEffect(() => {
    ...
    observerRef.current = new IntersectionObserver(
      (entries) => {
        const [entry] = entries;
        if (entry && entry.isIntersecting && displayedItems < transactions.length && !isLoadingMore) {
          loadMoreItems();
        }
      },
      {
    ...
    <div className="relative">
      ...
      <div className="divide-y divide-gray-200 dark:divide-gray-700">
        {visibleTransactions.map((transaction) => {
          const account = accounts.find((a): a is typeof accounts[number] => a.id === transaction.accountId);
          const isSelected = selectedTransactions?.has(transaction.id) || false;

          // Skip transactions without a valid account
          if (!account) {
            return null;
          }
