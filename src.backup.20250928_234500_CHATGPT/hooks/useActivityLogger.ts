...
export function useActivityLogger() {
  ...
  useEffect(() => {
    ...
    if (now - transactionTime < 60_000) {
      const amount = typeof latest.amount === 'number' ? latest.amount : Number(latest.amount);
  ...
  useEffect(() => {
    ...
    budgets.forEach(budget => {
      ...
      const budgetLabel = budget.name ?? budgetCategoryId;
      const budgetAmount = typeof budget.amount === 'number' ? budget.amount : Number(budget.amount);
      if (!budgetCategoryId || !Number.isFinite(budgetAmount) || budgetAmount === 0) {
        return;
      }

      const spent = Math.abs(
        transactions
          .filter(t => t.category === budgetCategoryId && (typeof t.amount === 'number' ? t.amount : Number(t.amount)) < 0)
          .reduce((sum, t) => {
            const value = typeof t.amount === 'number' ? t.amount : Number(t.amount);
      ...
      if (percentage >= 90 && !alreadyAlerted) {
        logActivity({
          type: 'budget',
          title: 'Budget Alert',
          description: `${budgetLabel} budget is ${percentage.toFixed(0)}% spent`,
          actionUrl: '/budget'
        });
